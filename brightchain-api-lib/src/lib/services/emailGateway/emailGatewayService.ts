...
export class EmailGatewayService {
  ...
  private async handleAnnouncement(
    ...
    console.info(
      `[EmailGatewayService] Gossip announcement received for messageId=${delivery.messageId}`,
    );
    ...
    if (!metadata) {
      ...
      console.warn(
        `[EmailGatewayService] Message ${delivery.messageId} not found in EmailMessageService — skipping`,
      );
      return;
    }
    ...
    if (external.length === 0) {
      ...
      console.info(
        `[EmailGatewayService] Message ${delivery.messageId} has no external recipients — internal delivery only`,
      );
      return;
    }

    console.info(
      `[EmailGatewayService] Message ${delivery.messageId} has ${external.length} external recipient(s): [${external.join(', ')}]`,
    );

    if (!this.outboundQueue) {
      // Queue not wired yet — cannot enqueue. This is a configuration issue.
      console.error(
        `[EmailGatewayService] Message ${delivery.messageId} has external recipients but outbound queue is not wired — message will not be delivered`,
      );
      return;
    }
    ...
    console.info(
      `[EmailGatewayService] Message ${delivery.messageId} enqueued for outbound delivery to [${external.join(', ')}]`,
    );
  }
}
