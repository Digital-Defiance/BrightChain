...
describe('Complete Ingestion & Reconstruction Workflow', () => {
  ...
  describe('Single Block Ingestion & Reconstruction', () => {
    it('should ingest and reconstruct a small document', async () => {
      ...
      const originalDocument = {
        ...
        content:
          'This is a test document that will be ingested into BrightChain.',
      ...
      const whitenedChecksum =
        ServiceLocator.getServiceProvider().checksumService.calculateChecksum(
          whitenedData,
        );
      const primeWhitenedBlock = new WhitenedBlock(
        blockSize,
        whitenedData,
        whitenedChecksum,
      );
      ...
      for (const whitener of whiteners) {
        ...
        storedBlockIds.push(
          uint8ArrayToHex(whitener.idChecksum.toUint8Array()),
        );
      }

      // Store prime whitened block
      await memoryStore.put(
        primeWhitenedBlock.idChecksum,
        primeWhitenedBlock.data,
      );
      storedBlockIds.push(
        uint8ArrayToHex(primeWhitenedBlock.idChecksum.toUint8Array()),
      );
      ...
      const reconstructedChecksum =
        ServiceLocator.getServiceProvider().checksumService.calculateChecksum(
          reconstructedSource,
        );
      expect(uint8ArrayToHex(reconstructedChecksum.toUint8Array())).toBe(
        retrievedCbl.originalChecksum,
      );
  ...
  describe('Multi-Block Ingestion & Reconstruction', () => {
    it('should ingest and reconstruct a larger document', async () => {
      ...
      const largeDocument = {
        ...
        abstract:
          'This paper presents BrightChain, a novel distributed storage system based on the Owner Free Filesystem concept...',
        sections: [
          {
            id: 1,
            title: 'Introduction',
          ...
          },
          {
            ...
            content:
              'The system consists of three main components: the block store layer, the whitening layer, and the constituent block list management system. Each component plays a crucial role in ensuring data privacy and integrity.',
          },
          {
            id: 3,
            title: 'Implementation',
            content:
              'Our implementation uses TypeScript and Node.js, providing both disk-based and memory-based storage backends. The system supports multiple block sizes and automatic size selection based on data length.',
          },
        ],
        metadata: {
      ...
      const fullDocumentData = Buffer.from(
        JSON.stringify(largeDocument, null, 2),
      );
      ...
      for (let blockIndex = 0; blockIndex < numBlocks; blockIndex++) {
        ...
        const primeWhitened = new WhitenedBlock(
          blockSize,
          whitenedData,
          whitenedChecksum,
        );
        ...
        tupleBlockIds.push(
          uint8ArrayToHex(primeWhitened.idChecksum.toUint8Array()),
        );
      ...
      const masterCbl = {
        ...
        originalChecksum: uint8ArrayToHex(
          ServiceLocator.getServiceProvider()
            .checksumService.calculateChecksum(fullDocumentData)
            .toUint8Array(),
        ),
        numBlocks,
        blockSize,
      ...
      for (
        let blockIndex = 0;
        blockIndex < retrievedCbl.numBlocks;
        blockIndex++
      ) {
      ...
      // Combine blocks
      const reconstructedDocument = Buffer.concat(reconstructedBlocks).subarray(
        0,
        retrievedCbl.originalLength,
      );
      ...
      expect(parsedDoc.title).toBe(
        'BrightChain: A Revolutionary Approach to Distributed Storage',
      );
  ...
  describe('Disk Storage Workflow', () => {
    it('should ingest and reconstruct using disk storage', async () => {
      ...
      const document = {
        message:
          'This document is stored on actual disk using DiskBlockAsyncStore',
        persistent: true,
        timestamp: new Date().toISOString(),
      };
      ...
      // Create CBL
      const cbl = {
        blockIds: storedBlockChecksums.map((c) =>
          uint8ArrayToHex(c.toUint8Array()),
        ),
        originalLength: sourceData.length,
        storage: 'disk',
        storePath: tempDir,
        _sourceChecksum: uint8ArrayToHex(sourceChecksum.toUint8Array()),
      };
      ...
      for (let i = 0; i < TUPLE.SIZE - 1; i++) {
        retrievedWhiteners.push(
          await diskStore.getData(storedBlockChecksums[i]),
        );
      }
      const retrievedPrime = await diskStore.getData(
        storedBlockChecksums[TUPLE.SIZE - 1],
      );
      ...
      const recoveredDoc = JSON.parse(
        reconstructed.subarray(0, cbl.originalLength).toString(),
      );
  ...
  describe('Workflow Verification', () => {
    it('should demonstrate that intermediate source blocks are never stored', async () => {
      const secret = Buffer.from(
        'TOP SECRET: This should never be stored directly',
      );
      ...
      expect(reconstructed.subarray(0, secret.length).toString()).toBe(
        'TOP SECRET: This should never be stored directly',
      );
    });
    ...
    it('should demonstrate complete workflow with metadata', async () => {
      ...
      const cblIds = [
        ...whiteners.map((w) => uint8ArrayToHex(w.idChecksum.toUint8Array())),
        uint8ArrayToHex(prime.idChecksum.toUint8Array()),
      ];

      // Full reconstruction
      const retrieved = await Promise.all(
        cblIds.map((id) => memoryStore.get(id)),
      );
      ...
      const recovered = JSON.parse(
        reconstructed.subarray(0, data.length).toString(),
      );
