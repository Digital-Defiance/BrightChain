...
describe('Owner Free Filesystem (OFF) Integration Tests', () => {
  ...
  describe('Block Whitening (Owner Free Filesystem)', () => {
    it('should whiten a data block with random blocks', async () => {
      ...
      const whitenedBlock = new WhitenedBlock(
        blockSize,
        whitenedData,
        whitenedChecksum,
      );
      ...
      expect(reconstructed.subarray(0, originalMessage.length).toString()).toBe(
        'This is a secret message that should be obscured!',
      );
    });
    ...
    it('should handle multi-block document whitening', async () => {
      ...
      const fullDocument = Buffer.from(
        JSON.stringify(
          {
            title: 'BrightChain Technical Specification',
            version: '1.0',
            description:
              'A revolutionary decentralized storage network using Owner Free Filesystem concepts',
            features: [
      ...
      for (let i = 0; i < blocksNeeded; i++) {
        ...
        whitenedBlocks.push(
          new WhitenedBlock(blockSize, whitenedData, whitenedChecksum),
        );
      }
      ...
      for (let i = 0; i < whitenedBlocks.length; i++) {
        ...
        await memoryStore.put(
          whitenedBlocks[i].idChecksum,
          whitenedBlocks[i].data,
        );
        recipe.push(
          uint8ArrayToHex(whitenedBlocks[i].idChecksum.toUint8Array()),
        );
      }
    ...
    it('should demonstrate that individual blocks are meaningless', async () => {
      ...
      expect(fullXor.subarray(0, secretData.length).toString()).toBe(
        'TOP SECRET: Launch codes are 12345',
      );
    });
  });
  ...
  describe('BrightChain Writeup Examples', () => {
    it('should implement the "soup can" analogy from the writeup', async () => {
      ...
      expect(reconstructedM.subarray(0, M.length).toString()).toBe(
        'Secret Recipe for Alphabet Soup',
      );
    });
  });
  ...
  describe('Real-World Document Example', () => {
    it('should store and retrieve a JSON configuration file', async () => {
      ...
      const recoveredConfig = JSON.parse(
        reconstructed.subarray(0, configJson.length).toString(),
      );
