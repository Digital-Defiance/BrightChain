...
describe('ACL enforcement e2e (real ECDSA)', () => {
  ...
  describe('Open mode', () => {
    it('allows writes without any proof', async () => {
      ...
      const baseReg = PersistentHeadRegistry.create({
        dataDir: await makeTempDir(),
      });
      const authorizedReg = new AuthorizedHeadRegistry(
        baseReg as any,
        aclManager,
        auth,
      );
      ...
      await db
        .collection('data')
        .insertOne({ _id: 'o1', val: 'open' } as never);
  ...
  describe('Restricted mode', () => {
    it('authorized writer can write with valid proof (auto-sign)', async () => {
      ...
      await db
        .collection('data')
        .insertOne({ _id: 'r1', val: 'restricted' } as never);
  ...
  describe('ACL survives restart', () => {
    it('authorized writer can still write after restart', async () => {
      ...
      try {
        ...
        const authReg1 = new AuthorizedHeadRegistry(
          baseReg1 as any,
          aclManager1,
          auth,
        );
        authReg1.setLocalSigner({
          publicKey: writerKeys.publicKey,
          privateKey: writerKeys.privateKey,
        });
        ...
        const db1 = new BrightDb(store, {
          name: 'persistdb',
          headRegistry: authReg1,
        });
        await db1.connect();
        await db1
          .collection('items')
          .insertOne({ _id: 'p1', val: 'before-restart' } as never);
        ...
        await baseReg2.load();
        const authReg2 = new AuthorizedHeadRegistry(
          baseReg2 as any,
          aclManager2,
          auth,
        );
        authReg2.setLocalSigner({
          publicKey: writerKeys.publicKey,
          privateKey: writerKeys.privateKey,
        });
        ...
        await db2
          .collection('items')
          .insertOne({ _id: 'p2', val: 'after-restart' } as never);
    ...
    it('unauthorized writer is still rejected after restart', async () => {
      ...
      try {
        ...
        await db1
          .collection('items')
          .insertOne({ _id: 'g1', val: 'guarded' } as never);
        ...
        await expect(
          db2
            .collection('items')
            .insertOne({ _id: 'g2', val: 'intruder' } as never),
        ).rejects.toThrow();
      } finally {
        await fs.rm(dataDir, { recursive: true, force: true });
      }
  ...
  describe('OwnerOnly mode', () => {
    it('creator can write, non-creator cannot', async () => {
      ...
      const baseReg1 = PersistentHeadRegistry.create({
        dataDir: await makeTempDir(),
      });
      const authReg1 = new AuthorizedHeadRegistry(
        baseReg1 as any,
      ...
      await expect(
        db2
          .collection('data')
          .insertOne({ _id: 'ow2', val: 'intruder' } as never),
      ).rejects.toThrow();
    });
  });
  ...
  describe('collection-level ACL override', () => {
    it('collection ACL overrides database ACL', async () => {
      ...
      const authReg = new AuthorizedHeadRegistry(
        baseReg as any,
        aclManager,
        auth,
      );
      authReg.setLocalSigner({
        publicKey: unauthorizedKeys.publicKey,
        privateKey: unauthorizedKeys.privateKey,
      });
      ...
      await db
        .collection('public')
        .insertOne({ _id: 'pub1', val: 'open' } as never);
      ...
      await expect(
        db
          .collection('secret')
          .insertOne({ _id: 'sec1', val: 'nope' } as never),
      ).rejects.toThrow();
    });
  });
  ...
  describe('signature verification', () => {
    it('write proof with wrong private key is rejected', async () => {
      ...
      // Verify it fails
      const isValid = await aclManager.verifyWriteProof(
        proof,
        'sigdb',
        'data',
        'fake-block-id',
