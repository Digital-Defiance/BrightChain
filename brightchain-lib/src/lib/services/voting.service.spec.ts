...
describe('VotingService', () => {
  ...
  beforeAll(() => {
    ...
    votingKeypair = VotingService.generateVotingKeyPair() as TestKeyPair;

    // Pre-generate test keypairs
    testKeypairs = Array(3)
      .fill(null)
      .map(() => VotingService.generateVotingKeyPair() as TestKeyPair);
  });

  describe('private key algorithms', () => {
    it('should encrypt and decrypt a private key', () => {
      const encryptedPrivateKey = VotingService.keyPairToEncryptedPrivateKey(
      ...
      const decryptedPrivateKey = VotingService.encryptedPrivateKeyToKeyPair(
        encryptedPrivateKey,
        keyPair.privateKey,
    ...
    it('should maintain key security through encryption/decryption cycle', () => {
      ...
      for (const testKeypair of testKeypairs) {
        ...
        const encryptedPrivateKey2 = VotingService.keyPairToEncryptedPrivateKey(
          testKeypair as unknown as KeyPair,
          keyPair.publicKey,
        );
  ...
  describe('public key algorithms', () => {
    it('should encode and decode a public key', () => {
      ...
      const recoveredPublicKey = VotingService.bufferToVotingPublicKey(
    ...
    it('should handle invalid public key buffer', () => {
      ...
      expect(() => {
        VotingService.bufferToVotingPublicKey(invalidBuffer);
    ...
    it('should preserve key properties through buffer conversion', () => {
      ...
      for (const testKeypair of testKeypairs) {
        ...
        const recoveredKey = VotingService.bufferToVotingPublicKey(buffer);
