...
import { MemberCblService } from '../services/member/memberCblService'; // Re-add import
import {
  InstanceMethods,
  SchemaDefinition,
  SchemaType,
...
export class Document<T> {
  ...
  public static initialize(
    ...
    memberService?: MemberCblService, // Restore optional parameter, suppress unused warning
  ): void {
    if (Document.blockService) {
      throw new DocumentError(DocumentErrorType.AlreadyInitialized);
      // Note: memberService is intentionally not checked or assigned here in the base class
    }
  ...
  public async save(): Promise<void> {
    ...
    const blockSize = Document.blockService!.getBlockSizeForData(json.length);
    const xorResult = Buffer.alloc(blockSize as number);
    let firstBlock = true;
    // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
    await Document.blockService!.processFileInChunks(
      json,
      false,
      TUPLE.SIZE,
