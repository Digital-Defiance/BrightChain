...
const BlockExplorerListPage: FC<{ searchMode?: boolean }> = ({
  searchMode,
}) => {
  ...
  const fetchBlocks = useCallback(
    async (cursor?: string) => {
      ...
      try {
        ...
        setError(t(BrightChainStrings.Explorer_FetchFailed));
      } finally {
        setLoading(false);
      }
    },
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      setError(t(BrightChainStrings.Explorer_SearchFailed));
    }
  };
  ...
  return (
    <Box>
      <Typography variant="h5" mb={2}>
        {searchMode
          ? t(BrightChainStrings.Explorer_Nav_Search)
          : t(BrightChainStrings.Explorer_Title)}
      </Typography>
      ...
      {pools.length > 0 && (
        <Box display="flex" alignItems="center" gap={2} mb={2}>
          ...
          >
            {pools.map((p) => (
              <MenuItem key={p.poolId} value={p.poolId}>
                {p.poolId} (
                {t(BrightChainStrings.Explorer_PoolBlocksTemplate, {
                  COUNT: String(p.blockCount),
                })}
                )
              </MenuItem>
            ))}
          ...
          {!searchMode && (
            <Chip
              size="small"
              label={t(BrightChainStrings.Explorer_PoolBlocksTemplate, {
                COUNT: String(poolTotal),
              })}
              variant="outlined"
            />
          )}
      ...
      <Box display="flex" gap={1} mb={2}>
        <TextField
          size="small"
          placeholder={t(BrightChainStrings.Explorer_SearchPlaceholder)}
          value={searchQuery}
          onChange={(e) => {
        ...
        <Button variant="outlined" onClick={handleSearch}>
          {t(BrightChainStrings.Explorer_SearchButton)}
        </Button>
      </Box>
      ...
      {searchResults && (
        <Chip
          size="small"
          label={t(BrightChainStrings.Explorer_SearchResultsTemplate, {
      ...
      <Table size="small">
        ...
        <TableBody>
          {loading && displayBlocks.length === 0 ? (
            ...
            <TableRow>
              <TableCell colSpan={4} align="center">
                {t(BrightChainStrings.Explorer_NoBlocksFound)}
              </TableCell>
            </TableRow>
          ) : (
            displayBlocks.map((b) => (
              <TableRow
                ...
                hover
                sx={{ cursor: 'pointer' }}
                onClick={() => navigate(`/explorer/block/${b.hash}`)}
              >
                <TableCell
                ...
                <TableCell>
                  {b.metadata?.size != null ? `${b.metadata.size} B` : '—'}
                </TableCell>
                <TableCell>
      ...
      {!searchResults && (
        <Box display="flex" justifyContent="center" gap={2} mt={2}>
          ...
          <Typography variant="body2" sx={{ lineHeight: '30px' }}>
            {t(BrightChainStrings.Explorer_PageTemplate, {
              PAGE: String(cursorStack.length),
