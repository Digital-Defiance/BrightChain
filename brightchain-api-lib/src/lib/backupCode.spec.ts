...
import type { BrightChainMember } from './backendMember';
...
describe('BackupCode.generateBackupCodes', () => {
  it('generates the configured number of unique codes with valid format', () => {
    ...
    expect(list).toHaveLength(constants.BACKUP_CODES.Count);
    ...
    for (const bc of list) {
      ...
      const displayOk = constants.BACKUP_CODES.DisplayRegex.test(raw);
      const normalizedOk =
