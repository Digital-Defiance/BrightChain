...
describe('Feature: brighthub-social-network, Text_Formatter Property Tests', () => {
   ...
   *
   * `&`, `"`, and `'` are excluded because the underlying `sanitize-html`
   * pipeline used by `prepareContentForCharacterCount` HTML-escapes them
   * (e.g. `&` -> `&amp;`), which inflates the post-sanitization length and
   * breaks tests that compare counted length against the raw input length.
   */
