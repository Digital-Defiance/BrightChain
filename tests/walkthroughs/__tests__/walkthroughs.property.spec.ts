...
describe('Walkthroughs property-based tests', () => {
  it('all .md files are readable', () => {
    const files = fs.readdirSync(walkthroughsDir).filter(f => f.endsWith('.md') && f !== '_template.md');
    for (const file of files) {
      expect(() => fs.readFileSync(path.join(walkthroughsDir, file), 'utf-8')).not.toThrow();
    }
  });
});
