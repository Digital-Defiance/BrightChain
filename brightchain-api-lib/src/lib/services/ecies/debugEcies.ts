...
function debugEciesFunctions() {
  ...
  try {
    ...
    // Log key information
    console.log(
      `Alice Private Key (${alicePrivateKey.length} bytes): ${alicePrivateKey
        .toString('hex')
        .substring(0, 16)}...`,
    );
    console.log(
      `Alice Public Key (${
        alicePublicKeyWithPrefix.length
      } bytes): ${alicePublicKeyWithPrefix
        .toString('hex')
        .substring(0, 16)}...`,
    );
    console.log(
      `Alice Public Key Prefix: 0x${alicePublicKeyWithPrefix[0].toString(
        16,
      )}\n`,
    );

    console.log(
      `Bob Private Key (${bobPrivateKey.length} bytes): ${bobPrivateKey
        .toString('hex')
        .substring(0, 16)}...`,
    );
    console.log(
      `Bob Public Key (${
        bobPublicKeyWithPrefix.length
      } bytes): ${bobPublicKeyWithPrefix.toString('hex').substring(0, 16)}...`,
    );
    console.log(
    ...
    console.log(
      `Encrypted Data (${encrypted.length} bytes): ${encrypted
        .toString('hex')
        .substring(0, 32)}...\n`,
    );
    ...
    try {
      ...
      console.log(
        ...
        } bytes): ${encryptedContent.toString('hex').substring(0, 32)}...`,
      );
    }
  } catch (error) {
