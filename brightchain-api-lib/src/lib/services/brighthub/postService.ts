...
import type { UrlRewriterService } from './urlRewriterService';
...
export class PostService implements IPostService {
  ...
  private urlRewriter?: UrlRewriterService;
  ...
  /**
   * Set the URL rewriter service for inline image processing.
   * Called after construction to avoid circular dependency issues.
   */
  setUrlRewriter(service: UrlRewriterService): void {
    this.urlRewriter = service;
  }
  ...
  async createPost(
    ...
    // --- Inline image processing (URL Rewriter integration) ---
    let processedContent = content;
    let inlineMediaAttachments: IBaseMediaAttachment<string>[] = [];

    if (this.urlRewriter) {
      // Strip external image URLs from content
      processedContent = stripExternalImageUrls(processedContent);

      // Validate inline image count ≤ 20
      const imageCount = countInlineImages(processedContent);
      if (imageCount > getMaxInlineImages()) {
        throw new PostServiceError(
          PostErrorCode.TooManyInlineImages,
          `Post contains ${imageCount} inline images, maximum is ${getMaxInlineImages()}`,
        );
      }

      // Determine hub context for vault container scoping
      const hubId = isHubPost && options?.hubIds ? options.hubIds[0] : null;

      // Call URL Rewriter to commit staged images and rewrite URLs
      const rewriteResult = await this.urlRewriter.rewriteContent(
        processedContent,
        hubId,
        authorId,
      );
      processedContent = rewriteResult.rewrittenContent;
      inlineMediaAttachments = rewriteResult.mediaAttachments;
    }
    ...
    // Format content (using rewritten content if URL rewriter was applied)
    const isBlogPost = !options?.parentPostId;
    const formatted = this.textFormatter.format(processedContent, { isBlogPost });
    ...
    const postRecord: PostRecord = {
      ...
      mediaAttachments:
        inlineMediaAttachments.length > 0
          ? inlineMediaAttachments
  ...
  async editPost(
    ...
    // --- Inline image processing for edits (URL Rewriter integration) ---
    let processedContent = newContent;
    let finalMediaAttachments: IBaseMediaAttachment<string>[] =
      post.mediaAttachments || [];

    if (this.urlRewriter) {
      // Strip external image URLs from the new content
      processedContent = stripExternalImageUrls(processedContent);

      // Extract staging tokens (new images to commit) and permanent file IDs (existing images to retain)
      const stagingTokens = extractStagingTokens(processedContent);
      const permanentFileIds = extractPermanentFileIds(processedContent);

      // If there are new staging URLs, call the rewriter to commit them
      let newMediaAttachments: IBaseMediaAttachment<string>[] = [];
      if (stagingTokens.length > 0) {
        // Determine hub context for vault container scoping
        const hubId =
          post.hubIds && post.hubIds.length > 0 ? post.hubIds[0] : null;
        ...
        processedContent = rewriteResult.rewrittenContent;
        newMediaAttachments = rewriteResult.mediaAttachments;
      }

      // Build final mediaAttachments:
      // - Keep existing attachments whose file IDs are still in the content (permanent URLs)
      // - Add new attachments from the rewrite result
      const retainedAttachments = (post.mediaAttachments || []).filter((att) =>
        permanentFileIds.includes(att._id as string),
      );
      finalMediaAttachments = [...retainedAttachments, ...newMediaAttachments];
    }

    // Format new content (using rewritten content if URL rewriter was applied)
    const formatted = this.textFormatter.format(processedContent, {
  ...
  async createQuotePost(
    ...
    // Format content
    const formatted = this.textFormatter.format(commentary, { isBlogPost: true });
   ...
   * Unpins any previously pinned post by the same author.
   */
  async pinPost(postId: string, userId: string): Promise<void> {
    ...
    // Unpin any currently pinned post by this author
    const currentlyPinned = await this.postsCollection
      .find({ authorId: userId, isPinned: true } as Partial<PostRecord>)
      .exec();
    for (const pinned of currentlyPinned) {
      await this.postsCollection
        .updateOne(
          { _id: pinned._id },
          { isPinned: false, updatedAt: now } as Partial<PostRecord>,
        )
        .exec();
    }
    ...
    await this.postsCollection
      .updateOne(
        { _id: postId },
        { isPinned: true, updatedAt: now } as Partial<PostRecord>,
      )
      .exec();
  }
  ...
  async unpinPost(postId: string, userId: string): Promise<void> {
    ...
    if (post.authorId !== userId) {
      throw new PostServiceError(
        PostErrorCode.Unauthorized,
        'You can only unpin your own posts',
      );
    }
