 ...
 */
export class DiskBlockAsyncStore
  ...
  implements IBlockStore, IPooledBlockStore
{
  ...
  public async *listBlocksInPool(
    ...
    for (const size of this._supportedBlockSizes) {
      ...
      try {
        char1Dirs = await readdir(poolPath);
      } catch {
        continue;
      }
      ...
      for (const c1 of char1Dirs.sort()) {
        ...
        try {
          char2Dirs = await readdir(c1Path);
        } catch {
          continue;
        }
  ...
  public async storeCBLWithWhiteningInPool(
    ...
    if (poolRandomBlocks.length > 0) {
      ...
      randomBlock =
        existingData.length >= paddedCbl.length
   ...
   */
  public override async *listBlockIds(options?: {
    limit?: number;
    cursor?: string;
  }): AsyncIterable<string> {
    const limit = options?.limit;
    ...
    for (const size of this._supportedBlockSizes) {
      ...
      let char1Dirs: string[];
      try {
        char1Dirs = await readdir(basePath);
      } catch {
        continue;
      }
