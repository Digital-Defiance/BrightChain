      ...
      fc.asyncProperty(
        fc.array(
          fc.record({
            ...
            recipients: fc.array(fc.string({ minLength: 64, maxLength: 64 }), { minLength: 0, maxLength: 5 }),
            messageType: fc.constantFrom('chat', 'notification', 'system', 'custom'),
            priority: fc.constantFrom(MessagePriority.LOW, MessagePriority.NORMAL, MessagePriority.HIGH),
        ...
        async (messages) => {
          ...
          const uniqueMessages = messages
            .filter((msg, idx, arr) => arr.findIndex(m => m.blockId === msg.blockId) === idx)
          ...
          if (senderIds.length > 0) {
            ...
            expect(results.length).toBe(uniqueMessages.filter(m => m.senderId === senderId).length);
          }

          // Test recipient filtering
          const allRecipients = [...new Set(uniqueMessages.flatMap(m => m.recipients))];
          if (allRecipients.length > 0) {
            const recipientId = allRecipients[0];
            const results = await store.queryMessages({ recipientId });
            expect(results.every(r => r.recipients.includes(recipientId))).toBe(true);
          }
          ...
          if (types.length > 0) {
            ...
            expect(results.every(r => r.messageType === messageType)).toBe(true);
            expect(results.length).toBe(uniqueMessages.filter(m => m.messageType === messageType).length);
          }

          // Test priority filtering
          const priorities = [...new Set(uniqueMessages.map(m => m.priority))];
          ...
          const sortedDates = uniqueMessages.map(m => m.timestamp).sort((a, b) => a.getTime() - b.getTime());
          if (sortedDates.length >= 4) {
            ...
            expect(results.every(r => r.createdAt >= startDate && r.createdAt <= endDate)).toBe(true);
          }
        }
      ),
      { numRuns: 100 }
