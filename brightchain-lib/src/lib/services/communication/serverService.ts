...
export class ServerService {
  ...
  /**
   * Load persisted servers and server invite tokens back into in-memory Maps.
   *
   * Idempotent: only the first call performs rehydration. No-op when no
   * storage provider was supplied at construction time.
   *
   * Requirements: 1.4, 1.5, 1.6, 5.1, 5.2, 8.1, 8.2, 9.1
   */
  public async init(): Promise<void> {
    if (this.initialized) return;
    if (!this.serverCollection) return;

    this.initialized = true;

    // ── Load servers ──────────────────────────────────────────────────
    let loadedServers: IServer[];
    try {
      loadedServers = await this.serverCollection.findMany();
    } catch (error) {
      console.error(
        '[ServerService] Failed to load from servers collection:',
        error,
      );
      throw error;
    }
    ...
    // ── Load server invite tokens ───────────────────────────────────
    let loadedInvites: IServerInviteToken[];
    try {
      loadedInvites = await this.serverInviteCollection!.findMany();
    } catch (error) {
      console.error(
        '[ServerService] Failed to load from serverInvites collection:',
        error,
      );
      throw error;
    }
  ...
  async createServer(
    ...
    // serverId is already set by createChannel above
