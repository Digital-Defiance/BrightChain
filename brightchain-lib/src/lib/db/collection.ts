...
import type { IReadConcernBlockStore } from '../interfaces/storage/readConcernBlockStore';
...
> implements ICollection<T> {
  ...
  /** UUID generator for creating document IDs */
  ...
  constructor(
    ...
    this.uuidGenerator = uuidGenerator ?? createDefaultUuidGenerator();
  }
  ...
  private async writeDoc(doc: T, logicalId?: DocumentId): Promise<T> {
    const id = logicalId ?? doc._id ?? this.uuidGenerator().replace(/-/g, '');
  ...
  async insertOne(doc: T, options?: WriteOptions): Promise<InsertOneResult> {
    ...
    if (options?.session && (options.session as DbSession).inTransaction) {
      ...
      const id = doc._id ?? this.uuidGenerator().replace(/-/g, '');
      const docWithId = { ...doc, _id: id } as T;
