...
export const ConsentBasedDemo = () => {
  ...
  const [voters] = useState(['Alice', 'Bob', 'Charlie', 'Diana', 'Eve', 'Frank', 'Grace', 'Henry']);
  const [votes, setVotes] = useState<Map<string, { consent: boolean; objection?: string }>>(new Map());
      ...
      <div className="voting-demo">
        <div className="election-intro">
          ...
          <div className="intro-story">
            <p>🏢 <strong>Sociocracy:</strong> A worker cooperative needs to make decisions that everyone can live with.</p>
            <p>🙋 <strong>Consent-Based:</strong> Not about agreement - it's about "no strong objections". Can you live with this?</p>
            <div className="intro-stakes security-warning">
              <p>⚠️ <strong>INSECURE METHOD:</strong> No privacy - objections must be heard and addressed!</p>
              <p>🎯 <strong>The Question:</strong> "Do you have a principled objection that would harm the organization?"</p>
            </div>
            <p className="intro-challenge">🌍 Used in sociocratic organizations, holacracy, and collaborative workplaces!</p>
          </div>
  ...
  const consentCount = Array.from(votes.values()).filter(v => v.consent).length;
  const objectionCount = Array.from(votes.values()).filter(v => !v.consent).length;

  return (
    <div className="voting-demo">
      <div className="demo-header">
        <h3>🙋 Consent-Based - Worker Co-op</h3>
        <p className="election-tagline">🤝 No strong objections = consent achieved</p>
      </div>
      ...
      {!results && (
        <>
          <div className="consent-question">
            ...
            <p><strong>The Question:</strong> Do you have a <em>principled objection</em> that would harm our organization?</p>
            <p className="consent-note">Note: "I prefer 5 days" is not a principled objection. "This would bankrupt us" is.</p>
          </div>
          ...
          <div className="consent-tracker">
            ...
            {objectionCount > 0 && (
              <p className="objection-warning">
                ⚠️ {objectionCount} principled objection(s) raised - proposal must be modified or withdrawn
              </p>
            )}
          </div>

          <div className="voters-section">
            <h4>Circle Members ({votes.size}/{voters.length} responded)</h4>
            <div className="consent-votes">
              {voters.map(voter => {
                ...
                return (
                  <div key={voter} className={`consent-voter ${hasVoted ? (vote?.consent ? 'consent' : 'objection') : ''}`}>
                    <strong>{voter}</strong>
                    {hasVoted ? (
                      <div className="vote-display">
                        {vote?.consent ? (
                          ...
                          <div className="objection-display">
                            <span className="objection-badge">🚫 Principled Objection</span>
                            {vote?.objection && (
                              <p className="objection-text">"{vote.objection}"</p>
                            )}
                          </div>
                        )}
                      </div>
                      ...
                      <div className="consent-buttons">
                        <button onClick={() => castVote(voter, true)} className="vote-btn consent-btn">
                          ✅ No Objection
                        </button>
                        <button 
                          onClick={() => {
                            const objection = prompt(`${voter}, what is your principled objection?`);
        ...
        <div className="results-section">
          ...
          <div className="tally-visualization">
            ...
            <p className="tally-explain">
              Consent achieved if zero principled objections<br/>
            ...
            <div className="consent-breakdown">
              ...
              {results.objections.length > 0 && (
                <div className="objection-group">
                  <h5>🚫 Principled Objections ({results.objections.length})</h5>
          ...
          <div className="referendum-outcome">
            <h3>{results.passes ? '✅ CONSENT ACHIEVED!' : '🚫 CONSENT BLOCKED!'}</h3>
            <p>
              {results.passes ?
