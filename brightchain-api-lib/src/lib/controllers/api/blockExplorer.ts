...
> {
  ...
  private async handleListPools(
    ...
    try {
      ...
      if (!store) {
        return {
          statusCode: 200,
          response: { message: 'OK', pools: [] } as IApiMessageResponse &
      ...
      const pools =
        blockCount > 0 || !store.getBlockCount
  ...
  private async handleListBlocks(
    ...
    try {
      const request = req as {
        query?: { pool?: string; limit?: string; cursor?: string };
      };
      ...
      if (isPooledBlockStore(store)) {
        return this.listBlocksFromPooledStore(
          store,
          request.query?.pool,
          limit,
          cursor,
  ...
  private async listBlocksFromPooledStore(
    ...
    let poolTotal = 0;
    try {
      poolTotal = (await store.getPoolStats(pool)).blockCount;
    } catch {
      /* */
    }
  ...
  private async handleSearchBlocks(
    ...
    try {
      ...
      const matches: Array<{
        hash: string;
        pool: PoolId;
        metadata: IBlockMetadata | null;
      }> = [];

      if (isPooledBlockStore(store)) {
        const pools: PoolId[] = request.query?.pool
          ? [request.query.pool]
          : await store.listPools();
  ...
  private async handleGetBlockMetadata(
    ...
    try {
      ...
      return {
        ...
        response: {
          message: 'OK',
          block: { blockId, exists: true, metadata: metadata ?? undefined },
        } as IApiMessageResponse & Record<string, unknown>,
      };
    } catch (error) {
      return handleError(error);
  ...
  private async handleLocateBlock(
    ...
    try {
      ...
      const discoveryProtocol = this.application.services.get(
        ...
        | {
            discoverBlock?: (id: string) => Promise<{
              found: boolean;
