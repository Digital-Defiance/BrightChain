...
export function computeAclContentHash(acl: IAclDocument): Uint8Array {
  ...
  const adminsHex = acl.aclAdministrators
    .map((a: Uint8Array<ArrayBufferLike>) => Buffer.from(a).toString('hex'))
    .sort()
    .join(',');
...
export function serializeAclDocument(
  ...
  return {
    ...
    authorizedWriters: acl.authorizedWriters.map(
      (k: Uint8Array<ArrayBufferLike>) => Buffer.from(k).toString('hex'),
    ),
    aclAdministrators: acl.aclAdministrators.map(
      (k: Uint8Array<ArrayBufferLike>) => Buffer.from(k).toString('hex'),
    ),
    scope: acl.scope,
    version: acl.version,
