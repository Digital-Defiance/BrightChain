...
describeOrSkip(
  'Conformance: AzureBlobBlockStore + CloudHeadRegistry (REAL)',
  () => {
    ...
    beforeAll(async () => {
      ...
      console.log(
        `[Azure Conformance] Creating container "${containerName}" ...`,
      );
    ...
    afterAll(async () => {
      console.log(
        `[Azure Conformance] Deleting container "${containerName}" ...`,
      );
      try {
        await blobServiceClient.getContainerClient(containerName).delete();
        console.log(
          `[Azure Conformance] Container "${containerName}" deleted.`,
        );
      } catch (err) {
        console.warn(
    ...
    it('should persist user doc via CloudHeadRegistry across restart', async () => {
      ...
      const db1 = new BrightDb(memStore, {
        name: 'conform',
        headRegistry: registry1,
      });
      await db1.connect();
      ...
      const db2 = new BrightDb(memStore, {
        name: 'conform',
    ...
    it('should persist full registration → login chain across restart', async () => {
      ...
      const userDoc = await db2
        .collection('users')
        .findOne({ username: 'alice' } as never);
