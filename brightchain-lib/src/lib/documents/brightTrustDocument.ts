import { Member, PlatformID } from '@digitaldefiance/ecies-lib';
import { ConstituentBlockListBlock } from '../blocks/cbl';
import { createECIESService } from '../browserConfig';
import { BrightChainStrings } from '../enumerations/brightChainStrings';
import { BrightTrustErrorType } from '../enumerations/brightTrustErrorType';
import { BrightTrustError } from '../errors/brightTrustError';
import { TranslatableBrightChainError } from '../errors/translatableBrightChainError';
import { getBrightChainIdProvider } from '../init';
import { IBrightTrustDocument } from '../interfaces/document/brightTrustDocument';
import { BrightTrustDocumentSchema } from '../schemas/brightTrustDocument';
import { BlockService } from '../services/blockService';
import { MemberCblService } from '../services/member/memberCblService';
import { SealingService } from '../services/sealing.service';
import { SchemaDefinition } from '../sharedTypes';
import { Document } from './base/document';

export type { IBrightTrustDocument };

/**
 * A BrightTrustDocument has its data stored in the blockchain and is encrypted with a key that is shared among a trusted brightTrust
 * of members using Shamir's Secret Sharing and the keys are kept within the BrightTrust.
 */
export class BrightTrustDocument<
  TID extends PlatformID = Uint8Array,
  T extends IBrightTrustDocument<TID> = IBrightTrustDocument<TID>,
> extends Document<T> {
  constructor(data: Partial<T>) {
    super(data, BrightTrustDocumentSchema as SchemaDefinition<T>);
  }

  protected static memberService: MemberCblService | undefined = undefined;
  public static override initialize(
    blockService: BlockService,
    memberService: MemberCblService,
  ): void {
    super.initialize(blockService);
    BrightTrustDocument.memberService = memberService;
  }

  static createFromJson<T extends IBrightTrustDocument<Uint8Array>>(
    json: string,
  ): BrightTrustDocument<Uint8Array, T> {
    const doc = Document.fromJson<T>(
      json,
      new BrightTrustDocumentSchema<Uint8Array>().schema as SchemaDefinition<T>,
    );
    return new BrightTrustDocument(JSON.parse(doc.toJson()));
  }

  /**
   * Save the document, creating CBLs for members as needed
   */
  public override async save(): Promise<void> {
    if (!BrightTrustDocument.memberService) {
      throw new BrightTrustError(BrightTrustErrorType.Uninitialized);
    }
    await super.save();
    const creator = this.get('creator');
    if (!creator) {
      throw new TranslatableBrightChainError(
        BrightChainStrings.Error_BrightTrustDocument_CreatorMustBeSetBeforeSaving,
      );
    }

    // Create CBL for creator
    const cbl = await BrightTrustDocument.memberService.createMemberCbl(
      creator as Member<Uint8Array>,
      creator as Member<Uint8Array>,
    );

    // Store creator's CBL ID
    this.set('creatorId', cbl.idChecksum);

    // Clear creator reference since we'll store just the ID
    this.set('creator', undefined);
  }

  /**
   * Load the document, hydrating members from CBLs
   */
  public override async load(): Promise<void> {
    if (!BrightTrustDocument.memberService) {
      throw new BrightTrustError(BrightTrustErrorType.Uninitialized);
    }
    await super.load();
    const creatorId = this.get('creatorId');
    if (creatorId) {
      const store = BrightTrustDocument.memberService.getBlockStore();
      const block = await store.getData(creatorId);
      if (block instanceof ConstituentBlockListBlock) {
        const hydratedCreator =
          await BrightTrustDocument.memberService.hydrateMember(block);
        this.set('creator', hydratedCreator as Member<TID>);
      }
    }
  }

  /**
   * Delete the document
   */
  public override async delete(): Promise<void> {
    if (!BrightTrustDocument.memberService) {
      throw new BrightTrustError(BrightTrustErrorType.Uninitialized);
    }
    await super.delete();
  }

  /**
   * Encrypt document data using Shamir's Secret Sharing
   * @param data The data to encrypt
   * @param members The members to distribute shares to
   * @param sharesRequired Optional number of shares required to decrypt (defaults to all members)
   */
  async encrypt<T>(
    data: T,
    members: Member<TID>[],
    sharesRequired?: number,
  ): Promise<void> {
    const creator = this.get('creator');
    if (!creator) {
      throw new TranslatableBrightChainError(
        BrightChainStrings.Error_BrightTrustDocument_CreatorMustBeSetBeforeEncrypting,
      );
    }

    const sealingService = new SealingService<TID>(
      createECIESService<TID>(),
      getBrightChainIdProvider<TID>(),
    );
    const encryptedData = await sealingService.brightTrustSeal(
      creator,
      data,
      members,
      sharesRequired,
    );
    this.set('encryptedData', encryptedData);
    this.set(
      'memberIDs',
      members.map((m) => m.id),
    );
    this.set('sharesRequired', sharesRequired ?? members.length);
  }

  /**
   * Decrypt document data using the provided member keys
   * @param membersWithKeys Members with loaded private keys
   * @returns The decrypted data
   */
  async decrypt<T>(membersWithKeys: Member<TID>[]): Promise<T> {
    const encryptedData = this.get('encryptedData');
    if (!encryptedData) {
      throw new TranslatableBrightChainError(
        BrightChainStrings.Error_BrightTrustDocument_DocumentHasNoEncryptedData,
      );
    }
    const sealingService = new SealingService<TID>(
      createECIESService<TID>(),
      getBrightChainIdProvider<TID>(),
    );
    return await sealingService.brightTrustUnseal<T>(
      encryptedData,
      membersWithKeys,
    );
  }
}
