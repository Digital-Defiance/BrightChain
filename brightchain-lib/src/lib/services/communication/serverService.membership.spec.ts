...
import { ServerPermissionError, ServerService } from './serverService';
import { createMockStorageProvider } from './__tests__/mockStorageProvider';
...
describe('ServerService membership management', () => {
  ...
  describe('addMembers', () => {
    ...
    it('should skip duplicate members already in the server', async () => {
      ...
      expect(updated.memberIds.filter((id) => id === 'user-a')).toHaveLength(1);
    });
  ...
  describe('removeMember', () => {
    ...
    it('should remove the member from all server channels', async () => {
      ...
      expect(channelBefore?.members.some((m) => m.memberId === 'user-a')).toBe(
        true,
      );
      ...
      expect(channelAfter?.members.some((m) => m.memberId === 'user-a')).toBe(
        false,
      );
    });
    ...
    it('should throw ServerPermissionError when a non-owner/non-admin calls removeMember', async () => {
      ...
      await serverService.addMembers(server.id, owner, [
        'regular-user',
        'user-a',
      ]);
