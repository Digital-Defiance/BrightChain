/**
 * Property-Based Tests for BrightTrust Seal-Unseal Operations
 *
 * Feature: api-server-operations
 * Property 16: BrightTrust Seal-Unseal Round-Trip
 *
 * **Validates: Requirements 7.2**
 *
 * Property 16: For any document sealed with a set of member shares,
 * unsealing with sufficient valid shares via POST /api/brightTrust/documents/:documentId/unseal
...
describe('BrightTrust Seal-Unseal Property Tests', () => {
  describe('Property 16: BrightTrust Seal-Unseal Round-Trip', () => {
    /**
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (document, memberCount) => {
            // Feature: api-server-operations, Property 16: BrightTrust Seal-Unseal Round-Trip
            const brightTrustService = new BrightTrustService<GuidV4Buffer>();
            ...
            for (let i = 0; i < memberCount; i++) {
              ...
              await brightTrustService.addMember(data.member, {
                name: `TestMember${i}`,
            ...
            try {
              sealResult = await brightTrustService.sealDocument(
                agent,
                document,
                memberIds,
    ...
    it('Property 16c: Insufficient shares fails to unseal', async () => {
      await fc.assert(
        fc.asyncProperty(documentArb, async (document) => {
          ...
          try {
            await brightTrustService.unsealDocument(
              sealResult.documentId,
              insufficientMembers,
            );
          } catch {
