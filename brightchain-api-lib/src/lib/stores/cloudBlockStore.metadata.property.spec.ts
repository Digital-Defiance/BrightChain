  ...
  .record(
    {
      ...
      replicaNodeIds: fc.array(fc.string({ minLength: 8, maxLength: 16 }), {
        minLength: 0,
        maxLength: 5,
      }),
    },
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ([blockSize, data], updates) => {
          ...
          expect(metaAfter!.durabilityLevel).toBe(metaBefore!.durabilityLevel);
          expect(metaAfter!.replicationStatus).toBe(
            metaBefore!.replicationStatus,
          );
          expect(metaAfter!.parityBlockIds).toEqual(metaBefore!.parityBlockIds);
