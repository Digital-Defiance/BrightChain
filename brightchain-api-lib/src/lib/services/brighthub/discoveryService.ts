...
export class DiscoveryService implements IDiscoveryService {
  ...
  constructor(application: IApplicationWithCollections, friendsService?: IFriendsService) {
  ...
  async getSuggestions(
    ...
    // Boost candidates with mutual friends (friendship graph)
    if (this.friendsService) {
      for (const [candidateId, data] of candidateScores) {
        const mutualFriendsResult =
          await this.friendsService.getMutualFriends(userId, candidateId, {
            limit: 10000,
          });
        if (mutualFriendsResult.totalCount && mutualFriendsResult.totalCount > 0) {
          // Mutual friends score higher than follow-based mutual connections
          data.mutualCount += mutualFriendsResult.totalCount;
          data.reason = SuggestionReason.MutualFriends;
        }
      }

      // Also check all user profiles for mutual friends with non-candidates
      const allProfiles = await this.userProfilesCollection
        .find({} as Partial<UserProfileRecord>)
        .exec();
      for (const profile of allProfiles) {
        if (excluded.has(profile._id) || candidateScores.has(profile._id)) {
          continue;
        }
        const mutualFriendsResult =
          await this.friendsService.getMutualFriends(userId, profile._id, {
            limit: 10000,
          });
        if (mutualFriendsResult.totalCount && mutualFriendsResult.totalCount > 0) {
          candidateScores.set(profile._id, {
            mutualCount: mutualFriendsResult.totalCount,
...
export function createDiscoveryService(
  ...
  return new DiscoveryService(application, friendsService);
}
