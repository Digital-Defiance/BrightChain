...
describe('Pool isolation e2e', () => {
  ...
  describe('basic isolation', () => {
    it('two pools on the same store keep documents separate', async () => {
      ...
      await dbA
        .collection('users')
        .insertOne({ _id: 'a1', name: 'alice' } as never);
      await dbB
        .collection('users')
        .insertOne({ _id: 'b1', name: 'bob' } as never);
    ...
    it('pool blocks are not in the default pool', async () => {
      ...
      for await (const h of store.listBlocksInPool(DEFAULT_POOL))
        defaultBlocks.push(h);
      expect(defaultBlocks).toHaveLength(0);

      const customBlocks: string[] = [];
      for await (const h of store.listBlocksInPool('custom-pool'))
        customBlocks.push(h);
  ...
  describe('pool isolation across restart (disk registry)', () => {
    ...
    it('pool A data survives restart and pool B still cannot see it', async () => {
      ...
      const regA1 = new PersistentHeadRegistry({
        dataDir,
        fileName: 'heads-a.json',
      });
      const dbA1 = new BrightDb(store, {
        name: 'dbA',
        headRegistry: regA1,
        poolId: 'pool-a',
      });
      ...
      await dbA1
        .collection('secrets')
        .insertOne({ _id: 's1', data: 'pool-a-secret' } as never);

      // Phase 1: write to pool B
      const regB1 = new PersistentHeadRegistry({
        dataDir,
        fileName: 'heads-b.json',
      });
      const dbB1 = new BrightDb(store, {
        name: 'dbB',
        headRegistry: regB1,
        poolId: 'pool-b',
      });
      ...
      await dbB1
        .collection('secrets')
        .insertOne({ _id: 's2', data: 'pool-b-secret' } as never);

      // Phase 2: restart both
      const regA2 = new PersistentHeadRegistry({
        dataDir,
        fileName: 'heads-a.json',
      });
      await regA2.load();
      const dbA2 = new BrightDb(store, {
        name: 'dbA',
        headRegistry: regA2,
        poolId: 'pool-a',
      });
      ...
      const regB2 = new PersistentHeadRegistry({
        dataDir,
        fileName: 'heads-b.json',
      });
      await regB2.load();
      const dbB2 = new BrightDb(store, {
        name: 'dbB',
        headRegistry: regB2,
        poolId: 'pool-b',
      });
    ...
    it('updates in one pool do not affect the other after restart', async () => {
      ...
      await dbA
        .collection('shared')
        .insertOne({ _id: 'x', val: 'alpha' } as never);
      ...
      await dbB
        .collection('shared')
        .insertOne({ _id: 'x', val: 'beta' } as never);
      ...
      expect(((await dbA2.collection('shared').findById('x')) as any).val).toBe(
        'alpha-v2',
      );
      expect(((await dbB2.collection('shared').findById('x')) as any).val).toBe(
        'beta',
      );
    });
  });
  ...
  describe('StoreLock concurrent disk access', () => {
    ...
    it('concurrent writes from two BrightDb instances produce valid registry', async () => {
      ...
      // Per-key storage: each key gets its own file — no single-file corruption.
      // Two-level layout: files are under <headsDir>/<dbName>/<collName>.json
      const headsDir = join(dataDir, '__heads');
      const dbDirsA = await fs.readdir(headsDir);
      const jsonFilesA: string[] = [];
      for (const dbDir of dbDirsA) {
        const sub = await fs.readdir(join(headsDir, dbDir));
        for (const f of sub.filter(
          (f) => f.endsWith('.json') && !f.endsWith('.tmp.json'),
        )) {
          jsonFilesA.push(join(headsDir, dbDir, f));
        }
      }
      expect(jsonFilesA.length).toBeGreaterThanOrEqual(1);
      for (const filePath of jsonFilesA) {
    ...
    it('rapid concurrent writes do not corrupt the registry', async () => {
      ...
      // Each collection gets its own per-key file — verify all are valid JSON.
      // Two-level layout: files are under <headsDir>/<dbName>/<collName>.json
      const headsDir = join(dataDir, '__heads');
      const dbDirsB = await fs.readdir(headsDir);
      const jsonFilesB: string[] = [];
      for (const dbDir of dbDirsB) {
        const sub = await fs.readdir(join(headsDir, dbDir));
        for (const f of sub.filter(
          (f) => f.endsWith('.json') && !f.endsWith('.tmp.json'),
        )) {
          jsonFilesB.push(join(headsDir, dbDir, f));
        }
      }
      expect(jsonFilesB.length).toBe(15);
      for (const filePath of jsonFilesB) {
        const content = await fs.readFile(filePath, 'utf-8');
    ...
    it('committed transaction across two collections persists', async () => {
      ...
      try {
        ...
        await db1.withTransaction(async () => {
          await db1
            .collection('orders')
            .insertOne({ _id: 'o1', total: 100 } as never);
          await db1
            .collection('payments')
            .insertOne({ _id: 'p1', orderId: 'o1', amount: 100 } as never);
        });
