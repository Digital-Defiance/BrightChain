...
export interface IAttachmentMetadata {
  ...
  /**
   * Transient raw content bytes.
   * Populated during email parsing for pipeline use; NOT persisted to the block store.
   * Consumed by InboundProcessor to call storeAttachment().
   */
  content?: Uint8Array;

  /**
   * Digital Burnbag vault file ID for this attachment.
   * Set when the attachment has been uploaded to a Digital Burnbag private vault
   * as part of inbound email processing. Used to generate download links in the UI.
   */
  vaultFileId?: string;
}
