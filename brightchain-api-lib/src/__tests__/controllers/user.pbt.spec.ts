      ...
      fc.asyncProperty(
        ...
        async (username, email, password) => {
          ...
          const userProfile: IUserProfile<string> = {
            ...
            availableBalance: microjoulesToJoules(
              energyAccount.availableBalance,
            ),
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (username, email, password) => {
          ...
          expect(regResult.energyBalance).toBe(
            microjoulesToJoules(energyAccount.balance),
          );

          // A freshly registered user should have trial credits,
          // zero spent/reserved, and neutral reputation
          expect(energyAccount.earned).toBe(0n);
          expect(energyAccount.spent).toBe(0n);
          expect(energyAccount.reserved).toBe(0n);
