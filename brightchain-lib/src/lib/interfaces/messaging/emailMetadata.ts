...
export interface IEmailMetadata<TID extends PlatformID = string>
  ...
  /**
   * Plain-text body of the email, decoded from MIME parts.
   * Stored alongside parts for efficient display without re-parsing.
   *
   * @see Requirement 6.2
   */
  textBody?: string;
  ...
  /**
   * HTML body of the email, decoded from MIME parts.
   * Stored alongside parts for efficient display without re-parsing.
   *
   * @see Requirement 6.3
   */
   ...
   * When set, a private vault was created for this email and all
   * attachments were uploaded into it. UI can construct download links
   * using this ID together with each attachment's `vaultFileId`.
   */
  ...
  /**
   * Mail folder this email belongs to.
   * Standard values: 'inbox', 'sent', 'drafts', 'trash', 'spam'.
  ...
  /**
   * Spam score assigned by the anti-spam engine (e.g. SpamAssassin).
   * Typically 0–20+; higher values indicate more likely spam.
   *
   * @see Requirement 7.2
   */
  spamScore?: number;
  ...
  /**
   * Whether this email has been starred/flagged by the user.
   */
  isStarred?: boolean;
}
