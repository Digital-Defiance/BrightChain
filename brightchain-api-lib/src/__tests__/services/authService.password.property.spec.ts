...
// Use bcrypt cost factor 1 in tests to avoid ~300ms-per-hash delays.
// The crypto security of bcrypt is not under test here.
jest.mock('bcrypt', () => ({
  ...jest.requireActual('bcrypt'),
  hash: (data: string, _saltOrRounds: string | number) =>
    jest.requireActual<typeof import('bcrypt')>('bcrypt').hash(data, 1),
}));
...
/** Only uses characters allowed by PasswordRegex: letters, digits, and !@#$%^&*()_+-=[]{};':"|,.<>/? */
const passwordArb: fc.Arbitrary<string> = fc
  .tuple(
    ...
    // Padding from allowed chars only
