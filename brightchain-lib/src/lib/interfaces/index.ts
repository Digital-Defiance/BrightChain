...
// Server icon configuration (runtime values: DEFAULT_SERVER_ICON_CONFIG, validation helpers)
export * from './communication/serverIconConfig';

// Server icon upload request interface
export type * from './communication/serverIconRequest';
...
// Friends system interfaces (friendships, friend requests, service contract)
export type * from './friends';

// App subsystem plugin interfaces (plugin architecture for App.start() subsystems)
export type * from './appSubsystemPlugin';

// Temporary upload staging interfaces (staging, preview, commit lifecycle)
// Uses `export *` because staging contains runtime value (DEFAULT_STAGING_CONFIG)
export * from './staging';

// PoUW (Proof of Useful Work) rate limiting interfaces and enums
// Uses `export *` because pouw contains runtime enums (WorkUnitOperation, ClientIdentifierStrategy, RateLimiterFallback)
export * from './pouw';
