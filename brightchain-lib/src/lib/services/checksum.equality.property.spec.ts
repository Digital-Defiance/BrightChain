...
describe('Checksum Equality Property Tests', () => {
  ...
  describe('Property 2: Checksum Equality is Reflexive and Symmetric', () => {
    ...
    it('should maintain equality properties across multiple checksums', () => {
      ...
      const checksums = testData.map(data => checksumService.calculateChecksum(data));
      ...
      for (let i = 0; i < checksums.length; i++) {
        for (let j = 0; j < checksums.length; j++) {
          const aEqualsB = checksumService.compareChecksums(checksums[i], checksums[j]);
          const bEqualsA = checksumService.compareChecksums(checksums[j], checksums[i]);
          expect(aEqualsB).toBe(bEqualsA);
        }
      }
    });
    ...
    it('should handle edge case: large data checksums', () => {
      const large1 = checksumService.calculateChecksum(Buffer.alloc(1024 * 1024, 'A'));
      const large2 = checksumService.calculateChecksum(Buffer.alloc(1024 * 1024, 'A'));
