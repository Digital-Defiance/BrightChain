...
export interface IBlockStore {
   ...
   * - MemoryBlockStore  → InMemoryHeadRegistry
   * - DiskBlockStore    → DriverBackedHeadRegistry(FileHeadRegistryDriver)
   * - Cloud stores      → DriverBackedHeadRegistry(CloudHeadRegistryDriver)
   * Wrappers and adapters may delegate to their inner store or omit this.
   */
  ...
  /**
   * List all block IDs in the store with optional cursor-based pagination.
   * Used by the block explorer to enumerate blocks in non-pooled stores.
   *
   * @param options - Optional pagination: limit and cursor (last block ID from previous page)
   * @returns Async iterable of block ID hex strings
   */
  listBlockIds?(options?: { limit?: number; cursor?: string }): AsyncIterable<string>;

  /**
   * Get the total number of blocks in the store.
   * Used by the block explorer for pagination metadata.
   *
