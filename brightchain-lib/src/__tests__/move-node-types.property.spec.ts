    ...
    fc.assert(
      fc.property(fc.constantFrom(...apiResponseFiles), (filePath: string) => {
        ...
        const brightchainLibImportPattern =
          /import\s+(?:type\s+)?\{[^}]+\}\s+from\s+['"]@brightchain\/brightchain-lib['"]/;
...
describe('Feature: move-node-types-to-api-lib, Property 3: Every API response wrapper has a corresponding base data interface', () => {
  // Files excluded from this property check — they use base model interfaces
  // from other brightchain-lib directories rather than the responses/ directory,
  // or are Express-specific error types / generic constraint patterns.
  const EXCLUDED_FILES = new Set([
    'api-error.ts',
    'api-express-validation-error.ts',
    'api-status-code-response.ts',
    'api-friends-response.ts', // imports IBaseFriendship/IBaseFriendRequest from interfaces/friends/
  ]);
  ...
  function extractBrightchainLibImports(content: string): string[] {
    const importPattern =
      /import\s+(?:type\s+)?\{([^}]+)\}\s+from\s+['"]@brightchain\/brightchain-lib['"]/g;
