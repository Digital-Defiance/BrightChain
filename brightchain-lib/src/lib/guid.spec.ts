...
describe('guid', () => {
  ...
  it('should throw an error for invalid guid validation in constructor', () => {
    const validGuidFormat = '12345678-1234-1234-1234-1234567890ab';
  ...
  it('should create a GuidV4 from a Guid', () => {
    jest.spyOn(uuid, 'validate').mockReturnValue(true);
    const uuidStr = DEFAULT_UUID;
    const guidV4 = new GuidV4(uuidStr as FullHexGuid);
    expect(guidV4.asFullHexGuid).toEqual(uuidStr);
  });

  it('should test verifyGuid with validate=false', () => {
    const uuidStr: FullHexGuid = DEFAULT_UUID as FullHexGuid;
    jest.spyOn(GuidV4, 'validateUuid').mockReturnValue(false);
    const result = GuidV4.verifyGuid(GuidBrandType.FullHexGuid, uuidStr);
    expect(result).toBeFalsy();
    expect(GuidV4.validateUuid).toHaveBeenCalledWith(uuidStr);
  });
  ...
  it('should test isShortHexGuid with an invalid guid string', () => {
    jest.spyOn(uuid, 'validate').mockReturnValue(false);
  ...
  it('should test isShortHexGuid with a valid guid', () => {
    jest.spyOn(uuid, 'validate').mockReturnValue(true);
    const shortHexGuid = DEFAULT_UUID.replace(/-/g, '');
  ...
  it('should validate a valid FullHexGuid', () => {
    jest.spyOn(uuid, 'validate').mockReturnValue(true);
    expect(GuidV4.isFullHexGuid(DEFAULT_UUID)).toBeTruthy();
  });
  ...
  it('should correctly convert between FullHexGuid and ShortHexGuid', () => {
    const fullHexGuid = DEFAULT_UUID;
  ...
  describe('Static Methods', () => {
    ...
    it('should handle the uuid.validate branch in isRawGuid', () => {
      ...
      const result = GuidV4.isRawGuidBuffer(buffer);
      expect(result).toBeFalsy();
      expect(GuidV4.validateUuid).toHaveBeenCalled();
    });
    ...
    describe('isBigIntGuid', () => {
      ...
      it('should return false in catch block for negative numbers', () => {
        expect(GuidV4.isBigIntGuid(-1n)).toBeFalsy();
      });
    });
  ...
  it('should throw when we break the rules', () => {
    jest
      .spyOn(GuidV4, 'whichBrand')
      .mockReturnValue(GuidBrandType.RawGuidBuffer);
    jest.spyOn(GuidV4, 'verifyGuid').mockReturnValue(false);
