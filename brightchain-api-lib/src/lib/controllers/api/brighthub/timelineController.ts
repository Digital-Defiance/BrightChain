...
> {
  ...
  > {
    try {
      ...
      let profile: IBaseUserProfile<string>;

      if (id.startsWith('@')) {
        // Username lookup: /users/@jessica
        const username = id.slice(1);
        if (!username) return validationError('Username cannot be empty');
        profile = await this.getUserProfileService().getProfileByUsername(
          username,
          requesterId,
        );
      } else {
        // ID lookup: /users/bb3ceeca9ee74f6895387d873dec84fa
        ...
        profile = await this.getUserProfileService().getProfile(
          id,
          requesterId,
          usernameHint,
        );
