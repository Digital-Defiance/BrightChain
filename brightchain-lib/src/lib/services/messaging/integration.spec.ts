...
describe('Message Passing Integration Tests', () => {
  ...
  beforeEach(async () => {
    ...
    router = new MessageRouter(metadataStore, 'local-node', { routingTimeoutMs: 5000 });
  ...
  it('should complete end-to-end message flow: send → route → deliver → ack', async () => {
    ...
    // Send
    const { messageId } = await messageCBL.createMessage(content, creator, options);
    expect(messageId).toBeDefined();

    // Verify initial status is PENDING
    const initialMetadata = await metadataStore.get(messageId) as IMessageMetadata;
    expect(initialMetadata?.deliveryStatus.get('recipient1')).toBe(MessageDeliveryStatus.PENDING);
    expect(initialMetadata?.deliveryStatus.get('recipient2')).toBe(MessageDeliveryStatus.PENDING);

    // Route
    const routingResult = await router.routeMessage(messageId, options.recipients);
    ...
    expect(metadata?.deliveryStatus.get('recipient1')).toBe(MessageDeliveryStatus.IN_TRANSIT);
    expect(metadata?.deliveryStatus.get('recipient2')).toBe(MessageDeliveryStatus.IN_TRANSIT);

    // Acknowledge
    await metadataStore.recordAcknowledgment(messageId, 'recipient1', new Date());
    await metadataStore.recordAcknowledgment(messageId, 'recipient2', new Date());

    // Verify acknowledgments
    const updatedMetadata = await metadataStore.get(messageId) as IMessageMetadata;
  ...
  it('should retrieve message content correctly', async () => {
    ...
    const { messageId } = await messageCBL.createMessage(originalContent, creator, options);
  ...
  it('should handle large messages across multiple blocks', async () => {
    ...
    const { messageId, contentBlockIds } = await messageCBL.createMessage(largeContent, creator, options);
