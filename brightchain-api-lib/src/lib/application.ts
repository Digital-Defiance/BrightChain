  ...
  public override async start(dbUri?: string): Promise<void> {
    ...
    if (this.apiRouter) {
      ...
      const ensureMemberPublicKey = async (
        ...
        // Uses idProvider.deserialize() which converts UUID strings to bytes.
        const memberIdBytes = serviceProvider.idProvider.deserialize(memberId);
        const hexKey =
          await memberStore.getMemberPublicKeyHex(memberIdBytes);
      ...
      // and a no-op gossip stub. When a real MessageCBLService / GossipService
      // is wired later (via setMessagePassingService / setPoolDiscoveryService),
      // the adapter can be replaced with a fully-connected instance.
      ...
      const chatMessageCBL = {
        createMessage: async (
          ...
          return {
            messageId,
            contentBlockIds: [] as string[],
            magnetUrl: `magnet:?xt=urn:brightchain:${messageId}`,
          };
        },
        getMessageMetadata: async () => null,
        getMessageContent: async () => null,
      ...
      this.services.register('blockContentStore', () => blockContentStoreAdapter);
      ...
      this.services.register('serverService', () => serverService);
      ...
      this.apiRouter.setServerService(serverService);
