...
> {
  ...
  private async handleGetUnreadCounts(
    ...
    try {
      ...
      const [hub, mail, chat, pass] = await Promise.all([
      ...
      const total = hub + mail + chat + pass;

      return {
        statusCode: 200,
        response: {
          message: 'OK',
          data: { total, hub, mail, chat, pass },
        },
      };
    } catch (error) {
