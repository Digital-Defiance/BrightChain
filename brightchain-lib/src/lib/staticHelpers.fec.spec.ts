...
describe('staticHelpers.fec', () => {
  it('should encode and decode with no errors present', async () => {
    ...
    const encoded = await StaticHelpersFec.fecEncode(
      input,
      blockSize,
      dataBlocks,
      parityBlocks,
    ...
    const decoded = await StaticHelpersFec.fecDecode(
      encoded,
      blockSize,
      dataBlocks,
      parityBlocks,
      buildAvailableBlocks(dataBlocks, parityBlocks),
    );
  ...
  testBlockSizes.forEach((blockSize: BlockSize) => {
    parityBlockCounts.forEach((parityBlockCount: number) => {
      it(`should produce parity blocks that can be used to recover from errors with size ${blockSize} and ${parityBlockCount} parity blocks`, async () => {
        ...
        const inputBlock = new BaseBlock(
          blockSize,
          input,
          BlockDataType.RawData,
          blockSize as number,
        );
        const parityBlocks = await StaticHelpersFec.createParityBlocks(
          inputBlock,
          parityBlockCount,
        );
        ...
        const chunksRequired = Math.ceil(
          (blockSize as number) / StaticHelpersFec.MaximumShardSize,
        );
        ...
        for (let i = 0; i < chunksRequired; i++) {
          for (let j = 0; j < damagePerChunk; j++) {
            ...
            const randomOffset =
              Math.floor(Math.random() * chunkSize) + i * chunkSize;
            inputBlock.data[randomOffset] = Math.floor(Math.random() * 255);
          }
        }
        const damagedBlock = new BaseBlock(
          blockSize,
          inputBlock.data,
          BlockDataType.RawData,
          blockSize as number,
        );
        const recoveredBlock = await StaticHelpersFec.recoverDataBlocks(
          damagedBlock,
          parityBlocks,
        );
