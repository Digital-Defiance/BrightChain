...
> extends BrightDbUserController<TID> {
  ...
  override async directChallenge(
    ...
    try {
      ...
      // Check if the user has TOTP enabled — if so, return a short-lived
      // pending token instead of signing a full JWT. The full JWT is only
      // created after TOTP verification succeeds (POST /totp/verify).
      try {
        ...
        console.error('[directChallenge] TOTP check failed:', totpCheckErr);
      }

      // No TOTP — sign the full JWT with RBAC roles.
