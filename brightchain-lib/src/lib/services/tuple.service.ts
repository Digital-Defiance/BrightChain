...
import { BlockHandleType } from '../blocks/handle';
...
export class TupleService {
  ...
  public async xorSourceToPrimeWhitened(
    sourceBlock: BaseBlock | BlockHandleType<BaseBlock>,
  ...
  public async xorPrimeWhitenedEncryptedToCbl(
    ...
    const encryptedBlock = await this.blockService.encrypt(
      BlockType.EncryptedOwnedDataBlock, // Assuming this is the correct target type
      ownedBlock,
      creator, // Assuming creator is the recipient here
    );
    ...
    // Use injected blockService instance
    const decryptedBlock = await this.blockService.decrypt(
      creator,
      encryptedBlock,
      BlockType.EphemeralOwnedDataBlock, // Assuming this is the correct target type after decryption
    );
    // Pass injected services to constructor
    return new ConstituentBlockListBlock(
      decryptedBlock.data,
  ...
  public async dataStreamToPlaintextTuplesAndCBL(
    ...
    try {
      ...
      // Create CBL, passing injected services
  ...
  public async dataStreamToEncryptedTuplesAndCBL(
    ...
    try {
      ...
      // Create and encrypt CBL, passing injected services
      const cbl = new ConstituentBlockListBlock(
        data,
        creator,
        this.cblService,
      ...
      // Use injected blockService instance
      const encryptedCbl = await this.blockService.encrypt(
        BlockType.EncryptedOwnedDataBlock, // Assuming this is the correct target type
        ownedBlock,
        creator, // Assuming creator is the recipient here
      );
