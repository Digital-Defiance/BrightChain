...
describe('BrightChainBackupCodeService Property-Based Tests', () => {
  // Spy on BackupCode.getBackupKeyV1 to return a deterministic 32-byte buffer instantly.
  // The Argon2id KDF security is not under test here; this avoids ~500ms per-code delays.
  let getBackupKeyV1Spy: jest.SpyInstance;
  beforeAll(() => {
    getBackupKeyV1Spy = jest
      .spyOn(BackupCode, 'getBackupKeyV1')
      .mockImplementation(() => Promise.resolve(Buffer.alloc(32, 0xaa)));
  });

  afterAll(() => {
    getBackupKeyV1Spy.mockRestore();
  });

  afterEach(() => {
    // Restore per-test spies (e.g. getMember mock) but keep the BackupCode spy active.
    // Re-apply the BackupCode spy in case restoreAllMocks was called by fast-check.
    getBackupKeyV1Spy = jest
      .spyOn(BackupCode, 'getBackupKeyV1')
      .mockImplementation(() => Promise.resolve(Buffer.alloc(32, 0xaa)));
  });
