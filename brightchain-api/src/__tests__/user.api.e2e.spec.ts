...
async function startApp(): Promise<TestServer> {
  ...
  setTestEnv(port);
  // BlockStoreService always builds a DiskBlockStore even with DEV_DATABASE,
  // so give it a writable tmp path instead of the default `/data/blockstore`.
  const blockstoreDir = mkdtempSync(join(tmpdir(), 'bc-user-e2e-'));
  process.env['BRIGHTCHAIN_BLOCKSTORE_PATH'] = blockstoreDir;
  const env = new Environment(undefined, false, false);
  const app = new App<GuidV4Buffer>(env as Environment<GuidV4Buffer>);
  await app.start();

  return { app, baseUrl: `http://127.0.0.1:${port}`, blockstoreDir };
}
...
describe('UserController E2E — RBAC-seeded direct-challenge', () => {
  ...
  beforeAll(async () => {
    ...
    const blockstoreDir = mkdtempSync(join(tmpdir(), 'bc-user-e2e-rbac-'));
    process.env['BRIGHTCHAIN_BLOCKSTORE_PATH'] = blockstoreDir;

    const env = new Environment(undefined, false, false);
    const app = new App<GuidV4Buffer>(env as Environment<GuidV4Buffer>);
    await app.start();
    server = { app, baseUrl: `http://127.0.0.1:${port}`, blockstoreDir };
  });

  afterAll(async () => {
    await server.app.stop();
    try {
      rmSync(server.blockstoreDir, { recursive: true, force: true });
    } catch {
      // best-effort cleanup
    }
...
describe('UserController E2E — disk-backed store', () => {
  ...
  beforeAll(async () => {
    ...
    process.env['BRIGHTCHAIN_BLOCKSTORE_TYPE'] = 'disk';
    process.env['DEBUG'] = 'false';
    process.env['PORT'] = String(port);
    process.env['HOST'] = '127.0.0.1';
    process.env['UPNP_ENABLED'] = 'false';
