---
title: "Storage Pools API Reference"
...
> For architectural context see [Storage Pools Architecture](./storage-pools-architecture).
...
`PooledStoreAdapter` wraps an `IPooledBlockStore` and exposes the standard `IBlockStore` interface scoped to a single pool. This lets existing code (e.g. `Collection`, `BrightDb`) work unchanged while all operations route through the pool.
...
// Pass it to BrightDb — the database sees a normal IBlockStore
const db = new BrightDb({ store: poolStore });
