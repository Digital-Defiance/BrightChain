...
export const BrightChainSoupDemo: React.FC = () => {
  ...
  const _uint8ArrayToText = (data: Uint8Array): string => {
    return new TextDecoder().decode(data);
  };

  const _xorArrays = (a: Uint8Array, b: Uint8Array): Uint8Array => {
  ...
  const _generateRandomData = (length: number): Uint8Array => {
    const data = new Uint8Array(length);
    crypto.getRandomValues(data);
    return data;
  };

  const createSoupCan = async (
    label: string,
    data: Uint8Array,
    type: 'original' | 'random' | 'result',
  ): Promise<SoupCan> => {
  ...
  const brightenMessage = useCallback(async () => {
    ...
    try {
      ...
      const originalCan = await createSoupCan(
        'Original Message',
        messageData,
        'original',
      );
  ...
  const reconstructMessage = useCallback(async () => {
    ...
    try {
      ...
      setReconstructed(
        'CBL created and stored! (Full reconstruction coming soon)',
      );
    } catch (error) {
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <p>
          BrightChain stores your data like soup cans in a market. Your original
          message gets mixed with random "soup flavors" using XOR operations,
          making it look completely random. Only with the right recipe can you
          reconstruct your original message!
        </p>
      </div>
            ...
            {soupCans.map((can) => (
              <div
                ...
                style={{
                  ...
                  padding: '10px',
                  minWidth: '150px',
                  backgroundColor: can.isOriginal
                    ? '#ffe6e6'
                    : can.isRandom
                      ? '#e6e6ff'
                      : '#e6ffe6',
      ...
      {reconstructed && (
        <div style={{ marginBottom: '20px' }}>
          ...
          <div
            style={{
              backgroundColor:
                reconstructed === message ? '#d4edda' : '#f8d7da',
              padding: '10px',
              borderRadius: '5px',
      ...
      >
        ...
        <ol>
          <li>
            <strong>Original Message:</strong> Your secret data (red can)
          </li>
          <li>
            <strong>Random Cans:</strong> Generated random data (blue cans)
          </li>
          <li>
            <strong>XOR Operation:</strong> Message ⊕ Random A ⊕ Random B ⊕
            Random C = Result Z
          </li>
          <li>
            <strong>Storage:</strong> Only random cans and result are stored
            publicly
          </li>
          <li>
            <strong>Reconstruction:</strong> Random A ⊕ Random B ⊕ Random C ⊕
            Result Z = Original Message
          </li>
        </ol>
        <p>
          <strong>Security:</strong> Without the complete recipe, the data
