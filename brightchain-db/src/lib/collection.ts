 ...
 */
export function calculateBlockId(data: Buffer | Uint8Array): string {
 ...
 * structurally, avoiding the "separate declarations of a private property"
 * error that occurs when TypeScript compares classes with private fields.
 */
export interface ICollectionHeadRegistry {
  ...
  /**
   * Subscribe to external head changes (e.g. inbound gossip merges).
   * Optional: only implemented by registries that support gossip.
...
export class Collection<T extends BsonDocument = BsonDocument> {
   ...
   */
  private headChangeSub?: () => void;
  /**
   * Re-entrancy guard: prevents mergeFromGossipHead() from re-triggering
   * itself via the setHead() call inside persistMeta().
   */
  private isMergingGossip = false;
  /**
   * Tail of the chain of in-flight gossip merges. Read paths await this so
   * that a `find()` issued immediately after a gossip-driven head update
   * observes the merged state, even when the listener (`onHeadChange`) only
   * gets a fire-and-forget Promise. Cleared once drained.
   */
  private pendingGossipMerge?: Promise<void>;
  ...
  constructor(
    ...
    if (this.headRegistry.onHeadChange) {
      this.headChangeSub = this.headRegistry.onHeadChange(
        this.dbName,
        this.name,
        (newBlockId) => {
          // Chain the merge onto any in-flight one so reads can await the
          // tail and see all gossip applied so far.
          const prior = this.pendingGossipMerge ?? Promise.resolve();
          const next = prior
            .catch(() => undefined)
            .then(() => this.mergeFromGossipHead(newBlockId));
          this.pendingGossipMerge = next;
          // Self-clear once this segment finishes, but only if we're still
          // the tail (a later listener may have chained further).
          void next.finally(() => {
            if (this.pendingGossipMerge === next) {
              this.pendingGossipMerge = undefined;
            }
          });
        },
      );
  ...
  private async loadFromStore(): Promise<void> {
    ...
    if (headBlockId) {
      try {
        ...
        // Restore per-document timestamps (used for per-key LWW merge)
        if (meta.docTimestamps) {
          for (const [id, ts] of Object.entries(meta.docTimestamps)) {
            this.docTimestamps.set(id, ts);
          }
        }

        // Restore per-document tombstones (used for LWW delete propagation)
        if (meta.docTombstones) {
          for (const [id, ts] of Object.entries(meta.docTombstones)) {
            this.docTombstones.set(id, ts);
          }
        }
  ...
  /**
   * Merge a gossip-delivered remote head into the current in-memory state.
   *
   * Per-document LWW strategy:
   * - For each _id in the remote mapping: adopt the remote blockId only when
   *   the remote write-timestamp is strictly newer than the local timestamp
   *   AND strictly newer than any local tombstone for the same _id.
   * - For each _id in the remote tombstone map: apply the delete only when the
   *   remote tombstone-timestamp is strictly newer than the local write and
   *   the current local tombstone.
   *
   * After merging, the merged state is persisted so the next local setHead()
   * advances the timestamp and subsequent gossip announces the merged view.
   * persistMeta() fires setHead(), which does NOT trigger onHeadChange()
   * (because onHeadChange is only wired to mergeHeadUpdate), so there is no
   * re-entrancy risk.
   *
   * @param newBlockId - Block ID announced by the remote node's gossip
   */
  private async mergeFromGossipHead(newBlockId: string): Promise<void> {
    // Re-entrancy guard: if we're already processing a gossip merge (which
    // calls persistMeta → setHead), ignore the recursive call.
    if (this.isMergingGossip) return;

    // If this collection has never been loaded, don't try to merge — the next
    // ensureLoaded() will read the latest head from the registry directly.
    if (!this.loaded) return;

    this.isMergingGossip = true;
    try {
      const checksum = Checksum.fromHex(newBlockId);
      const block = await this.store.getData(checksum);
      const remoteMeta = JSON.parse(
        Buffer.from(block.data).toString('utf8'),
      ) as CollectionMeta;

      let changed = false;

      // ── Apply remote inserts / updates ──────────────────────────────────
      if (remoteMeta.mappings) {
        const remoteTimestamps = remoteMeta.docTimestamps ?? {};
        for (const [id, remoteBlockId] of Object.entries(remoteMeta.mappings)) {
          const remoteTs = remoteTimestamps[id] ?? 0;
          const localWriteTs = this.docTimestamps.get(id) ?? 0;
          const localTombstoneTs = this.docTombstones.get(id) ?? 0;

          if (remoteTs > localWriteTs && remoteTs > localTombstoneTs) {
            // Remote write is the latest state for this document
            const oldDoc = this.docCache.get(id);
            if (oldDoc) this.indexManager.removeDocument(oldDoc);

            this.docIndex.set(id, remoteBlockId);
            this.docTimestamps.set(id, remoteTs);
            this.docTombstones.delete(id);
            this.docCache.delete(id); // evict cache; will be re-fetched lazily
            changed = true;
          }
        }
      }

      // ── Apply remote deletes (tombstones) ────────────────────────────────
      if (remoteMeta.docTombstones) {
        for (const [id, remoteTombstoneTs] of Object.entries(
          remoteMeta.docTombstones,
        )) {
          const localWriteTs = this.docTimestamps.get(id) ?? 0;
          const localTombstoneTs = this.docTombstones.get(id) ?? 0;

          if (
            remoteTombstoneTs > localWriteTs &&
            remoteTombstoneTs > localTombstoneTs
          ) {
            // Remote delete wins
            ...
            this.docTimestamps.delete(id);
            this.docTombstones.set(id, remoteTombstoneTs);
            changed = true;
          }
        }
      }
      ...
      if (changed) {
        // Persist merged state so that future gossip announces the union.
        // This does NOT re-trigger onHeadChange because onHeadChange is only
        // fired by mergeHeadUpdate(), not by setHead().
        await this.persistMeta();
      }
    } catch {
      // Gossip merge errors must not crash the collection; they are best-effort.
    } finally {
      this.isMergingGossip = false;
    }
  }
  ...
  private async persistMeta(): Promise<void> {
    const meta: CollectionMeta = {
      ...
      docTimestamps: Object.fromEntries(this.docTimestamps.entries()),
      docTombstones: Object.fromEntries(this.docTombstones.entries()),
...
export class HeadRegistry {
  ...
  async applyDeferredUpdates(_blockId: string): Promise<number> {
    return 0;
  }
  ...
  async mergeSnapshot(
    _snapshot: Iterable<readonly [string, HeadRecord]>,
