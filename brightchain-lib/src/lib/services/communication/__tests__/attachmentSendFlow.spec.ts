...
describe('ChannelService sendMessage with attachments', () => {
  ...
  it('throws AttachmentTooLargeError for oversized file', async () => {
    ...
    const oversized = [
      makeAttachment('big.bin', DEFAULT_ATTACHMENT_CONFIG.maxFileSizeBytes + 1),
    ];
    await expect(
  ...
  it('throws TooManyAttachmentsError when exceeding count limit', async () => {
    ...
    const tooMany = Array.from(
      { length: DEFAULT_ATTACHMENT_CONFIG.maxAttachmentsPerMessage + 1 },
      (_, i) => makeAttachment(`file${i}.txt`, 10),
    );
    await expect(
