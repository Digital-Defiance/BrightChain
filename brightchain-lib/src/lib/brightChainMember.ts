...
export class BrightChainMember implements IBrightChainMemberOperational {
  ...
  public deriveVotingKeyPair(): void {
    ...
    const { privateKey: votingPrivateKey } =
      VotingService.deriveVotingKeysFromECDH(this._privateKey, this._publicKey);
  ...
  public toJson(): string {
    const storage: IMemberStorageData = {
      ...
      privateKey: this._privateKey?.toString('base64'),
      votingPublicKey: VotingService.votingPublicKeyToBuffer(
        this._votingPublicKey,
      ).toString('base64'),
      votingPrivateKey: this._votingPrivateKey
        ? VotingService.votingPrivateKeyToBuffer(
