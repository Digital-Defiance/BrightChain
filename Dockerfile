...
COPY brightcal-api-lib/package.json brightcal-api-lib/
COPY brightcal-lib/package.json brightcal-lib/
COPY brightcal-react-components/package.json brightcal-react-components/
COPY brightchart-lib/package.json brightchart-lib/
COPY brightchart-react-components/package.json brightchart-react-components/
...
# brightcal-api-lib and brightcal-lib are implicit dependencies of brightchain-api,
# so their source files are transpiled into the API dist automatically.
...
# Install Postfix, OpenDKIM, SpamAssassin, and minimal utilities
RUN apt-get update && \
...
# SpamAssassin: socket directory (inside Postfix queue so chroot can reach it),
# home dir for Bayesian DB, and trusted networks config
RUN mkdir -p /var/spool/postfix/spamass && \
    chown spamass-milter:spamass-milter /var/spool/postfix/spamass && \
    chmod 750 /var/spool/postfix/spamass && \
    mkdir -p /var/lib/spamassassin && \
    chown debian-spamd:debian-spamd /var/lib/spamassassin && \
    sa-update --nogpg || true
...
COPY --from=builder /build/dist/brightchain-api ./dist/api

# Copy built React frontend (static files)
COPY --from=builder /build/dist/brightchain-react ./dist/react
