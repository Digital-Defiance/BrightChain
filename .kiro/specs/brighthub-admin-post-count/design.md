...
The admin dashboard and admin hub post list both report zero posts due to two independent bugs: (1) `AdminHubController` queries the wrong BrightDB collection name (`'posts'` instead of `'brighthub_posts'`), and (2) `PostService` is missing a `getStats()` method that the dashboard's `gatherHubStats()` expects. The fix corrects the collection name in the controller and adds the missing stats method to `PostService`.
