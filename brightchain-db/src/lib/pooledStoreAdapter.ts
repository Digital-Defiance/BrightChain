...
export class PooledStoreAdapter implements IBlockStore {
  ...
  public createHeadRegistry(): IHeadRegistry {
    if (!this.inner.createHeadRegistry) {
      throw new Error('Inner store does not support createHeadRegistry');
    }
    return this.inner.createHeadRegistry();
  }

  // === Block Enumeration (scoped to this pool) ===

  public async *listBlockIds(options?: { limit?: number; cursor?: string }): AsyncIterable<string> {
    const opts = options ? { limit: options.limit, cursor: options.cursor } : undefined;
    for await (const hash of this.inner.listBlocksInPool(this.poolId, opts)) {
      yield hash;
    }
  }
  ...
  public async getBlockCount(): Promise<number> {
    try {
      const stats = await this.inner.getPoolStats(this.poolId);
