...
import {
  BrightChartStrings,
  createBrightChartComponentPackage,
...
const AdminJoulePanel = lazy(() =>
  import('./components/AdminJoulePanel').then((m) => ({
    default: m.default,
  })),
);
...
// Registrations are unconditional because menu configs reference string keys
// during render even when the feature is ultimately filtered out.
...
registerI18nComponentPackage(createBrightCalComponentPackage());
...
const getTotpAvailable = (): boolean => {
  if (typeof window !== 'undefined') {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const appConfig = (window as any).APP_CONFIG;
    if (appConfig) {
      return !!appConfig.totpAvailable;
    }
  }
  return false;
};
...
const TOTP_AVAILABLE: boolean = getTotpAvailable();
...
const InnerApp: FC = () => {
  ...
  const [chatServers, setChatServers] = useState<IServer<string>[]>([]);
  const [unreadNotifCount, setUnreadNotifCount] = useState(0);

  // Local index counter — reset on every render so priorities stay stable
  let indexPriority = INITIAL_INDEX_PRIORITY;
  ...
  // Fetch BrightChat servers for the menu (guarded by authentication)
  const fetchChatServers = useCallback(() => {
    if (!userData?.id) return;
    chatApi
      .listServers()
      .then((result: { items?: unknown[] } | null | undefined) => {
        if (result && Array.isArray(result.items)) {
          setChatServers(result.items as IServer<string>[]);
        }
      })
      .catch(() => {
        // Silently fail — menu renders with static items only
      });
  }, [chatApi, userData?.id]);
  ...
  // Re-fetch chat servers when a new server is created inside BrightChat
  useEffect(() => {
    const handler = () => fetchChatServers();
    window.addEventListener('brightchat:servers-changed', handler);
    return () =>
      window.removeEventListener('brightchat:servers-changed', handler);
  }, [fetchChatServers]);
  ...
  const { options: brightChatMenuItems } = useBrightChatMenuItems(
    chatMenu,
    chatServers,
    nextIndex + 5,
  ...
  const brightChartMenuConfig: IMenuConfig = {
    ...
    options: [
      ...
      },
      {
        ...
        label: t(BrightChartStrings.Nav_Clinician),
        includeOnMenus: [chartMenu, MenuTypes.SideMenu],
        index: indexPriority + 5,
      ...
      },
      {
        ...
        label: t(BrightChartStrings.Nav_PatientPortal),
        includeOnMenus: [chartMenu, MenuTypes.SideMenu],
        index: indexPriority + 10,
        icon: <FavoriteIcon />,
      ...
      },
      {
        ...
        label: t(BrightChartStrings.Nav_Schedule),
        includeOnMenus: [chartMenu, MenuTypes.SideMenu],
        index: indexPriority + 15,
        icon: <CalendarMonthIcon />,
      ...
      },
      {
        ...
        label: t(BrightChartStrings.Nav_Billing),
        includeOnMenus: [chartMenu, MenuTypes.SideMenu],
        index: indexPriority + 20,
        icon: <BarChartIcon />,
