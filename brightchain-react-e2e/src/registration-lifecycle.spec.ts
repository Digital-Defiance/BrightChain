...
import { test, expect, generateCredentials } from './fixtures';
...
test.describe('Authenticated Dashboard Access', () => {
  test(
    'registered user can access dashboard with authenticated content',
    async ({ authenticatedPage, authResult }) => {
...
test.describe('Password Change Lifecycle', () => {
  test(
    'change password succeeds and new password works for login',
    async ({ authenticatedPage, authResult }) => {
