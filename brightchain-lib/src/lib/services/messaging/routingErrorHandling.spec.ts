...
describe('MessageRouter Error Handling', () => {
  ...
  beforeEach(() => {
    ...
    router = new MessageRouter(metadataStore, 'local-node', { routingTimeoutMs: 100 });
  });

  it('should handle routing timeout', async () => {
    jest.spyOn(metadataStore, 'updateDeliveryStatus').mockImplementation(
    ...
    try {
      ...
      expect((error as MessageError).errorType).toBe(MessageErrorType.DELIVERY_FAILED);
    }
  });

  it('should throw when all recipients fail', async () => {
    jest.spyOn(metadataStore, 'updateDeliveryStatus').mockRejectedValue(new Error('Network error'));

    await expect(router.routeMessage('msg1', ['recipient1', 'recipient2'])).rejects.toThrow(MessageError);
    await expect(router.routeMessage('msg1', ['recipient1', 'recipient2'])).rejects.toMatchObject({
  ...
  it('should mark failed recipients as FAILED', async () => {
    ...
    try {
      ...
      expect(error).toBeInstanceOf(MessageError);
      expect(updateSpy).toHaveBeenCalledWith('msg1', 'recipient1', MessageDeliveryStatus.FAILED);
    }
  });
  ...
  it('should succeed with partial failures', async () => {
    ...
    const result = await router.routeMessage('msg1', ['recipient1', 'recipient2']);
