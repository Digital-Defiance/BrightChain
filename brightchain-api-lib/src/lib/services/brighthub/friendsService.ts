...
export class FriendsService implements IFriendsService {
  ...
  constructor(
    ...
    this.friendshipsCollection = application.getModel<FriendshipRecord>(
      'brightchain_friendships',
    );
    this.friendRequestsCollection = application.getModel<FriendRequestRecord>(
  ...
  async getMutualFriends(
    ...
    const mutualFriendships = otherFriendsResult.items.filter((f) => {
      const otherId = f.memberIdA === otherUserId ? f.memberIdB : f.memberIdA;
  ...
  async onUserBlocked(blockerId: string, blockedId: string): Promise<void> {
