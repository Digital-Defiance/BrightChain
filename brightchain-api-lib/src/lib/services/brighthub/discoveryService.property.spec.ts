      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (userId, candidateId, mutualFriendId) => {
            ...
            const userRequests =
              await friendsService.getReceivedFriendRequests(mutualFriendId);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (userId, candidateId, mutualFriendIds) => {
            const unique = [
              ...new Set([userId, candidateId, ...mutualFriendIds]),
            ];
            ...
            for (const mfId of actualMutualFriends) {
              ...
              const reqs1 =
                await friendsService.getReceivedFriendRequests(mfId);
