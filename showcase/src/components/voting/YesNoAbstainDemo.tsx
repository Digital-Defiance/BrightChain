      ...
      <div className="voting-demo">
        <div className="election-intro">
          ...
          <div className="intro-story">
            <p>🌍 <strong>The Resolution:</strong> "Should the UN impose sanctions on Country X for human rights violations?"</p>
            <p>🤷 <strong>Yes/No/Abstain:</strong> Sometimes you're not ready to decide. Abstentions don't count toward the total but are recorded.</p>
            <div className="intro-stakes">
              <p>✅ <strong>YES:</strong> Impose sanctions immediately</p>
              <p>❌ <strong>NO:</strong> Reject the resolution</p>
              <p>🤷 <strong>ABSTAIN:</strong> Neutral - don't want to take a side</p>
            </div>
            <p className="intro-challenge">🏛️ Used in UN votes, parliamentary procedures, and board meetings worldwide.</p>
          </div>
  ...
  return (
    <div className="voting-demo">
      <div className="demo-header">
        ...
        <p className="election-tagline">🌍 Three choices: Support, Oppose, or Stay Neutral</p>
      </div>
      ...
      {!results ? (
        <>
          ...
          <div className="voters-section">
            <h4>Security Council Members ({votes.size}/{voters.length} voted)</h4>
                ...
                <div key={voter} className="voter-card">
                  ...
                  {votes.has(voter) ? (
                    <div className="vote-cast">
                      ✓ {votes.get(voter) === 0 ? '👍 YES' : votes.get(voter) === 1 ? '👎 NO' : '🤷 ABSTAIN'}
                    </div>
                  ) : (
                    <div className="abstain-buttons">
                      <button onClick={() => castVote(voter, 0)} className="vote-btn yes-btn">👍 YES</button>
                      <button onClick={() => castVote(voter, 1)} className="vote-btn no-btn">👎 NO</button>
                      <button onClick={() => castVote(voter, 2)} className="vote-btn abstain-btn">🤷 ABSTAIN</button>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="results-section">
          ...
          <div className="tally-visualization">
            <h5>📊 Vote Counting</h5>
            <p className="tally-explain">Abstentions are recorded but don't count toward the decision. Winner needs majority of YES/NO votes:</p>
          </div>

          <div className="abstain-results">
            <div className={`abstain-result ${results.winner === 0 ? 'winner' : ''}`}>
              <span className="abstain-emoji">👍</span>
              <h3>YES</h3>
              <p className="abstain-count">{Number(results.tallies[0])} votes</p>
              <p className="abstain-percent">{((Number(results.tallies[0]) / (Number(results.tallies[0]) + Number(results.tallies[1]))) * 100).toFixed(1)}%</p>
            </div>
            <div className={`abstain-result ${results.winner === 1 ? 'winner' : ''}`}>
              <span className="abstain-emoji">👎</span>
              <h3>NO</h3>
              <p className="abstain-count">{Number(results.tallies[1])} votes</p>
              <p className="abstain-percent">{((Number(results.tallies[1]) / (Number(results.tallies[0]) + Number(results.tallies[1]))) * 100).toFixed(1)}%</p>
            </div>
            <div className="abstain-result neutral">
              <span className="abstain-emoji">🤷</span>
              <h3>ABSTAIN</h3>
              <p className="abstain-count">{Number(results.tallies[2])} votes</p>
              <p className="abstain-note">Not counted in decision</p>
            </div>
          </div>

          <div className="referendum-outcome">
            <h3>{results.winner === 0 ? '✅ Resolution PASSES!' : '❌ Resolution FAILS!'}</h3>
            <p>Deciding votes: {Number(results.tallies[0]) + Number(results.tallies[1])} | Abstentions: {Number(results.tallies[2])}</p>
          </div>
          <button onClick={reset} className="reset-btn">New Resolution</button>
        </div>
      )}
    </div>
  );
