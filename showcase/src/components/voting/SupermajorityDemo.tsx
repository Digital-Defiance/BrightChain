...
export const SupermajorityDemo = () => {
  ...
  const [voters] = useState(['State 1', 'State 2', 'State 3', 'State 4', 'State 5', 'State 6', 'State 7', 'State 8', 'State 9']);
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const { member } = Member.newMember(eciesService, MemberType.System, 'Constitutional Convention', new EmailString('convention@gov.us'));
        await member.deriveVotingKeys();
        setAuthority(member as Member);
        const newPoll = PollFactory.create(['Ratify', 'Reject'], 'supermajority' as any, member);
        setPoll(newPoll);
      } catch (e) {
  ...
  const castVote = (voterName: string, choice: number) => {
    ...
    const { member: voter } = Member.newMember(eciesService, MemberType.User, voterName, new EmailString(`${emailName}@example.com`));
  ...
  const reset = () => {
    if (!authority) return;
    const newPoll = PollFactory.create(['Ratify', 'Reject'], 'supermajority' as any, authority);
    setPoll(newPoll);
    setVotes(new Map());
    setResults(null);
  };
      ...
      <div className="voting-demo">
        <div className="election-intro">
          ...
          <div className="intro-story">
            <p>🏛️ <strong>The Stakes:</strong> Amending the Constitution requires more than a simple majority. We need a SUPERMAJORITY!</p>
            <p>🎯 <strong>2/3 Threshold:</strong> At least 66.67% must vote YES for the amendment to pass. This protects against hasty changes.</p>
            <div className="intro-stakes">
              <p>📜 <strong>The Amendment:</strong> "Add term limits for all federal judges"</p>
              <p>⚠️ <strong>High Bar:</strong> 6 out of 9 states must ratify (simple majority isn't enough!)</p>
            </div>
            <p className="intro-challenge">🌎 Used for constitutional changes, treaty ratifications, and impeachment trials.</p>
          </div>
  ...
  return (
    <div className="voting-demo">
      <div className="demo-header">
        ...
        <p className="election-tagline">📊 Requires {(threshold * 100).toFixed(0)}% to pass ({requiredVotes}/{voters.length} states)</p>
      </div>
        ...
        <>
          <div className="supermajority-tracker">
            ...
            <div className="threshold-bar">
              <div className="threshold-progress" style={{ width: `${yesPercent}%` }}>
                {yesVotes > 0 && <span>{yesVotes} YES</span>}
              </div>
              <div className="threshold-line" style={{ left: `${threshold * 100}%` }}>
                <span className="threshold-label">{(threshold * 100).toFixed(0)}% Required</span>
              </div>
            </div>
          ...
          <div className="voters-section">
            <h4>State Legislatures ({votes.size}/{voters.length} voted)</h4>
                ...
                <div key={voter} className="voter-card">
                  ...
                  {votes.has(voter) ? (
                    ...
                    <div className="yesno-buttons">
                      <button onClick={() => castVote(voter, 0)} className="vote-btn yes-btn">✅ RATIFY</button>
                      <button onClick={() => castVote(voter, 1)} className="vote-btn no-btn">❌ REJECT</button>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="results-section">
          ...
          <div className="tally-visualization">
            ...
            <p className="tally-explain">
              Required: {requiredVotes}/{voters.length} states ({(threshold * 100).toFixed(0)}%)<br/>
              Actual: {Number(results.tallies[0])}/{votes.size} states ({((Number(results.tallies[0]) / votes.size) * 100).toFixed(1)}%)
            </p>
          </div>

          <div className="supermajority-final">
            <div className="final-bar">
              <div className="final-yes" style={{ width: `${(Number(results.tallies[0]) / votes.size) * 100}%` }}>
                ✅ {Number(results.tallies[0])} RATIFY
              </div>
              <div className="final-no" style={{ width: `${(Number(results.tallies[1]) / votes.size) * 100}%` }}>
          ...
          <div className="referendum-outcome">
            <h3>{Number(results.tallies[0]) >= requiredVotes ? '✅ AMENDMENT RATIFIED!' : '❌ AMENDMENT FAILS!'}</h3>
            <p>
          ...
          <button onClick={reset} className="reset-btn">New Amendment</button>
        </div>
      )}
    </div>
  );
