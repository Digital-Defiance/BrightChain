    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ([senderId, recipientId], content) => {
          ...
          // encryptedContent MUST NOT equal the original content string
          expect(result.encryptedContent).not.toBe(content);

          // encryptedContent MUST be a block reference (magnet URL format)
          expect(result.encryptedContent).toMatch(/^magnet:\?/);
