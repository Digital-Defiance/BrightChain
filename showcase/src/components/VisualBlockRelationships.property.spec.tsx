...
class MockVisualRelationshipManager {
  ...
  private generateBlockColor = (block: MockBlockInfo, fileId: string, highlighted: boolean = false): string => {
  ...
  describe('Property 3: Visual Block Relationships', () => {
    ...
    it('should handle files with different block counts correctly', () => {
      ...
      for (const testCase of testCases) {
        ...
        const connections = relationshipManager.getConnectionsForFile(testCase.fileId);
        ...
        for (const connection of connections) {
          ...
          const fromBlock = relationshipManager.getBlockVisualState(connection.fromBlockId);
          const toBlock = relationshipManager.getBlockVisualState(connection.toBlockId);
    ...
    it('should handle edge cases gracefully', () => {
      ...
      const singleBlockFile: MockFileReceipt = {
        ...
        blocks: [
          { id: 'single-block-0', index: 0, size: 100, checksum: 'mock-checksum-single' }
        ]
      };
      ...
      const singleFileConnections = relationshipManager.getConnectionsForFile('single-file');
      expect(singleFileConnections).toHaveLength(0);
      
      // But the block should still be highlighted
      const singleBlockState = relationshipManager.getBlockVisualState('single-block-0');
    ...
    it('should maintain performance with large numbers of blocks', () => {
      ...
      const connections = relationshipManager.getConnectionsForFile('large-file');
    ...
    it('should provide consistent visual indicators across all relationship types', () => {
      ...
      for (const file of mockFiles) {
        ...
        for (const block of fileBlocks) {
          ...
          const blockConnections = relationshipManager.getConnectionsForBlock(block.id);
          expect(visualState!.connected).toBe(blockConnections.length > 0);
          expect(visualState!.connectionStrength).toBe(blockConnections.length > 0 ? 1.0 : 0);
        }
