...
const createServices = () => {
  ...
  const serverService = new ServerService({ channelService, storageProvider: createMockStorageProvider() });
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (serverName, ownerId, queryUser, addQueryUserAsMember) => {
            ...
            const ownerResult =
              await serverService.listServersForMember(ownerId);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (serverName, ownerId, nonOwnerId) => {
            ...
            // Owner CAN update
            await expect(
              serverService.updateServer(server.id, ownerId, {
                name: 'new-name',
              }),
            ...
            await expect(
              serverService.updateServer(server.id, nonOwnerId, {
                name: 'hack',
              }),
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (serverName, ownerId, newMemberIds) => {
            ...
            const filteredNewMembers = newMemberIds.filter(
              (id) => id !== ownerId,
            );
            ...
            await serverService.addMembers(
              server.id,
              ownerId,
              filteredNewMembers,
            );
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (serverName, ownerId, memberId) => {
            ...
            const channelBefore =
              channelService.getChannelById(generalChannelId)!;
            expect(
              channelBefore.members.some((m) => m.memberId === memberId),
            ).toBe(true);
            ...
            for (const channelId of updatedServer.channelIds) {
              ...
              if (channel) {
                expect(
                  channel.members.some((m) => m.memberId === memberId),
                ).toBe(false);
              }
            }
          },
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (serverName, ownerId, maxUses) => {
            ...
            for (let i = 0; i < maxUses; i++) {
              await serverService.redeemInvite(
                server.id,
                invite.token,
                userIds[i],
              );
