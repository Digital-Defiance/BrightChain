...
export class GroupService {
  ...
  public async init(): Promise<void> {
    ...
    if (!this.groupCollection) return;

    this.initialized = true;

    // ── Load groups ───────────────────────────────────────────────────
    let loadedGroups: IGroup[];
    try {
      loadedGroups = await this.groupCollection.findMany();
    } catch (error) {
      console.error(
        '[GroupService] Failed to load from groups collection:',
        error,
      );
      throw error;
    }
    ...
    for (const group of loadedGroups) {
      ...
      const epochState = reconstructKeyEpochState(group.encryptedSharedKey);
      if (epochState) {
        this.keyEpochStates.set(group.id, epochState);
      } else {
        console.warn(
          `[GroupService] Skipping key epoch reconstruction for group ${group.id}: malformed encryptedSharedKey`,
        );
      }
    ...
    try {
      ...
      console.error(
        '[GroupService] Failed to load from groupMessages collection:',
        error,
      );
      throw error;
    }
