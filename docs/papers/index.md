...
- [BrightLink Protocol](brightlink) — Hardware-anchored credential delivery for developer workflows: a Unix-socket protocol between CLI tools and a single resident desktop agent (BrightNexus on Apple Silicon), with key custody anchored to Apple s Secure Enclave.
