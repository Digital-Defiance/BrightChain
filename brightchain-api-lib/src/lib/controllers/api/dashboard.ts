...
> {
  ...
  private async handleGetDashboard(): Promise<{
    ...
    try {
      ...
      const { nodes, localNodeId, nodeIdSource, disconnectedPeers } =
        this.gatherNodeData();
      ...
      const pools = await this.gatherPoolInfo();
      ...
      // --- Fake email capture stats (dev/test mode only) ---
      const fakeEmail = this.gatherFakeEmailStats();
  ...
  private gatherNodeData(): {
    ...
    try {
      ...
      return {
        nodes: [],
        localNodeId: null,
        nodeIdSource: this.localNodeIdSourceOverride,
        disconnectedPeers: [],
      };
    }
  }
  ...
  /**
   * Gather pool info from the local block store (via `listPools()` /
   * `getPoolStats()`) and the PoolDiscoveryService cache (for remote pools).
   */
  private async gatherPoolInfo(): Promise<IAdminPoolInfo[]> {
    try {
      const pools: IAdminPoolInfo[] = [];
      const seen = new Set<string>();

      // Locally-known pools from the block store (new IPooledBlockStore API).
      const blockStore = this.application.services.get('blockStore') as
        | {
            listPools?: () => Promise<string[]>;
            getPoolStats?: (pool: string) => Promise<{
              poolId: string;
              blockCount: number;
              totalBytes: number;
              createdAt?: Date;
      ...
      if (blockStore?.listPools) {
        try {
          const localPoolIds = await blockStore.listPools();
          for (const poolId of localPoolIds) {
            if (seen.has(poolId)) continue;
            seen.add(poolId);
            let metadata: Record<string, unknown> | undefined;
            if (blockStore.getPoolStats) {
              try {
                ...
                // Per-pool stats failure is non-fatal.
              }
            }
            pools.push({ poolId, ...(metadata ? { metadata } : {}) });
          }
        } catch {
          // Local pool enumeration failure is non-fatal; fall through.
        }
      }

      // Remote pools from PoolDiscoveryService cache.
      ...
      if (qsm && !seen.has('brightTrust-system')) {
   ...
   *
   * Prefers the new IPooledBlockStore APIs (`listPools()` /
   * `listBlocksInPool()` / `getPoolStats()`) when available, falling back to
   * the legacy in-memory metadata store for older implementations.
   */
  private async gatherBlockStoreStats(): Promise<IAdminBlockStoreStats> {
    const empty: IAdminBlockStoreStats = {
      totalBlocks: 0,
      totalSizeBytes: 0,
      countByDurability: {},
    };
    ...
    try {
      ...
      const store = blockStore as {
        ...
        getPoolStats?: (
          pool: string,
        ) => Promise<{ blockCount: number; totalBytes: number }>;
        getMetadata?: (
          key: string,
        ) => Promise<{ durabilityLevel?: string } | null>;
        getMetadataStore?: () => {
          size?: () => number;
      ...
      // Preferred path: aggregate over pools using the new pooled APIs.
      if (typeof store.listPools === 'function') {
        try {
          ...
          for (const poolId of poolIds) {
            // Totals from pool stats (cheap aggregate).
            if (typeof store.getPoolStats === 'function') {
              try {
                const stats = await store.getPoolStats(poolId);
                totalBlocks += stats.blockCount ?? 0;
                totalSizeBytes += stats.totalBytes ?? 0;
              } catch {
                // Per-pool stats failure is non-fatal.
              }
            }

            // Best-effort durability breakdown via per-block metadata.
            const listBlocksInPool = store.listBlocksInPool;
            const getMetadata = store.getMetadata;
            if (listBlocksInPool && getMetadata) {
              try {
                for await (const hash of listBlocksInPool(poolId)) {
                  try {
                    const meta = await getMetadata(hash);
                    const level = meta?.durabilityLevel ?? 'unknown';
                    countByDurability[level] =
                      (countByDurability[level] ?? 0) + 1;
                  } catch {
                    // Skip blocks whose metadata cannot be fetched.
                  }
                }
              } catch {
                // Skip pools that fail to enumerate.
              }
            }
          }
          ...
          // Fall through to legacy path on enumeration failure.
        }
      }

      // Legacy path: in-memory metadata store iteration.
  ...
  /**
   * Gather fake email capture stats when running in dev/test email mode.
   * Returns undefined when not in fake email mode.
   */
  private gatherFakeEmailStats():
    | { recipientCount: number; totalCaptured: number }
    | undefined {
    try {
      const env = this.application.environment;
      const useFakeEmail =
        env.disableEmailSend || env.emailService === EmailServices.Fake;
      if (!useFakeEmail) {
        return undefined;
      }
      const svc = FakeEmailService.getInstance();
      const recipients = svc.getAllRecipients();
      const totalCaptured = recipients.reduce(
        (sum, addr) => sum + svc.getEmails(addr).length,
        0,
      );
      return { recipientCount: recipients.length, totalCaptured };
    } catch {
      return undefined;
    }
  }
