...
function createMockGossipService(): IGossipService {
  return {
    ...
    announceHeadUpdate: jest.fn<any>().mockResolvedValue(undefined),
    announceACLUpdate: jest.fn<any>().mockResolvedValue(undefined),
...
describe('Email Integration via MessagePassingService', () => {
  ...
  describe('cross-node delivery via gossip', () => {
    ...
    it('should simulate two-node delivery: sender stores, recipient queries inbox', async () => {
      ...
      nodeA.configureEmail(sharedStore, {
        nodeId: 'node-a.brightchain',
        canonicalDomain: 'brightchain',
      });
      ...
      nodeB.configureEmail(sharedStore, {
        nodeId: 'node-b.brightchain',
        canonicalDomain: 'brightchain',
      });
    ...
    it('should handle CC recipients across nodes', async () => {
      ...
      nodeC.configureEmail(sharedStore, {
        nodeId: 'node-c.brightchain',
        canonicalDomain: 'brightchain',
      });
  ...
  describe('multi-node cross-node delivery', () => {
    function createNode(
      ...
      svc.configureEmail(sharedStore, {
        nodeId,
        canonicalDomain: 'brightchain',
      });
