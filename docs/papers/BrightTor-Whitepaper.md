...
BrightChain's `BlockAnnouncement` interface uses a discriminated type union for its `type` field. BrightTor extends this union additively with three new values. The existing announcement types (`add`, `remove`, `ack`, `pool_deleted`, `cbl_index_update`, `cbl_index_delete`, `head_update`, `acl_update`, `pool_announce`, `pool_remove`, `BrightTrust_proposal`, `BrightTrust_vote`) remain unchanged. The three additions are:
