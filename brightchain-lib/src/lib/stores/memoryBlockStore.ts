...
import { InMemoryHeadRegistry } from '../db/inMemoryHeadRegistry';
...
export class MemoryBlockStore implements IBlockStore {
   ...
   * Picks the smallest supported block size that fits the data. Throws
   * `StoreError(BlockValidationFailed)` when no configured size can hold
   * the data.
   */
  public async put(
    key: Checksum | string,
    ...
    // Find the smallest supported size that fits, or BlockSize.Unknown
    // (which is treated as accepting any size).
    ...
    const block = new RawDataBlock(fitting, data);
   ...
   */
  public async *listBlockIds(options?: { limit?: number; cursor?: string }): AsyncIterable<string> {
