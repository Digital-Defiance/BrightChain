...
/**
 * @fileoverview Unit tests for Quorum API endpoints (Task 22).
 *
 * Tests request validation, success responses, error responses, and
 * service unavailability for the new quorum bootstrap redesign endpoints:
...
import { QuorumController } from './quorum';
...
describe('QuorumController - Bootstrap Redesign Endpoints', () => {
  let controller: QuorumController<any>;
  ...
  beforeEach(() => {
    ...
    controller = new QuorumController(createMockApplication() as any);
    controller.setQuorumStateMachine(mockStateMachine as any);
  ...
  describe('handleSubmitProposal', () => {
    const validBody = {
      description: 'Add a new member to the quorum',
    ...
    it('should return 503 when state machine is not set', async () => {
      const noSmController = new QuorumController(
    ...
    it('should handle QuorumError from state machine', async () => {
      mockStateMachine.submitProposal.mockRejectedValue(
        new QuorumError(QuorumErrorType.MissingAttachment),
      );
      const result = await (controller as any).handleSubmitProposal({
        body: validBody,
      });
      // mapQuorumError returns an error response
  ...
  describe('handleGetStatus', () => {
    it('should return 200 with status data', async () => {
      mockStateMachine.getMode.mockResolvedValue(QuorumOperationalMode.Quorum);
      ...
      expect(result.statusCode).toBe(200);
      expect(result.response.status.mode).toBe(QuorumOperationalMode.Quorum);
    ...
    it('should return bootstrap mode when in bootstrap', async () => {
      ...
      expect(result.statusCode).toBe(200);
      expect(result.response.status.mode).toBe(QuorumOperationalMode.Bootstrap);
  ...
  describe('handleAuditVerify', () => {
    ...
    it('should return 503 when audit log service is not set', async () => {
      const noAuditController = new QuorumController(
        createMockApplication() as any,
      );
      noAuditController.setQuorumStateMachine(mockStateMachine as any);
