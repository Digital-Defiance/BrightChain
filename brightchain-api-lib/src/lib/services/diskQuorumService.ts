 ...
 */
export class DiskQuorumService extends QuorumService<GuidV4> {
  ...
  override async addMember(
    member: Member<GuidV4>,
    metadata: QuorumMemberMetadata,
  ): Promise<IQuorumMember<GuidV4>> {
  ...
  override async getMember(
    memberId: ShortHexGuid,
  ): Promise<IQuorumMember<GuidV4> | null> {
  ...
  override async listMembers(): Promise<IQuorumMember<GuidV4>[]> {
  ...
  override async sealDocument<T>(
    ...
    memberIds: ShortHexGuid[],
    sharesRequired?: number,
  ): Promise<SealedDocumentResult<GuidV4>> {
