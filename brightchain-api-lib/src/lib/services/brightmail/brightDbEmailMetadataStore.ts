...
function serializeEmail(metadata: IEmailMetadata): EmailDocument {
  ...
  if (metadata.deliveryStatus instanceof Map) {
    const statuses: Record<string, string> = {};
    for (const [k, v] of metadata.deliveryStatus) {
      statuses[String(k)] = String(v);
    }
  ...
  if (metadata.encryptionAuthTag) {
    doc['encryptionAuthTag'] = Buffer.from(metadata.encryptionAuthTag).toString(
      'base64',
    );
  }
  if (metadata.contentSignature) {
    doc['contentSignature'] = Buffer.from(metadata.contentSignature).toString(
      'base64',
    );
  }
  if (metadata.signerPublicKey) {
    doc['signerPublicKey'] = Buffer.from(metadata.signerPublicKey).toString(
      'base64',
    );
  }
...
function deserializeEmail(doc: EmailDocument): IEmailMetadata {
  ...
  if (result.customHeaders && !(result.customHeaders instanceof Map)) {
    ...
    for (const [k, v] of Object.entries(
      result.customHeaders as Record<string, string[]>,
    )) {
      map.set(k, v);
    }
  ...
  if (result.deliveryReceipts && !(result.deliveryReceipts instanceof Map)) {
    ...
    for (const [k, v] of Object.entries(
      result.deliveryReceipts as Record<string, unknown>,
    )) {
      map.set(k, v);
    }
  ...
  if (result.readReceipts && !(result.readReceipts instanceof Map)) {
    ...
    for (const [k, v] of Object.entries(
      result.readReceipts as Record<string, string>,
    )) {
  ...
  if (result.encryptedKeys && !(result.encryptedKeys instanceof Map)) {
    ...
    for (const [k, v] of Object.entries(
      result.encryptedKeys as Record<string, string>,
    )) {
  ...
  // Reconstitute deliveryStatus Map
  if (result.deliveryStatus && !(result.deliveryStatus instanceof Map)) {
    const map = new Map<string, string>();
    for (const [k, v] of Object.entries(
      result.deliveryStatus as Record<string, string>,
    )) {
      map.set(k, v);
    }
...
export class BrightDbEmailMetadataStore implements IEmailMetadataStore {
  ...
  async delete(messageId: string): Promise<void> {
    // Delete all copies — sendEmail stores a sender copy and a recipient
    // copy with the same messageId, so deleteOne would leave one behind.
    await this.emailCollection.deleteMany({
      messageId,
    } as Partial<EmailDocument>);
  }
  ...
  async storeAttachmentContent(
    ...
    await this.attachmentCollection.deleteOne({
      key,
    } as Partial<AttachmentDocument>);
  ...
  private applyFilters(
    ...
    if (query.labels && query.labels.length > 0) {
      results = results.filter((e) =>
        query.labels!.every((label) => e.keywords?.includes(label)),
      );
    }

    if (query.isStarred !== undefined) {
      results = results.filter(
        (e) => Boolean(e.isStarred) === query.isStarred,
      );
    }
