...
describe('StaticHelpersECIES', () => {
  ...
  test('encrypt and decrypt (buffer version) should return original message', () => {
    ...
    const decrypted = StaticHelpersECIES.decryptWithHeader(
      keyPair.privateKey,
      encrypted,
    );
  ...
  test('decrypt (buffer version) with wrong private key should throw error', () => {
    ...
    expect(() => {
      StaticHelpersECIES.decryptWithHeader(wrongPrivateKey, encrypted);
    }).toThrow();
  });
  ...
  it('should derive a known key', () => {
    ...
    const seed =
      StaticHelpersECIES.walletAndSeedFromMnemonic(mnemonic).seed.value;
    const testKeyPair = StaticHelpersECIES.seedToSimpleKeyPairBuffer(seed);
  ...
  it('should return false when a the signature is altered', () => {
    ...
    const modifiedSignature: SignatureBuffer = Buffer.copyBytesFrom(
      signature,
    ) as SignatureBuffer;
  ...
  it('should validate ecie overhead length', () => {
    ...
    expect(encryptedData.length).toBe(
      StaticHelpersECIES.eciesOverheadLength + inputData.length,
    );
  });
});
