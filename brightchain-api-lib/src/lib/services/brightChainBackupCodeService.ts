...
export class BrightChainBackupCodeService<TID extends PlatformID = Uint8Array> {
   ...
   * 2. Retrieve the member's encrypted mnemonic from their private profile
   * 3. Decrypt the mnemonic using the system user's private key
   * 4. Reconstruct a node-ecies-lib Member with private key from the mnemonic
   * 5. Generate `Count` random backup codes via upstream BackupCode primitives
   * 6. Encrypt codes (Argon2id KDF → AEAD → ECIES wrap) via upstream static method
   * 7. Persist encrypted codes to member's private profile via MemberStore
   * 8. Return plaintext display-formatted strings
   */
  async generateCodes(memberId: TID): Promise<string[]> {
    const systemUser = this.getSystemUser();

    // Retrieve the member's private profile to get the encrypted mnemonic
    const profile = await this.memberStore.getMemberProfile(memberId);
    if (!profile.privateProfile?.mnemonicRecovery) {
      throw new Error(
        'Member has no encrypted mnemonic — cannot generate backup codes. ' +
          'This member may have been created before mnemonic recovery was implemented.',
      );
    }

    // Decrypt the mnemonic using the system user's private key
    const decryptedMnemonicBuf = await systemUser.decryptData(
      Buffer.from(profile.privateProfile.mnemonicRecovery, 'hex'),
    );
    const mnemonicString = decryptedMnemonicBuf.toString('utf-8');

    // Retrieve the base member for metadata (name, email, type, id)
    const baseMember = await this.memberStore.getMember(memberId);

    // Reconstruct a node-ecies-lib Member with private key from the mnemonic.
    // This produces the same keypair that was generated during registration.
    const { member: backupUser } = Member.newMember<TID>(
      this.eciesService,
      baseMember.type,
      baseMember.name,
      baseMember.email instanceof EmailString
        ? baseMember.email
        : new EmailString(String(baseMember.email)),
      new SecureString(mnemonicString),
    );

    // Patch the reconstructed member's ID to match the stored member
    Object.defineProperty(backupUser, 'id', {
      get: () => baseMember.id,
      configurable: true,
    });
  ...
  useBackupCode(
    ...
    switch (version) {
      case BackupCode.BackupCodeVersion: {
        ...
        const isValid = BackupCode.validateBackupCode(versionCodes, backupCode);
        if (!isValid) {
          throw new InvalidBackupCodeError();
        }
