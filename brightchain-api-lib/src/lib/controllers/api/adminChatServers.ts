...
> {
    ...
    try {
      ...
      if (brightDb) {
        ...
        const servers = docs.map((doc: Record<string, unknown>) => ({
          ...
          iconAssetId: doc['iconAssetId'] ?? undefined,
          iconVaultContainerId: doc['iconVaultContainerId'] ?? undefined,
          ownerId: doc['ownerId'] ?? '',
          memberCount: Array.isArray(doc['memberIds'])
            ? (doc['memberIds'] as unknown[]).length
            : 0,
          channelCount: Array.isArray(doc['channelIds'])
            ? (doc['channelIds'] as unknown[]).length
            : 0,
  ...
  private async handleUpdateServer(
    ...
    try {
      ...
      if (brightDb) {
        ...
        if (request.body?.iconUrl !== undefined)
          update['iconUrl'] = request.body.iconUrl;
        if (request.body?.iconAssetId !== undefined)
          update['iconAssetId'] = request.body.iconAssetId;
        if (request.body?.iconVaultContainerId !== undefined)
          update['iconVaultContainerId'] = request.body.iconVaultContainerId;

        const result = await brightDb
          .collection('brightchat_servers')
          .updateOne({ _id: serverId }, { $set: update });
  ...
  private async handleDeleteServer(
    ...
    try {
      ...
      if (brightDb) {
        ...
        await brightDb
          .collection('brightchat_channels')
          .deleteMany({ serverId });
        // Delete the server
        const result = await brightDb
          .collection('brightchat_servers')
          .deleteOne({ _id: serverId });
  ...
  private async handleListChannels(
    ...
    try {
      const request = req as {
        query?: { page?: string; limit?: string; serverId?: string };
      };
      const { page, limit, skip } = this.parsePagination(request.query);
      const brightDb = this.getDb();
      ...
      if (brightDb) {
        ...
        const channels = docs.map((doc: Record<string, unknown>) => ({
          ...
          memberCount: Array.isArray(doc['members'])
            ? (doc['members'] as unknown[]).length
            : 0,
    ...
    try {
      ...
      if (brightDb) {
        ...
        for (const doc of serverDocs) {
          ...
          const memberIds = Array.isArray(doc['memberIds'])
