...
export class AvailabilityAwareBlockStore implements IBlockStore {
  ...
  private async fetchBlockAvailable(
    ...
    const result = await Promise.race([
      fetchPromise.then((r: BlockFetchResult) => ({
    ...
    if (!result.value.success) {
      ...
      throw new PendingBlockError(
        blockId,
        AvailabilityState.Remote,
        locations.map((loc: ILocationRecord) => loc.nodeId),
      );
    }
  ...
  // === Block Enumeration (delegated to inner store) ===

  async *listBlockIds(options?: {
    limit?: number;
    cursor?: string;
  }): AsyncIterable<string> {
    if (this.innerStore.listBlockIds) {
      yield* this.innerStore.listBlockIds(options);
    }
  }

  async getBlockCount(): Promise<number> {
    if (this.innerStore.getBlockCount) {
      return this.innerStore.getBlockCount();
    }
    return 0;
  }
}
