...
> implements IAuthenticationProvider<TID, TLanguage>
{
  ...
  async buildRequestUserDTO(userId: string): Promise<IRequestUserDTO | null> {
    try {
      ...
      // Look up RBAC role from user_roles → roles junction
      let rolePrivileges = {
        admin: false,
        member: true,
        child: false,
      ...
      let roles: IRequestUserDTO['roles'] = [];
      try {
        const userRolesCol = this.db.collection<{
          userId?: unknown;
          roleId?: unknown;
        }>('user_roles');
        const rolesCol = this.db.collection<{
          _id?: string;
          name?: string;
          admin?: boolean;
          member?: boolean;
        ...
        const userRoleDoc = await userRolesCol.findOne({
          userId,
        } as never);
