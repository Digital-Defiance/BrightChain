...
const arbCloudConfig: fc.Arbitrary<ICloudBlockStoreConfig> = fc.record({
  ...
  supportedBlockSizes: fc
    .constantFrom(...validBlockSizes)
    .map((bs) => [bs] as readonly BlockSize[]),
  keyPrefix: fc.option(
    fc
  ...
  it('Azure: register + create returns the factory-produced instance', () => {
    fc.assert(
      fc.property(arbCloudConfig, (config) => {
        const expectedStore = createMockBlockStore(
          config.supportedBlockSizes[0],
        );
    ...
    fc.assert(
      fc.property(arbCloudConfig, (config) => {
        ...
        const factory = (cfg: ICloudBlockStoreConfig) => {
          receivedConfig = cfg;
          return createMockBlockStore(cfg.supportedBlockSizes[0]);
        };
