...
function createTestApp(dbName: string, adminKey: Uint8Array) {
  ...
  app.use(router);
  // Catch-all error handler to prevent socket hang up under parallel test load.
  // Without this, any unhandled error in Express 5 closes the TCP connection.

  app.use((err: Error, _req: Request, res: Response, _next: NextFunction) => {
    res.status(500).json({ error: err.message });
  });

  // Use a persistent server to avoid ephemeral port exhaustion and
  // truncated request bodies under heavy parallel test load.
  const server = app.listen(0);
  const close = (): Promise<void> =>
    new Promise((resolve) => server.close(() => resolve()));

  return { app, server, close, aclManager, auditLogger };
}
...
describe('Feature: brightdb-write-acls, Property 14: API Admin Authentication Enforcement', () => {
    ...
    await fc.assert(
      fc.asyncProperty(arbName, arbEndpoint, async (dbName, endpoint) => {
        ...
        try {
          ...
          // We deliberately do NOT send request bodies on POST/PUT to avoid
          // express.json() body-parser races under heavy parallel test load
  ...
  it('mutating ACL requests with valid admin signature SHALL succeed (not 403)', async () => {
    await fc.assert(
      fc.asyncProperty(arbName, arbEndpoint, async (dbName, endpoint) => {
        const adminKey = makePublicKey(0x42);
        const { server, close, aclManager } = createTestApp(dbName, adminKey);
