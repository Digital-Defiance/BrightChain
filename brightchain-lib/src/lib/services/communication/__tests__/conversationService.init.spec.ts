...
describe('ConversationService.init()', () => {
  ...
  it('should be idempotent: second call is a no-op', async () => {
    ...
    const findManySpy = jest.spyOn(storageProvider.conversations, 'findMany');

    const service = new ConversationService(null, undefined, storageProvider);
  ...
  it('should log warning and skip epoch for conversation with non-Map encryptedSharedKey', async () => {
    const conversation: IConversation = {
      ...
      encryptedSharedKey: { 0: { carol: 'key' } } as unknown as Map<
        number,
        Map<string, string>
      >,
      createdAt: 9000,
