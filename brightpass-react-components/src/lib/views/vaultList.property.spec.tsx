...
function formatDate(date: Date): string {
  ...
  // VaultListView appends BrightDate — our mock toBrightDateString returns
  // a decimal days-since-J2000 value, so replicate that here.
  const daysSinceJ2000 = (d.getTime() - 946684800000) / 86400000;
  const bd = daysSinceJ2000.toFixed(3);
 ...
 * Uses noon UTC to avoid timezone boundary issues where midnight UTC
 * renders as the previous day in local time. */
const arbDate = fc
  .date({
    min: new Date('2000-01-01T12:00:00Z'),
    max: new Date('2099-12-31T12:00:00Z'),
  })
  .filter((d) => !isNaN(d.getTime()))
  .map((d) => {
    // Normalize to noon UTC to avoid timezone edge cases
