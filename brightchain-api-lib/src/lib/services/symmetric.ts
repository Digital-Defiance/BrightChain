...
export class SymmetricService {
  ...
  public static get symmetricKeyBytes(): number {
    return ECIES.SYMMETRIC.KEY_SIZE;
  }
  ...
  public static encryptBuffer(
    ...
    if (encryptionKey && encryptionKey.length != ECIES.SYMMETRIC.KEY_SIZE) {
    ...
    const key: Buffer = encryptionKey ?? randomBytes(ECIES.SYMMETRIC.KEY_SIZE);
