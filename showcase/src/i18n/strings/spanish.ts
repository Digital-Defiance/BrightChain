...
> = {
  ...
  [ShowcaseStrings.FAQ_Ohm_Character]: 'Personaje de Ohm',
  [ShowcaseStrings.FAQ_Volta_Character]: 'Personaje de Volta',
  ...
  [ShowcaseStrings.FAQ_Toggle_Technical_Sublabel]: 'Ohm guarda los detalles',
  [ShowcaseStrings.FAQ_Toggle_Ecosystem_Sublabel]: 'Volta revela la visión',
  ...
  [ShowcaseStrings.FAQ_Tech_Q11_TopSecret_Title]: 'Ultra Secreto',
  [ShowcaseStrings.FAQ_Tech_Q11_TopSecret_Text]:
    'Una dApp Ultra Secreta viene con el lanzamiento que cambiará todo lo que sabes y revolucionará el juego completamente.',
  ...
  [ShowcaseStrings.FAQ_Tech_Q16_Answer]:
    'IPFS es transparente por defecto; si tienes el hash, puedes ver el archivo. BrightChain usa una Línea Base XOR. Los datos son funcionalmente "Triturados" antes de que toquen la red. La privacidad no es un "plugin" — es el estado mecánico de los datos.',
  ...
  [ShowcaseStrings.FAQ_Eco_Soot_Answer]:
    'El Hollín es la consecuencia visible de cada operación — la "huella de carbono" de tus acciones digitales. No es una moneda que gastas; es un costo que no puedes evitar. Cada vez que se consume energía, se produce Hollín — una representación visual del trabajo computacional que se acumula con el tiempo. Cuanto más haces, más Hollín se acumula. El uso ligero deja una huella aquí y allá; el uso pesado crea una acumulación visible. El Hollín representa el karma en el ecosistema BrightChain: cada acción deja una marca. En palabras de Ohm: "Volta te da la energía, las operaciones la convierten en calor, y el sistema rastrea las consecuencias. Yo solo me aseguro de que no desperdiciemos más de lo necesario."',
  ...
  [ShowcaseStrings.FAQ_Eco_HowFitsTogether_Answer]:
    'El ecosistema funciona con la tensión dinámica entre Volta (el Gastador) y Ohm (el Ahorrador), con Julios fluyendo entre ellos como la moneda de energía. Volta está ansiosa por alimentar cada operación a máxima potencia, mientras que Ohm se asegura de que los recursos no se desperdicien. Cada acción consume Julios y produce Hollín — la huella visible del trabajo computacional. Cuando se dispara una operación, Volta alcanza los Julios, Ohm evalúa el costo y reluctantemente los deja pasar, y el sistema rastrea el Hollín resultante. Esto crea una economía auto-equilibrada donde la contribución equivale a la capacidad, y cada acción deja su marca en la red.',

  [ShowcaseStrings.FAQ_Eco_Beliefs_Title]: '🧘 ¿En qué cree BrightChain?',
  [ShowcaseStrings.FAQ_Eco_Beliefs_Answer]:
    'La energía se conserva. Las acciones tienen consecuencias. Los datos tienen peso. Cada elemento en el ecosistema BrightChain mapea a un principio más profundo: Volta es la Chispa — potencial puro y caótico y el deseo de actuar. Ohm es el Ancla — sabiduría arraigada y la disciplina para actuar correctamente. Los Julios son el Flujo — el espíritu moviéndose entre ellos. El Hollín es el Karma — el costo visible que no puede evitarse. Juntos forman un ciclo cerrado: Volta proporciona la energía, Ohm asegura que se gaste sabiamente, y el sistema rastrea cada consecuencia. Nada es gratis. Nada se desperdicia. Todo deja una marca.',
  ...
  [ShowcaseStrings.FAQ_Eco_MascotsInAction_Answer]:
    'Las mascotas están tejidas a lo largo de la experiencia del producto. Volta y Ohm viven en el medidor de Julios de toda la plataforma, con Volta crepitando cerca del indicador de energía y Ohm interviniendo durante operaciones costosas para realizar su Meditación Resistiva — cambiando la barra de progreso de azul neón a un ámbar calmado. El Hollín se acumula visiblemente a lo largo de tu sesión, reflejando el trabajo computacional realizado. Próximamente: apariciones de mascotas en páginas de error, pantallas de carga, diálogos de confirmación escalados a la severidad de la operación, y sí — merchandising.',
