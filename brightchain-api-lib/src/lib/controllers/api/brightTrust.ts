...
import {
  ...
  IBrightTrustMember,
  IBrightTrustMetricsData,
  IBrightTrustStatusData,
  IEpochData,
  IProposalData,
  ISubmitProposalData,
  IVoteData,
...
import { BrightTrustServiceWrapper } from '../../services/brightTrust';
...
/**
 * Error codes for BrightTrust API operations
 * @deprecated Use ErrorCode from '../../utils/errorResponse' instead
 */
export enum BrightTrustErrorCode {
...
interface GetDocumentResponse extends IApiMessageResponse {
  document: BrightTrustDocumentInfo<GuidV4Buffer>;
  [key: string]: any;
}
...
// === New BrightTrust Bootstrap Redesign Response Interfaces ===
...
interface GetMetricsResponse extends IApiMessageResponse {
  metrics: IBrightTrustMetricsData;
  [key: string]: any;
}
...
/**
 * Serialized BrightTrust member for API responses
 */
interface SerializedBrightTrustMember {
  id: string;
...
function idToHex<TID extends PlatformID>(id: TID): string {
  ...
  try {
    ...
    return Buffer.isBuffer(binaryId) || binaryId instanceof Uint8Array
...
interface BrightTrustHandlers extends TypedHandlers {
...
/**
 * Controller for BrightTrust operations including member management
 * and document sealing/unsealing.
 *
 * Provides REST API endpoints for managing BrightTrust members and performing
 ...
 */
export class BrightTrustController<
  TID extends PlatformID = DefaultBackendIdType,
> extends BaseController<
  TID,
  BrightTrustApiResponse,
  BrightTrustHandlers,
  CoreLanguageCode
> {
  private brightTrustServiceWrapper: BrightTrustServiceWrapper<TID>;
  ...
  constructor(application: IBrightChainApplication<TID>) {
    super(application);
    this.brightTrustServiceWrapper = new BrightTrustServiceWrapper(application);
  }

  /**
   * Set the BrightTrustStateMachine instance for proposal/voting/epoch/metrics endpoints.
   * Called during application initialization after all dependencies are wired.
   */
  public setBrightTrustStateMachine(
    stateMachine: BrightTrustStateMachine<TID>,
  ): void {
    this.brightTrustStateMachine = stateMachine;
  }
   ...
   * Add a new member to the BrightTrust with generated cryptographic keys.
   *
   * Creates a new member with ECIES key pair and adds them to the BrightTrust.
  ...
  private async handleAddMember(
    ...
    try {
      ...
      const brightTrustService = this.brightTrustServiceWrapper.getService();
      const brightTrustMember = await brightTrustService.addMember(
      ...
      return {
        ...
        response: {
          message: 'Member added successfully',
          member: serializeMember(brightTrustMember),
          mnemonic: memberWithMnemonic.mnemonic.value ?? '',
        },
      };
    } catch (_error) {
      if (_error instanceof BrightTrustError) {
        return mapBrightTrustError(_error);
      }
   ...
   * List all active BrightTrust members.
   *
   * Returns all members that are currently active in the BrightTrust.
  ...
  private async handleListMembers(
    ...
    try {
      const brightTrustService = this.brightTrustServiceWrapper.getService();
      const members = await brightTrustService.listMembers();
   ...
   * ```json
   * // Request
   * DELETE /api/brightTrust/members/abc123...
   *
   * // Response
   * {
  ...
  private async handleRemoveMember(
    ...
    try {
      ...
      const brightTrustService = this.brightTrustServiceWrapper.getService();
      await brightTrustService.removeMember(
        memberId as unknown as GuidV4Buffer,
      );
  ...
  private async handleSealDocument(
    ...
    try {
      ...
      // Get the agent (authenticated member) from req.member
      // The authenticateToken middleware loads the full Member object
      // via authProvider.findMemberById when available.
      const agent = (req as unknown as { member?: Member<GuidV4Buffer> })
        .member;
      ...
      const brightTrustService = this.brightTrustServiceWrapper.getService();
      const result = await brightTrustService.sealDocument(
        agent,
        document,
   ...
   *
   * Note: This endpoint requires the BrightTrustService to have the document
   ...
   * ```json
   * // Request
   * POST /api/brightTrust/documents/doc123.../unseal
   * {
   *   "memberCredentials": [
  ...
  private async handleUnsealDocument(
    ...
    try {
      ...
      const docInfo = await brightTrustService.getDocument(
      ...
      const canUnlockResult = await brightTrustService.canUnlock(
        documentId as unknown as GuidV4Buffer,
        memberIds,
      );
      ...
      for (const cred of memberCredentials) {
        ...
        const storedMember = await brightTrustService.getMember(
      ...
      // Unseal the document
      const unsealedDocument = await brightTrustService.unsealDocument(
        documentId as unknown as GuidV4Buffer,
   ...
   * ```json
   * // Request
   * GET /api/brightTrust/documents/doc123...
   *
   * // Response
   * {
  ...
  private async handleGetDocument(
    ...
    try {
      ...
      const brightTrustService = this.brightTrustServiceWrapper.getService();
      const document = await brightTrustService.getDocument(
        documentId as unknown as GuidV4Buffer,
      );
   ...
   * ```json
   * // Request
   * GET /api/brightTrust/documents/doc123.../can-unlock?memberIds=member1,member2
   *
   * // Response
   * {
  ...
  private async handleCanUnlock(
    ...
    try {
      ...
      const brightTrustService = this.brightTrustServiceWrapper.getService();
      const result = await brightTrustService.canUnlock(
        documentId as unknown as GuidV4Buffer,
        memberIds,
      );
  ...
  // === BrightTrust Bootstrap Redesign Handlers (Task 22) ===

  /**
   * Ensure the BrightTrustStateMachine is available.
   * Returns an error result if not yet wired.
   */
  private ensureStateMachine() {
    if (!this.brightTrustStateMachine) {
      return createApiErrorResult(
        503,
        ErrorCode.SERVICE_UNAVAILABLE,
        'BrightTrust state machine not initialized',
      );
    }
    return null;
  }
   ...
   *
   * Validates the request body and delegates to BrightTrustStateMachine.submitProposal.
   *
  ...
  private async handleSubmitProposal(
    ...
    try {
      ...
      const proposal = await this.brightTrustStateMachine!.submitProposal({
  ...
  private async handleGetProposal(
    ...
    try {
      ...
      const proposal = await this.brightTrustStateMachine!.getProposal(
      ...
      // The BrightTrustStateMachine doesn't expose getVotesForProposal directly,
   ...
   * Expose BrightTrustMetrics from BrightTrustStateMachine.getMetrics().
   *
   * @requirements 12.5
   */
  private async handleGetMetrics(
    ...
    try {
      ...
      const metrics = await this.brightTrustStateMachine!.getMetrics();
  ...
  private async handleGetEpoch(
    ...
    try {
      ...
      const epoch = await this.brightTrustStateMachine!.getEpoch(epochNumber);
      if (!epoch) {
      ...
      const epochData: IEpochData = {
        ...
        innerBrightTrustMemberIds: epoch.innerBrightTrustMemberIds?.map((id) =>
  ...
  private async handleGetStatus(
    ...
    try {
      ...
      const mode = await this.brightTrustStateMachine!.getMode();
      const currentEpoch =
        await this.brightTrustStateMachine!.getCurrentEpoch();

      const statusData: IBrightTrustStatusData = {
        mode,
   ...
   * Check alias availability (public) or resolve alias (requires brightTrust auth).
   *
   * This endpoint is public for availability checks. Alias resolution
   * (mapping alias to real identity) requires a BrightTrust vote via the
  ...
  private async handleGetAlias(
    ...
    try {
      ...
      // proposal with brightTrust approval — that flow is handled via POST /proposals.
      const _brightTrustService = this.brightTrustServiceWrapper.getService();
      // The BrightTrustServiceWrapper wraps PersistentBrightTrustService which doesn't have
