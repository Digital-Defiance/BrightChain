    ...
    it("GET roles for each member returns only that member's roles with no cross-contamination", async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          memberIdArb,
          memberIdArb,
          fc.tuple(validPractitionerCodeArb, validPractitionerCodeArb),
          async (
            orgName,
            adminId,
            staff1Id,
            ...
            for (const role of patientRoles) {
              if (role.patient) {
                expect(role.patient.reference).toBe(`Patient/${patientId}`);
              }
        ...
        fc.asyncProperty(
          ...
          async (
            ...
            const directRole = directRoles.find(
              (r) =>
                r.roleCode === roleCode && r.organization.reference === orgRef,
            );
