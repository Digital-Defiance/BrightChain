...
// Interface for SerializedMemberData was removed as it's no longer needed
...
export class TestMembers {
  ...
  private static initialize() {
    ...
    const votingService = new VotingService(this.eciesService); // Pass eciesService
    memberConfigs.forEach((config) => {
      ...
      const { member: newMember } = BrightChainMember.newMember(
        this.eciesService, // Use static instance
        votingService,
        config.type,
        config.name,
  ...
  // Removed recreateMemberFromJson method as it's no longer used in the tests
}
