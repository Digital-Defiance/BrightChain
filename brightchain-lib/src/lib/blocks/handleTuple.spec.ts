...
class TestDiskBlockAsyncStore extends DiskBlockAsyncStore {
  public ensureTestBlockPath(blockId: ChecksumUint8Array): void {
    this.ensureBlockPath(blockId);
  }

  public getTestBlockPath(blockId: ChecksumUint8Array): string {
...
describe('BlockHandleTuple', () => {
  ...
  describe('XOR operations', () => {
    it('should XOR blocks correctly', async () => {
      ...
      const resultHandle = await store.get(xorResult.idChecksum);
