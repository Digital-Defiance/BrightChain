      ...
      <Box sx={{ mb: 2 }}>
        <PostComposer
          ...
          stagingApi={{
            ...
            discardFile: async (commitToken: string) => {
              await api.delete(`/temp-upload/${commitToken}`);
            },
          }}
        />
      </Box>
        ...
        <>
          ...
          <Timeline
            ...
            onAuthorClick={(authorId) => {
              ...
              const slug = author?.username ? `@${author.username}` : authorId;
              navigate(`/brighthub/profile/${slug}`);
            }}
            onDelete={(postId) => {
              if (!userId) return;
            ...
            onPin={(postId) => {
              ...
              const req = isPinned
                ? api.delete(`/brighthub/posts/${postId}/pin`, { data: { userId } })
                : api.post(`/brighthub/posts/${postId}/pin`, { userId });
              req
                .then(() => {
                  setPosts((prev) =>
                    prev.map((p) => ({
                      ...p,
                      isPinned: p._id === postId ? !isPinned : (isPinned ? p.isPinned : false),
                    })),
                  );
                })
                .catch(() => {});
...
const ThreadPage: FC = () => {
  ...
  return <ThreadView thread={thread} authors={authors} />;
};
...
const ProfilePage: FC = () => {
  ...
  const [pinnedPost, setPinnedPost] = useState<IBasePostData<string> | null>(
    null,
  );

  // The URL param can be @username or a raw ID
  const isUsernameRoute = id?.startsWith('@') ?? false;
  const lookupParam = isUsernameRoute ? id : id;
  const isSelf = isUsernameRoute
    ? userData?.username === id?.slice(1)
    : userData?.id === id;
  ...
  useEffect(() => {
    ...
    // The API endpoint handles both @username and raw ID
    const url = userData?.id
      ? `/brighthub/users/${encodeURIComponent(lookupParam)}?requesterId=${userData.id}`
      : `/brighthub/users/${encodeURIComponent(lookupParam)}`;
    ...
    api
      ...
      .then((res) => {
        if (!cancelled && res.data?.data) {
          ...
          // If we navigated by ID but now have the username, update the URL
          // to the canonical @username form without a full navigation
          if (
            !isUsernameRoute &&
            res.data.data.username &&
            res.data.data.username !== lookupParam
          ) {
            navigate(`/brighthub/profile/@${res.data.data.username}`, {
              replace: true,
            });
          }
        }
  ...
  }, [api, lookupParam, userData?.id]);

  // Fetch pinned post when profile has a pinnedPostId
  useEffect(() => {
    ...
    let cancelled = false;
    api
      .get(`/brighthub/posts/${user.pinnedPostId}`)
      .then((res) => {
        if (!cancelled && res.data?.data) {
          setPinnedPost(res.data.data);
        }
      })
      .catch(() => {
  ...
  return (
    <Box>
      <UserProfileCard
        ...
        onEditProfile={isSelf ? () => setEditProfileOpen(true) : undefined}
      />
      {/* Pinned post */}
      {pinnedPost && (
        <Box sx={{ mt: 2 }}>
            ...
            onUnpin={
              isSelf
                ? (postId) => {
                    ...
                    api
                      .delete(`/brighthub/posts/${postId}/pin`, {
                        data: { userId: userData.id },
                      })
                      .then(() => setPinnedPost(null))
                      .catch(() => {});
                  }
                : undefined
            }
      ...
      {isSelf && (
        <EditProfileDialog
          ...
          onSave={async (updates: EditProfileUpdates) => {
            if (!userData?.id) return;
            await api.put(`/brighthub/users/${userData.id}`, {
              userId: userData.id,
              ...updates,
            });
