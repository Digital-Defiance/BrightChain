  ...
  implements IMemberStore<TID>
{
  ...
  public async getMemberPublicKeyHex(id: TID): Promise<string | null> {
    ...
    if (this._db) {
      ...
      if (userDoc) {
        console.warn(
          `[MemberStore] getMemberPublicKeyHex: user document found for id=${idHex} but publicKey is empty`,
        );
      } else {
        console.warn(
          `[MemberStore] getMemberPublicKeyHex: no user document found in users collection for id=${idHex}`,
        );
      }
    } else {
      console.warn(
        '[MemberStore] getMemberPublicKeyHex: no DB reference set, skipping fast path',
      );
    }
  ...
  public async updateIndex(entry: IMemberIndexEntry<TID>): Promise<void> {
    ...
    if (this._db) {
      try {
        const indexCol =
          this._db.collection<IMemberIndexDocument>('member_index');
        const doc: IMemberIndexDocument = {
          id,
        ...
        const existing = await indexCol.findOne({ id } as never);
        if (existing) {
          await indexCol.updateOne({ id } as never, { $set: doc } as never);
        } else {
          await indexCol.insertOne(doc as never);
        }
      } catch (err) {
        ...
        // partial recovery.
        console.warn(
          '[MemberStore] Failed to persist member_index entry to DB:',
