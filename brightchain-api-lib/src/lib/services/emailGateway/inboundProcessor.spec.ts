...
function makeMockEmailMessageService(): EmailMessageService & {
  receiveEmail: jest.Mock;
} {
  return {
    receiveEmail: jest.fn().mockResolvedValue({
      messageId: '<test@brightchain.org>',
      brightchainMessageId: 'bc-1',
      deliveryStatus: new Map(),
      success: true,
    }),
  } as unknown as EmailMessageService & { receiveEmail: jest.Mock };
}
...
describe('InboundProcessor', () => {
  ...
  let emailService: EmailMessageService & { receiveEmail: jest.Mock };
  ...
  describe('processFile', () => {
    it('should parse, store, create metadata, announce, and delete on success', async () => {
      ...
      // EmailMessageService.receiveEmail was called (not sendEmail — avoids relay loop)
      expect(emailService.receiveEmail).toHaveBeenCalledTimes(1);
      const sendArg = emailService.receiveEmail.mock.calls[0][0];
  ...
  describe('authentication verification (Req 6.4, 6.5)', () => {
    ...
    it('should move file to error directory when DMARC fails (Req 6.5)', async () => {
      ...
      expect(blockStore.put).not.toHaveBeenCalled();
      expect(emailService.receiveEmail).not.toHaveBeenCalled();
  ...
  describe('relay-loop prevention', () => {
    /**
     * Core anti-regression test: InboundProcessor must call receiveEmail(),
     * NOT sendEmail(). Calling sendEmail() with relayAllRecipientsExternally=true
     * would route the message back through SES → Postfix → InboundProcessor
     * creating an infinite loop.
     */
    it('calls receiveEmail() and never calls sendEmail()', async () => {
      ...
      await proc.processFile('relay-loop-test.eml');

      // receiveEmail must be called (stores without outbound relay)
      expect(serviceWithBoth.receiveEmail).toHaveBeenCalledTimes(1);
      // sendEmail must NOT be called (would trigger SES → relay loop)
      expect(serviceWithBoth.sendEmail).not.toHaveBeenCalled();
    });

    it('does not invoke any external dispatcher during inbound processing', async () => {
      const externalDispatcher = jest.fn().mockResolvedValue(undefined);

      // Wire a service where the dispatcher is registered
      ...
      const service = {
        receiveEmail: dispatchSpy,
        setExternalDispatcher: jest.fn(),
      } as unknown as EmailMessageService;
      ...
      await proc.processFile('no-dispatch.eml');

      // externalDispatcher was never invoked via InboundProcessor
