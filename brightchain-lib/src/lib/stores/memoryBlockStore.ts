...
export class MemoryBlockStore implements IBlockStore {
   ...
   * @returns Array of parity block checksums (as hex strings converted to ChecksumUint8Array)
  ...
  public async generateParityBlocks(
    ...
    const parityData = await this.fecService.createParityData(block.data, parityCount);
    
    // Store parity data
    this.parityBlocks.set(keyHex, parityData);
    
    // Update metadata with parity block IDs
    const parityBlockIds = parityData.map((_, index) => `${keyHex}.p${index}`);
    if (this.metadataStore.has(keyHex)) {
      await this.metadataStore.update(keyHex, { parityBlockIds });
    }
    
    // Return parity block IDs as ChecksumUint8Array
    // Note: These are synthetic IDs based on the data block ID
