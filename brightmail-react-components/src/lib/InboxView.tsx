...
const InboxView: FC<InboxViewProps> = ({ folder = 'inbox' }) => {
  ...
  const fetchInbox = useCallback(
    ...
    [state.pageSize, emailApi, folder],
  );
  ...
  if (state.emails.length === 0) {
    return (
      // The empty wrapper exposes both `email-list` (for tests that
      // unconditionally locate the list region) and a nested
      // `inbox-empty` marker so empty-state assertions still work.
