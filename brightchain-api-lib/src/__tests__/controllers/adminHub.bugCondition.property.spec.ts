...
import {
  createPostService,
  PostService,
} from '../../lib/services/brighthub/postService';
...
describe('Property 1: Bug Condition — Admin Hub Wrong Collection & Missing getStats', () => {
        ...
        async (page, limit) => {
          ...
          const handler = (
            controller as unknown as {
              handlers: { listPosts: (req: unknown) => Promise<unknown> };
            }
          ).handlers.listPosts;
          await handler({
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (postId) => {
          ...
          const handler = (
            controller as unknown as {
              handlers: { deletePost: (req: unknown) => Promise<unknown> };
            }
          ).handlers.deletePost;
          await handler({
            params: { postId },
  ...
  it('PostService has a getStats method returning { totalPosts, activeUsersLast30Days }', async () => {
    ...
    expect(
      typeof (service as unknown as Record<string, unknown>).getStats,
    ).toBe('function');
