/**
 * @fileoverview Integration tests for the BrightTrust bootstrap redesign.
 *
 * Tests critical end-to-end flows through real service implementations
 * wired together via an in-memory database mock.
 *
 * Covers:
 * 25.1 Bootstrap → Add Members → Transition → BrightTrust Mode
...
interface InMemoryDb extends IBrightTrustDatabase<TID> {
  ...
  _members: Map<TID, IBrightTrustMember<TID>>;
  _documents: Map<string, BrightTrustDataRecord<TID>>;
...
function createInMemoryDb(): InMemoryDb {
  ...
  const members = new Map<TID, IBrightTrustMember<TID>>();
  const documents = new Map<string, BrightTrustDataRecord<TID>>();
  ...
  const db: InMemoryDb = {
    ...
    // Documents
    saveDocument: jest.fn(async (doc: BrightTrustDataRecord<TID>) => {
...
describe('BrightTrust Integration Tests', () => {
  beforeAll(() => {
  ...
  // ==========================================================================
  // 25.1 Bootstrap → Add Members → Transition Ceremony → BrightTrust Mode
  // ==========================================================================
  describe('25.1 Full Lifecycle: Bootstrap → Add Members → Transition → BrightTrust', () => {
    it('should complete the full lifecycle from bootstrap to BrightTrust mode', async () => {
      const db = createInMemoryDb();
      const gossip = createMockGossipService();
      const qsm = new BrightTrustStateMachine<TID>(db, sealingService, gossip);
      ...
      expect(epoch1.mode).toBe(BrightTrustOperationalMode.Bootstrap);
      expect(epoch1.threshold).toBe(1); // effective threshold = member count
      expect(await qsm.getMode()).toBe(BrightTrustOperationalMode.Bootstrap);
      ...
      expect(epoch2.mode).toBe(BrightTrustOperationalMode.Bootstrap);
      expect(epoch2.memberIds).toHaveLength(2);

      // Step 4: Add member 3 → now members >= threshold, mode becomes BrightTrust
      const epoch3 = await qsm.addMember(memberPool[2].member, {
        name: 'Charlie',
      });
      expect(epoch3.epochNumber).toBe(3);
      expect(epoch3.mode).toBe(BrightTrustOperationalMode.BrightTrust);
      expect(epoch3.memberIds).toHaveLength(3);
      expect(epoch3.threshold).toBe(3);

      // Step 5: Verify mode is now BrightTrust
      expect(await qsm.getMode()).toBe(BrightTrustOperationalMode.BrightTrust);
  ...
  describe('25.2 Proposal Submission → Gossip → Vote → Tally → Action', () => {
    it('should submit a proposal, collect votes via gossip, and execute action on approval', async () => {
      ...
      expect(gossip.announceBrightTrustProposal).toHaveBeenCalled();
      ...
      expect(gossip.announceBrightTrustVote).toHaveBeenCalled();
