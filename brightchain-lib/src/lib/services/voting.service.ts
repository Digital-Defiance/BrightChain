...
export class VotingService {
  ...
  public encryptedPrivateKeyToKeyPair(
    ...
    const lambdaLength = decryptedBuffer.readUInt32BE(0);
    const lambdaBuffer = decryptedBuffer.subarray(
      CONSTANTS.UINT32_SIZE,
      CONSTANTS.UINT32_SIZE + lambdaLength,
    );

    // Read mu length and extract mu buffer
    const muLength = decryptedBuffer.readUInt32BE(
      CONSTANTS.UINT32_SIZE + lambdaLength,
    );
    const muBuffer = decryptedBuffer.subarray(
