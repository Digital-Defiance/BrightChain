...
export class BrightDbAuthService<TID extends PlatformID = Buffer> {
  ...
  async login(credentials: IAuthCredentials): Promise<IAuthToken> {
    ...
    try {
      ...
      // Try all ID formats: raw id, hex, dashed GUID, and memberId string.
      // The auth provider (buildRequestUserDTO) uses the raw id as the first
      // lookup — we must match that to avoid format mismatches.
      let userRoleDocs = await userRolesCol
        .find({ userId: reference.id } as never)
        .exec();
      ...
      // RBAC lookup failed — fall through to MemberType-based role derivation
    }

    // If RBAC lookup returned no matching user-role documents (e.g. due
    // to an ID format mismatch between the member store and the user_roles
    // collection, or the catch block swallowed an error), fall back to
    // deriving roles from MemberType so that Admin/System members still
    // receive the 'admin' JWT claim.
    if (!roles || roles.length === 0) {
      if (
        reference.type === MemberType.Admin ||
        reference.type === MemberType.System
      ) {
        roles = ['admin'];
      }
    }
  ...
  /**
   * Sign a short-lived pending TOTP token.
   * Contains only `userId` and `pendingTotp: true` — no roles or privilege claims.
