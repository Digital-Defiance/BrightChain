...
describe('GovernancePayloadSerializer', () => {
  ...
  describe('genesis payload', () => {
    it('round-trips single admin + unanimous quorum', () => {
      const g: IGenesisPayloadData = {
        brightTrustPolicy: { type: QuorumType.Unanimous },
        signers: [
          {
      ...
      expect(d.genesis!.brightTrustPolicy.type).toBe(QuorumType.Unanimous);
      ...
      expect(new Uint8Array(d.genesis!.signers[0].publicKey)).toEqual(
        makePk(0x02),
      );
    ...
    it('round-trips multiple signers, threshold quorum, and metadata', () => {
      const g: IGenesisPayloadData = {
        brightTrustPolicy: { type: QuorumType.Threshold, threshold: 2 },
        signers: [
          {
      ...
      expect(d.genesis).toBeDefined();
      expect(d.genesis!.brightTrustPolicy.type).toBe(QuorumType.Threshold);
      expect(d.genesis!.brightTrustPolicy.threshold).toBe(2);
    ...
    it('round-trips majority quorum', () => {
      const g: IGenesisPayloadData = {
        brightTrustPolicy: { type: QuorumType.Majority },
        signers: [
          {
            publicKey: makePk(0x01),
            role: SignerRole.Admin,
      ...
      const d = serializer.deserialize(serializer.serializeGenesis(g));
      expect(d.genesis!.brightTrustPolicy.type).toBe(QuorumType.Majority);
    });
  });
  ...
  describe('amendment payloads', () => {
    it('round-trips add_signer', () => {
        ...
        actions: [
          {
            ...
            publicKey: makePk(0x05),
            role: SignerRole.Writer,
            metadata: new Map([['name', 'W']]),
          },
        ],
        cosignatures: [],
      };
      ...
      const a = d.actions[0] as Extract<
        IGovernanceAction,
        { type: GovernanceActionType.AddSigner }
      >;
    ...
    it('round-trips remove_signer', () => {
      const p: IGovernancePayload = {
        actions: [
          { type: GovernanceActionType.RemoveSigner, publicKey: makePk(0x06) },
        ],
        cosignatures: [],
      };
      const d = serializer.deserialize(serializer.serialize(p));
        ...
        actions: [
          {
            ...
            newPolicy: { type: QuorumType.Unanimous },
          },
        ],
        cosignatures: [],
      };
      const a = serializer.deserialize(serializer.serialize(p))
        .actions[0] as Extract<
        IGovernanceAction,
        { type: GovernanceActionType.UpdateQuorum }
      >;
    ...
    it('round-trips update_quorum (threshold)', () => {
      const p: IGovernancePayload = {
        actions: [
          {
            type: GovernanceActionType.UpdateQuorum,
            newPolicy: { type: QuorumType.Threshold, threshold: 3 },
          },
        ],
        cosignatures: [],
      };
    ...
    it('round-trips suspend_signer', () => {
      const p: IGovernancePayload = {
        actions: [
          { type: GovernanceActionType.SuspendSigner, publicKey: makePk(0x08) },
        ],
        cosignatures: [],
      };
      const a = serializer.deserialize(serializer.serialize(p))
  ...
  describe('cosignatures', () => {
    it('round-trips multiple cosignatures', () => {
      ...
      expect(new Uint8Array(d.cosignatures[0].signerPublicKey)).toEqual(
        makePk(0x01),
      );
      expect(new Uint8Array(d.cosignatures[0].signature)).toEqual(
        makeSig(0xaa),
      );
      expect(new Uint8Array(d.cosignatures[1].signerPublicKey)).toEqual(
        makePk(0x02),
      );
      expect(new Uint8Array(d.cosignatures[1].signature)).toEqual(
        makeSig(0xbb),
      );
    });
  });
    ...
    it('round-trips mixed actions', () => {
      const p: IGovernancePayload = {
        actions: [
          {
            type: GovernanceActionType.AddSigner,
            publicKey: makePk(0x30),
            role: SignerRole.Writer,
          },
          {
            type: GovernanceActionType.ChangeRole,
            publicKey: makePk(0x31),
            newRole: SignerRole.Admin,
          },
  ...
  describe('error cases', () => {
    ...
    it('rejects truncated amendment', () => {
      expect(() =>
        serializer.deserialize(new Uint8Array([0x01, 0x01, 0x00, 0x02])),
      ).toThrow(LedgerSerializationError);
    });

    it('rejects truncated genesis', () => {
      expect(() =>
        serializer.deserialize(new Uint8Array([0x01, 0x00])),
      ).toThrow(LedgerSerializationError);
    });
    ...
    it('rejects unknown subtype', () => {
      expect(() =>
        serializer.deserialize(
          new Uint8Array([0x01, 0xff, 0x00, 0x00, 0x00, 0x00]),
        ),
      ).toThrow(LedgerSerializationError);
      try {
        serializer.deserialize(
  ...
  describe('isGovernancePayload', () => {
    it('returns true for governance payloads', () => {
      const p: IGovernancePayload = {
        actions: [
          { type: GovernanceActionType.RemoveSigner, publicKey: makePk(0x01) },
        ],
        cosignatures: [],
      };
      expect(
        GovernancePayloadSerializer.isGovernancePayload(
    ...
    it('returns false for empty', () => {
      expect(
        GovernancePayloadSerializer.isGovernancePayload(new Uint8Array(0)),
      ).toBe(false);
    });
