...
export function parseBioContent(content: string, maxLength: number): string {
  // Pre-pipeline: reject image markdown syntax
  const inlineImageRegex = /!\[([^\]]*)\]\(([^)]+)\)/;
  const referenceImageRegex = /!\[([^\]]*)\]\[([^\]]*)\]/;

  if (inlineImageRegex.test(content)) {
    throw new Error(
      'Bio content contains inline image markdown syntax (![alt](url)), which is not allowed in bios.',
    );
  }
  if (referenceImageRegex.test(content)) {
    throw new Error(
      'Bio content contains reference image markdown syntax (![alt][ref]), which is not allowed in bios.',
    );
  }

  // Pre-pipeline: validate length
  if (getCharacterCount(content, true) > maxLength) {
    throw new Error('bio_exceeds_max_length');
  }

  // Phase 1: Strip all embedded HTML
  content = sanitizeHtml(content, {
    allowedTags: [],
    allowedAttributes: {},
  });

  // Phase 2: Parse markdown (bios always use full markdown rendering)
  content = parseMarkdown(content);

  // Phase 3: Parse custom icon markup ({{ }} → FontAwesome <i> elements)
  content = parseIconMarkup(content);
