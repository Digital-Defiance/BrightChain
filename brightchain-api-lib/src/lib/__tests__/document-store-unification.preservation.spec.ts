...
describe('Property 2: Preservation — Registered Models and Service API Signatures', () => {
  ...
  it('wrapCollection bridges collections to BrightHubCollection<T>', async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.record({
          collectionName: fc.constantFrom(
            'brighthub_posts',
            'brighthub_likes',
            'brighthub_follows',
          ),
        ...
        async ({ collectionName }) => {
          ...
          try {
            const rawCollection =
              app.getModel<Record<string, unknown>>(collectionName);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async ({ memberA, memberB, messageContent }) => {
          ...
          const messages = await service.getMessages(conversation.id, memberA);
  ...
  it('IEmailMetadataStore interface contract is satisfied by InMemoryEmailMetadataStore', () => {
    ...
    } = require('@brightchain/brightchain-lib');
