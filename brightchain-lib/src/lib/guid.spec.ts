  ...
  describe('FEC', () => {
    ...
    describe('computeFEC', () => {
      it('should compute FEC for a valid GUID', async () => {
        ...
        const fecData = await guid.computeFEC(fecService);
      ...
      it('should throw an error if the GUID length is invalid', async () => {
        const invalidGuid = Buffer.alloc(15); // Invalid length
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const guid = new GuidV4(invalidGuid as any);
        await expect(guid.computeFEC(fecService)).rejects.toThrow(FecError);
      });

      it('should throw an error if the FEC service fails', async () => {
        const guid = GuidV4.new();
        jest
          .spyOn(fecService, 'encode')
          .mockRejectedValue(new FecError(FecErrorType.FecEncodingFailed));
        await expect(guid.computeFEC(fecService)).rejects.toThrow(FecError);
      });
    });
    ...
    describe('reconstituteFEC', () => {
      it('should reconstitute a GUID from valid FEC data', async () => {
        const guid = GuidV4.new();
        const fecData = await guid.computeFEC(fecService);
        const reconstitutedGuid = await GuidV4.reconstituteFEC(
          fecService,
          fecData,
        );
        expect(reconstitutedGuid).toBeInstanceOf(Buffer);
        expect(reconstitutedGuid.length).toBe(16);
        expect(reconstitutedGuid).toEqual(guid.asRawGuidBuffer);
      });

      it('should throw an error if the FEC data length is invalid', async () => {
        const invalidFecData = Buffer.alloc(10); // Invalid length
        await expect(
          GuidV4.reconstituteFEC(fecService, invalidFecData),
        ).rejects.toThrow(FecError);
      });
      ...
      it('should throw an error if not enough shards are available', async () => {
        ...
        const fecData = await guid.computeFEC(fecService);
        jest
          .spyOn(fecService, 'decode')
          .mockRejectedValue(
            new FecError(FecErrorType.NotEnoughShardsAvailable),
          );
        await expect(
          GuidV4.reconstituteFEC(fecService, fecData),
        ).rejects.toThrow(FecError);
      });
      ...
      it('should throw an error if the FEC service fails', async () => {
        ...
        jest
          .spyOn(fecService, 'decode')
          .mockRejectedValue(new FecError(FecErrorType.FecDecodingFailed));
        await expect(
          GuidV4.reconstituteFEC(fecService, fecData),
        ).rejects.toThrow(FecError);
      });
