...
import { InvalidBackupCodeError, KeyWrappingService } from '@digitaldefiance/node-express-suite';
...
describe('BrightChainBackupCodeService Unit Tests', () => {
  ...
  it('getSystemUser throws before setSystemUser is called', () => {
    ...
    expect(
      service.generateCodes(new Uint8Array(12) as any),
  ...
  it('getCodeCount returns 0 when no backup codes exist', async () => {
    ...
    const { member: sysUser } = Member.newMember(
      ecies, MemberType.System, 'sys', new EmailString('sys@bc.org'),
    );
  ...
  it('useBackupCode throws InvalidBackupCodeError for invalid code', async () => {
    ...
    const { member: testMember } = Member.newMember(
      ecies, MemberType.User, 'testuser', new EmailString('test@test.com'),
    );
    const sp = ServiceProvider.getInstance();
    jest.spyOn(memberStore, 'getMember').mockImplementation(async (id) => {
      const idHex = Buffer.from(sp.idProvider.toBytes(id as Uint8Array)).toString('hex');
      const mHex = Buffer.from(sp.idProvider.toBytes(memberId)).toString('hex');
      if (idHex === mHex) {
        const proxy = Object.create(testMember);
        Object.defineProperty(proxy, 'id', { get: () => memberId, configurable: true });
        Object.defineProperty(proxy, 'idBytes', {
          get: () => sp.idProvider.toBytes(memberId), configurable: true,
        });
        return proxy;
      }
  ...
  it('double consumption of the same code fails on second attempt', async () => {
    ...
    const { member: testMember } = Member.newMember(
      ecies, MemberType.User, 'testuser2', new EmailString('test2@test.com'),
    );
    const sp = ServiceProvider.getInstance();
    jest.spyOn(memberStore, 'getMember').mockImplementation(async (id) => {
      const idHex = Buffer.from(sp.idProvider.toBytes(id as Uint8Array)).toString('hex');
