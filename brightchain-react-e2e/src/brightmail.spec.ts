 ...
 * Uses `.first()` because self-sent emails may appear in both inbox and sent
 * views, producing duplicate rows with the same messageId data-testid.
 */
...
test.describe('BrightMail Compose Flow', () => {
  test('compose, send, and verify email appears in inbox', async ({
    ...
    const composeFab = authenticatedPage.locator(
      '[data-testid="compose-fab"], [data-testid="sidebar-header"] button',
    ).filter({ hasText: /compose/i }).first();
...
test.describe('BrightMail Delete Flow', () => {
  test('delete an email with confirmation dialog', async ({
    ...
    // Wait for the delete API response to complete before checking the UI
    const deleteResponsePromise = authenticatedPage.waitForResponse(
      (resp) => resp.url().includes('/api/emails/') && resp.request().method() === 'DELETE',
      { timeout: 15000 },
    ).catch(() => null); // Don't fail if the URL pattern doesn't match exactly
    ...
    // Wait for the delete operation to complete — success snackbar confirms it.
    // The dialog closes immediately, but the API call is async.
    await expect(
      authenticatedPage.getByRole('alert').filter({ hasText: /deleted/i }),
    ).toBeVisible({ timeout: 15000 });

    // Verify the email is removed from the inbox
    await expect(emailRow).not.toBeVisible({ timeout: 15000 });
  });
});
