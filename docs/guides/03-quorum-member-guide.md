---
title: "Quorum Member/Operator Guide"
parent: "Guides"
nav_order: 3
permalink: /guides/03-quorum-member-guide/
---
# Quorum Member/Operator Guide

This guide is for current and prospective quorum members. It covers what the quorum does, how governance works, the proposal/vote lifecycle, sealed document management, the ban mechanism, and your responsibilities as a quorum member.

## What the Quorum Is

The Quorum is BrightChain's governance layer. It is a group of trusted members (recommended ~24, typically charitable organizations with board oversight) who collectively manage:
...
The quorum uses Shamir's Secret Sharing: sensitive data is split into cryptographic shares distributed to members. Reconstructing the data requires a threshold number of members to contribute their shares. No single member — and no minority — can access sealed data alone.

## How You Get In

Quorum membership is by invitation only. The process:
...
3. An existing quorum member proposes your admission:

```typescript
const proposal = await quorumStateMachine.submitProposal({
  actionType: ProposalActionType.ADD_MEMBER,
  description: 'Admit NodeOperatorX as quorum member',
  actionPayload: {
...
4. Active quorum members vote. The proposal must reach the configured threshold (typically 51–75%).
...
Your node holds shares of sealed documents. If too many quorum members go offline simultaneously, the threshold for document reconstruction may not be reachable. Uptime matters.
...
Your private key is used to decrypt your shares of sealed documents. If compromised, an attacker could contribute your share to an unauthorized reconstruction. If lost, the quorum may need to remove you and redistribute shares — an expensive operation.
...
|-------------|-------------|-----------|----------------|
| `ADD_MEMBER` | Admit a new quorum member | Standard (51–75%) | None |
| `REMOVE_MEMBER` | Remove a quorum member, redistribute shares | Standard | None |
...
| `CHANGE_INNER_QUORUM` | Update the inner quorum subset | Standard | None |
...
The proposal is stored, assigned an ID, and announced to other quorum members via gossip.
...
2. A signed `BanRecord` is created with the quorum's approval signatures
3. The record is persisted to the quorum database
4. A `member_banned` gossip announcement propagates to all nodes
5. If the target is a quorum member, `REMOVE_MEMBER` is also executed (share redistribution)
...
```typescript
const result = await quorumStateMachine.sealDocument(
  agentMember,
  sensitiveData,
...
```typescript
const data = await quorumStateMachine.unsealDocument(
  documentId,
...
4. Encrypted shards are distributed to quorum members
...
Identity can only be reconstructed through majority quorum consensus, typically in response to legal processes (e.g., FISA warrants with documentation). After a configurable statute of limitations, identity recovery shards are permanently deleted, making the real identity permanently unrecoverable.

## Epochs

An epoch is a versioned snapshot of the quorum's membership. Epoch transitions happen when:

- A member is added or removed
- A ban is enacted against a quorum member
...
| Quorum | Members ≥ threshold | Full security, all operations available |
...
```typescript
const metrics = await quorumStateMachine.getMetrics();
