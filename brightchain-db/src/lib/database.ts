...
export interface BrightDbOptions {
  ...
  /**
   * Optional gossip service for peer-to-peer head registry synchronization.
   * When provided, the head registry is wrapped with a HeadRegistryGossipTransport
   * that announces local writes and merges incoming head updates from peers.
   *
  ...
  /**
   * Anti-entropy interval in milliseconds for the gossip transport.
   * When `gossipService` is provided, this controls how often the transport
   * re-announces all known head pointers to help nodes that joined late converge.
   * Defaults to 30 000 ms (30 seconds).  Set to 0 to disable anti-entropy.
   */
...
export class BrightDb {
  ...
  constructor(blockStore: IBlockStore, options?: BrightDbOptions) {
    ...
    if (options?.writeAclConfig) {
      ...
      resolvedRegistry = baseRegistry;
    }

    // Wrap with gossip transport when a gossip service is provided.
    // The transport announces outbound writes to peers and merges inbound
    // head_update announcements from peers via last-writer-wins.
    // @see Requirements 2.1, 7.1
    if (options?.gossipService) {
      const transport = new HeadRegistryGossipTransport(
        resolvedRegistry as unknown as import('@brightchain/brightchain-lib').IHeadRegistry,
        options.gossipService,
      );
      const intervalMs = options.antiEntropyIntervalMs ?? 30_000;
      transport.start(intervalMs);
      this.headRegistry = transport as unknown as ICollectionHeadRegistry;
    } else {
      this.headRegistry = resolvedRegistry;
    }
