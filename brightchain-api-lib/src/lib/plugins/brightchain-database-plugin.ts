...
> extends BrightDbDatabasePlugin<TID> {
  ...
  /**
   * Check whether the database is empty (no RBAC data from init).
   * Used to guard against auto-seeding a production database that was
   * already initialized by brightchain-inituserdb.
   *
   * Checks multiple collections written by initializeWithRbac() —
   * member_index, users, and roles — so that a single collection's
   * head being unreadable doesn't cause a false "empty" result that
   * triggers a destructive re-seed.
   */
  async isDatabaseEmpty(): Promise<boolean> {
    const db = this.brightDb;

    // Check member_index (written by base initialize())
    const memberIndexCol = db.collection<IMemberIndexDocument>('member_index');
    const memberIndexDocs = await memberIndexCol.find({}).limit(1).toArray();
    console.log(
      `[BrightChain] isDatabaseEmpty: member_index query returned ${memberIndexDocs.length} doc(s)`,
    );
    if (memberIndexDocs.length > 0) return false;

    // Check users collection (written by initializeWithRbac())
    const usersCol = db.collection<Record<string, unknown>>('users');
    const usersDocs = await usersCol.find({}).limit(1).toArray();
    console.log(
      `[BrightChain] isDatabaseEmpty: users query returned ${usersDocs.length} doc(s)`,
    );
    if (usersDocs.length > 0) return false;

    // Check roles collection (written by initializeWithRbac())
    const rolesCol = db.collection<Record<string, unknown>>('roles');
    const rolesDocs = await rolesCol.find({}).limit(1).toArray();
    console.log(
      `[BrightChain] isDatabaseEmpty: roles query returned ${rolesDocs.length} doc(s)`,
    );
    if (rolesDocs.length > 0) return false;

    console.log(
      '[BrightChain] isDatabaseEmpty: all collections empty — database is empty',
    );
    return true;
  }
  ...
  async seedWithRbac(
    ...
    try {
      ...
      const initResult = await service.initializeWithRbac(
        ...
        this._brightDb ?? undefined,
      );

      // The service uses the provided BrightDb when available, or creates
      // its own internally. Update our reference so plugin.brightDb always
      // points to the db that actually holds the seeded data.
