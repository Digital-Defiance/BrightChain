 ...
 */
export interface BrightTrustMemberMetadata {
  name: string;
  email?: string;
  role?: string;
}
...
/**
 * A BrightTrust member with their public key and metadata
 * @template TID - Platform ID type (string, Uint8Array, or Buffer-based types)
 */
export interface IBrightTrustMember<TID extends PlatformID = GuidV4Uint8Array> {
  id: TID;
...
/**
 * Information about a sealed BrightTrust document
 * @template TID - Platform ID type (string, Uint8Array, or Buffer-based types)
 */
export interface BrightTrustDocumentInfo<
  TID extends PlatformID = GuidV4Uint8Array,
> {
  id: TID;
  memberIds: TID[];
...
/**
 * Interface for BrightTrust service operations.
 ...
 */
export interface IBrightTrustService<
  TID extends PlatformID = GuidV4Uint8Array,
> {
  // === Member Management ===
   ...
   * @returns The created BrightTrust member record
   */
  addMember(
    member: Member<TID>,
    metadata: BrightTrustMemberMetadata,
   ...
   * @returns Array of all active BrightTrust members
   */
  listMembers(): Promise<IBrightTrustMember<TID>[]>;
   ...
   */
  getDocument(documentId: TID): Promise<BrightTrustDocumentInfo<TID> | null>;
   ...
   */
  listDocuments(memberId?: TID): Promise<BrightTrustDocumentInfo<TID>[]>;
