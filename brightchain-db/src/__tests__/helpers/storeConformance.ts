...
export interface StoreTestFactory {
  ...
  createStore(): Promise<{
    store:
      | PooledMemoryBlockStore
      | import('@brightchain/brightchain-lib').IBlockStore;
...
export function runPersistenceConformance(factory: StoreTestFactory): void {
  ...
  describe('insertOne + findOne + findById', () => {
    it('should persist a doc and find by field after restart', async () => {
      ...
      await db1
        .collection('t_ins')
        .insertOne({ _id: 'i1', name: 'alice', age: 30 } as never);
      const db2 = await restart();
      const found = await db2
        .collection('t_ins')
        .findOne({ name: 'alice' } as never);
  ...
  describe('find with sort/skip/limit', () => {
    it('should support cursor ops on persisted data', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        await db1
          .collection('t_cur')
          .insertOne({ _id: `c${i}`, seq: i } as never);
      }
      const db2 = await restart();
  ...
  describe('update operators', () => {
    it('$set', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'us', a: 1, b: 2 } as never);
      await db1
        .collection('t_upd')
        .updateOne({ _id: 'us' } as never, { $set: { a: 99 } } as never);
      const db2 = await restart();
    ...
    it('$inc', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'ui', counter: 5 } as never);
      await db1
        .collection('t_upd')
        .updateOne({ _id: 'ui' } as never, { $inc: { counter: 3 } } as never);
      const db2 = await restart();
      expect(
        ((await db2.collection('t_upd').findById('ui')) as any).counter,
      ).toBe(8);
    });
    ...
    it('$mul', async () => {
      ...
      await db1
        .collection('t_upd')
        .updateOne({ _id: 'um' } as never, { $mul: { val: 3 } } as never);
      const db2 = await restart();
      expect(((await db2.collection('t_upd').findById('um')) as any).val).toBe(
        12,
      );
    });
    ...
    it('$min / $max', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'umm', lo: 10, hi: 10 } as never);
      await db1
        .collection('t_upd')
        .updateOne(
          { _id: 'umm' } as never,
          { $min: { lo: 3 }, $max: { hi: 20 } } as never,
        );
      const db2 = await restart();
    ...
    it('$unset', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'uu', keep: 1, remove: 2 } as never);
      await db1
        .collection('t_upd')
        .updateOne({ _id: 'uu' } as never, { $unset: { remove: '' } } as never);
      const db2 = await restart();
    ...
    it('$push', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'up', tags: ['a'] } as never);
      await db1
        .collection('t_upd')
        .updateOne({ _id: 'up' } as never, { $push: { tags: 'b' } } as never);
      const db2 = await restart();
      expect(
        ((await db2.collection('t_upd').findById('up')) as any).tags,
      ).toEqual(['a', 'b']);
    });
    ...
    it('$pop', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'upo', arr: [1, 2, 3] } as never);
      await db1
        .collection('t_upd')
        .updateOne({ _id: 'upo' } as never, { $pop: { arr: 1 } } as never);
      const db2 = await restart();
      expect(
        ((await db2.collection('t_upd').findById('upo')) as any).arr,
      ).toEqual([1, 2]);
    });
    ...
    it('$pull', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'upl', arr: [1, 2, 3, 2] } as never);
      await db1
        .collection('t_upd')
        .updateOne({ _id: 'upl' } as never, { $pull: { arr: 2 } } as never);
      const db2 = await restart();
      expect(
        ((await db2.collection('t_upd').findById('upl')) as any).arr,
      ).toEqual([1, 3]);
    });
    ...
    it('$addToSet', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'uas', tags: ['a', 'b'] } as never);
      await db1
        .collection('t_upd')
        .updateOne(
          { _id: 'uas' } as never,
          { $addToSet: { tags: 'b' } } as never,
        );
      await db1
        .collection('t_upd')
        .updateOne(
          { _id: 'uas' } as never,
          { $addToSet: { tags: 'c' } } as never,
        );
      const db2 = await restart();
      expect(
        ((await db2.collection('t_upd').findById('uas')) as any).tags,
      ).toEqual(['a', 'b', 'c']);
    });
    ...
    it('$rename', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'urn', oldName: 'val' } as never);
      await db1
        .collection('t_upd')
        .updateOne(
          { _id: 'urn' } as never,
          { $rename: { oldName: 'newName' } } as never,
        );
      const db2 = await restart();
    ...
    it('$currentDate', async () => {
      ...
      await db1
        ...
        .updateOne(
          { _id: 'ucd' } as never,
          { $currentDate: { updatedAt: true } } as never,
        );
      const db2 = await restart();
    ...
    it('replacement (non-operator update)', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'urep', old: true, extra: 'x' } as never);
      await db1
        .collection('t_upd')
        .updateOne(
          { _id: 'urep' } as never,
          { _id: 'urep', replaced: true } as never,
        );
      const db2 = await restart();
    ...
    it('nested dot-path $set', async () => {
      ...
      await db1
        .collection('t_upd')
        .insertOne({ _id: 'udot', meta: { score: 1 } } as never);
      await db1
        .collection('t_upd')
        .updateOne(
          { _id: 'udot' } as never,
          { $set: { 'meta.score': 42 } } as never,
        );
      const db2 = await restart();
      expect(
        ((await db2.collection('t_upd').findById('udot')) as any).meta.score,
      ).toBe(42);
    });
  });
  ...
  describe('replaceOne', () => {
    it('should persist full replacement', async () => {
      ...
      await db1
        .collection('t_rep')
        .insertOne({ _id: 'r1', old: true } as never);
      await db1
        .collection('t_rep')
        .replaceOne({ _id: 'r1' } as never, { _id: 'r1', new: true } as never);
      const db2 = await restart();
  ...
  describe('query operators on persisted data', () => {
    ...
    it('$in / $nin', async () => {
      ...
      const inR = await db2
        .collection(COL)
        .find({ name: { $in: ['alice', 'bob'] } } as never)
        .toArray();
      expect(inR).toHaveLength(2);
      const ninR = await db2
        .collection(COL)
        .find({ name: { $nin: ['alice', 'bob'] } } as never)
        .toArray();
    ...
    it('$exists', async () => {
      ...
      const has = await db2
        .collection(COL)
        .find({ extra: { $exists: true } } as never)
        .toArray();
      expect(has).toHaveLength(1);
      const not = await db2
        .collection(COL)
        .find({ extra: { $exists: false } } as never)
        .toArray();
    ...
    it('nested dot-path query', async () => {
      ...
      const r = await db2
        .collection(COL)
        .find({ 'nested.score': { $gte: 20 } } as never)
        .toArray();
    ...
    it('$all', async () => {
      ...
      const r = await db2
        .collection(COL)
        .find({ tags: { $all: ['admin', 'user'] } } as never)
        .toArray();
  ...
  describe('aggregation on persisted data', () => {
    ...
    it('$sort + $limit + $skip', async () => {
      ...
      const r = await db2
        .collection(COL)
        .aggregate([{ $sort: { salary: -1 } }, { $skip: 1 }, { $limit: 2 }]);
      expect(r).toHaveLength(2);
    ...
    it('$group with $sum', async () => {
      ...
      const r = await db2
        .collection(COL)
        .aggregate([{ $group: { _id: '$dept', total: { $sum: '$salary' } } }]);
      expect(r).toHaveLength(2);
    ...
    it('$count', async () => {
      ...
      const r = await db2
        .collection(COL)
        .aggregate([{ $match: { dept: 'eng' } }, { $count: 'total' }]);
      expect(r).toHaveLength(1);
    ...
    it('$addFields', async () => {
      ...
      const r = await db2
        .collection(COL)
        .aggregate([{ $match: { _id: 'a1' } }, { $addFields: { bonus: 50 } }]);
  ...
  describe('text search on persisted data', () => {
    it('text search works on reloaded data', async () => {
      ...
      await c.insertMany([
        {
          _id: 'tx1',
          title: 'BrightChain Database',
          body: 'A block-based database engine',
        },
        { _id: 'tx2', title: 'Recipe Book', body: 'How to cook pasta' },
        {
          _id: 'tx3',
          title: 'Database Internals',
          body: 'B-trees and LSM trees',
        },
      ] as never);
      ...
      const r = await c2
        .find({ $text: { $search: 'database' } } as never)
        .toArray();
  ...
  describe('bulkWrite', () => {
    it('mixed bulk ops persist', async () => {
      ...
      await c.bulkWrite([
        ...
        {
          updateOne: {
            filter: { _id: 'bk1' } as never,
            update: { $set: { val: 99 } } as never,
          },
        },
      ...
      const db2 = await restart();
      expect(
        ((await db2.collection('t_bulk').findById('bk1')) as any).val,
      ).toBe(99);
  ...
  describe('multiple collections', () => {
    it('independent persistence, no cross-contamination', async () => {
      ...
      const db2 = await restart();
      expect(((await db2.collection('t_ma').findById('a1')) as any).src).toBe(
        'a',
      );
      expect(((await db2.collection('t_mb').findById('b1')) as any).src).toBe(
        'b',
      );
  ...
  describe('drop', () => {
    it('collection drop persists', async () => {
      ...
      await db1
        .collection('t_drop')
        .insertMany([{ _id: 'x1' }, { _id: 'x2' }] as never);
  ...
  describe('registration → restart → login', () => {
    it('should find registered user by username after restart', async () => {
      ...
      await db1.collection('member_index').insertOne({
        _id: memberId,
        id: memberId,
        publicCBL: '00'.repeat(64),
        privateCBL: '00'.repeat(64),
        type: 1,
        status: 'Active',
        poolId: 'BrightChain',
      } as never);
      ...
      const db2 = await restart();
      const userDoc = await db2
        .collection('users')
        .findOne({ username: 'jessica' } as never);
      expect(userDoc).not.toBeNull();
      const idHex = ((userDoc as any)._id as string).replace(/-/g, '');
      const indexDoc = await db2
        .collection('member_index')
        .findOne({ id: idHex } as never);
