...
describe('StaticHelpersTuple', () => {
  ...
  describe('xorSourceToPrimeWhitened', () => {
    it('should XOR source block with whitening and random blocks', async () => {
      // Create test blocks
      const maxDataSize = blockSize - StaticHelpersECIES.eciesOverheadLength;
      ...
      const sourceBlock = await OwnedDataBlock.from(
        ...
        blockSize,
        paddedData,
        StaticHelpersChecksum.calculateChecksum(paddedData),
        creator,
      );
      ...
      const result = await StaticHelpersTuple.xorSourceToPrimeWhitened(
        sourceBlock,
        whiteners,
        randomBlocks,
      );
    ...
    it('should throw error if block sizes do not match', async () => {
      const maxDataSize =
        BlockSize.Small - StaticHelpersECIES.eciesOverheadLength;
      const sourceData = randomBytes(maxDataSize);
      const sourceBlock = await OwnedDataBlock.from(
        BlockType.OwnedDataBlock,
        BlockDataType.RawData,
        BlockSize.Small,
        sourceData,
        StaticHelpersChecksum.calculateChecksum(sourceData),
        creator,
      );
      ...
      await expect(
        StaticHelpersTuple.xorSourceToPrimeWhitened(
          sourceBlock,
          whiteners,
          randomBlocks,
        ),
  ...
  describe('xorDestPrimeWhitenedToOwned', () => {
    it('should recover original data from whitened blocks', async () => {
      ...
      const primeWhitened = await StaticHelpersTuple.xorSourceToPrimeWhitened(
        originalBlock,
        whiteners,
        randomBlocks,
      );
      ...
      const recoveredBlock =
        await StaticHelpersTuple.xorDestPrimeWhitenedToOwned(
          creator,
          primeWhitened,
          whiteners,
          randomBlocks,
