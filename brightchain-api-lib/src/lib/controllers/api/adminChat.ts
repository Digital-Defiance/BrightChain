  ...
  private async handleListMessages(
    ...
    try {
      ...
      if (brightDb) {
        ...
        const blockContentStore = this.application.services.has(
          'blockContentStore',
        )
          ? (this.application.services.get(
              'blockContentStore',
            ) as IBlockContentStore)
          : undefined;
        ...
        const messages = await Promise.all(
          docs.map(async (doc: Record<string, unknown>) => {
            ...
            if (blockContentStore && storedContent) {
              try {
                const contentBytes =
                  await blockContentStore.retrieveContent(storedContent);
                if (contentBytes) {
