/**
 * Property-Based Tests for BrightTrustStateMachine
 *
...
/**
 * Creates a mock IBrightTrustDatabase that stores data in memory.
...
describe('BrightTrustStateMachine Property-Based Tests', () => {
   ...
   */
  function createMemberTrackingMockDatabase(): IBrightTrustDatabase<GuidV4Uint8Array> & {
    savedEpochs: BrightTrustEpoch<GuidV4Uint8Array>[];
    savedOperationalStates: OperationalState[];
  } {
    const epochs = new Map<number, BrightTrustEpoch<GuidV4Uint8Array>>();
    const savedEpochs: BrightTrustEpoch<GuidV4Uint8Array>[] = [];
    const savedOperationalStates: OperationalState[] = [];
    const members = new Map<HexString, IBrightTrustMember<GuidV4Uint8Array>>();
    let operationalState: OperationalState | null = null;
  ...
  describe('P1: Bootstrap Mode Threshold Invariant', () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (memberCount, rawThreshold) => {
            ...
            const sm = new BrightTrustStateMachine<GuidV4Uint8Array>(
              mockDb,
              sealingService,
              mockGossip,
            );
            ...
            expect(mode).toBe(BrightTrustOperationalMode.Bootstrap);
            ...
            expect(epoch.mode).toBe(BrightTrustOperationalMode.Bootstrap);
            ...
            expect(lastState.mode).toBe(BrightTrustOperationalMode.Bootstrap);
          },
        ),
        { numRuns: 20 },
      );
    ...
    it('should enter BrightTrust mode when members >= threshold', async () => {
      await fc.assert(
        fc.asyncProperty(
          // Member count: 2 to 6
          ...
          fc.integer({ min: 2, max: 6 }),
          async (memberCount, rawThreshold) => {
            // Ensure threshold <= memberCount to trigger BrightTrust mode
            ...
            // Should be in BrightTrust mode
            const mode = await sm.getMode();
            expect(mode).toBe(BrightTrustOperationalMode.BrightTrust);

            // Threshold should be the configured threshold
            expect(epoch.threshold).toBe(threshold);

            // Epoch records the correct mode
            expect(epoch.mode).toBe(BrightTrustOperationalMode.BrightTrust);
          },
        ),
        { numRuns: 20 },
      );
    ...
    it('should produce strictly increasing epoch numbers across a sequence of addMember operations', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          async (addCount) => {
            // Start with 2 members, threshold 2 (BrightTrust mode)
   ...
   * For any brightTrust with N active members and threshold T,
   ...
   * under Bootstrap_Mode have shares for the full BrightTrust member set with the
  ...
  describe('P14: Transition Ceremony Atomicity', () => {
     ...
     */
    function createDocumentTrackingMockDatabase(): IBrightTrustDatabase<GuidV4Uint8Array> & {
      savedEpochs: BrightTrustEpoch<GuidV4Uint8Array>[];
      savedOperationalStates: OperationalState[];
      documents: Map<string, BrightTrustDataRecord<GuidV4Uint8Array>>;
      ...
      const documents = new Map<
        string,
        BrightTrustDataRecord<GuidV4Uint8Array>
      >();
      const journalEntries = new Map<number, RedistributionJournalEntry[]>();
      let operationalState: OperationalState | null = null;
      let saveDocCallCount = 0;

      const db: IBrightTrustDatabase<GuidV4Uint8Array> & {
        savedEpochs: BrightTrustEpoch<GuidV4Uint8Array>[];
        savedOperationalStates: OperationalState[];
        documents: Map<string, BrightTrustDataRecord<GuidV4Uint8Array>>;
        journalEntries: Map<number, RedistributionJournalEntry[]>;
        ...
        saveEpoch: jest.fn(
          async (epoch: BrightTrustEpoch<GuidV4Uint8Array>) => {
            epochs.set(epoch.epochNumber, epoch);
            savedEpochs.push(epoch);
          },
        ),
        ...
        saveDocument: jest.fn(
          async (doc: BrightTrustDataRecord<GuidV4Uint8Array>) => {
            saveDocCallCount++;
        ...
        fc.asyncProperty(
          ...
          async (docCount) => {
            ...
            for (let i = 0; i < docCount; i++) {
              const sealed = await sealingService.brightTrustSealBootstrap(
                bootstrapMembers[0],
                { data: `doc-${i}`, index: i },
                bootstrapMembers,
              ...
              const docWithEpoch = new BrightTrustDataRecord<GuidV4Uint8Array>(
                sealed.creator,
                sealed.memberIDs,
            ...
            const fakeActiveMembers: IBrightTrustMember<GuidV4Uint8Array>[] =
              allMembers.map((m) => ({
                id: m.id,
            ...
            try {
              ...
              // SUCCESS: verify mode is now BrightTrust
              const mode = await sm.getMode();
              expect(mode).toBe(BrightTrustOperationalMode.BrightTrust);
   ...
   * BrightTrustError.TransitionInProgress.
   */
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (operationIndex) => {
            ...
            // via addMember (which would change mode to BrightTrust)
            const fakeActiveMembers: IBrightTrustMember<GuidV4Uint8Array>[] =
              allMembers.map((m) => ({
                id: m.id,
                publicKey: m.publicKey,
            ...
            (mockDb.listDocumentsByEpoch as jest.Mock).mockImplementation(
              async () => {
                try {
                  ...
                  if (
                    error instanceof BrightTrustError &&
                    error.type === BrightTrustErrorType.TransitionInProgress
                  ) {
   ...
   */
  function createVotingMockDatabase(): IBrightTrustDatabase<GuidV4Uint8Array> & {
    savedEpochs: BrightTrustEpoch<GuidV4Uint8Array>[];
    savedOperationalStates: OperationalState[];
          ...
          async (duplicateCount) => {
            ...
            for (let i = 1; i < duplicateCount; i++) {
              try {
                ...
                throw new Error('Expected DuplicateVote error');
              } catch (error) {
                expect(error).toBeInstanceOf(BrightTrustError);
                expect((error as BrightTrustError).type).toBe(
        ...
        fc.asyncProperty(
          ...
          async (description, actionPayload) => {
            ...
            try {
              ...
              throw new Error('Expected MissingAttachment error');
            } catch (error) {
              expect(error).toBeInstanceOf(BrightTrustError);
              expect((error as BrightTrustError).type).toBe(
