import { KeyPair, PublicKey, PrivateKey, generateRandomKeysSync } from 'paillier-bigint';
...
export abstract class StaticHelpersVoting {
  ...
  public static keyPairToEncryptedPrivateKey(keyPair: KeyPair, walletPublicKey: Buffer): Buffer {
    ...
    const encryptedPrivateKey = StaticHelpersECIES.encrypt(walletPublicKey, Buffer.concat([lambdaLengthBuffer, lambda, mu]));
    return encryptedPrivateKey;
  }
  public static encryptedPrivateKeyToKeyPair(encryptedPrivateKey: Buffer, walletPrivateKey: Buffer, votingPublicKey: PublicKey): PrivateKey {
    const decryptedPrivateKeyBuffer = StaticHelpersECIES.decrypt(walletPrivateKey, encryptedPrivateKey);
    const lambdaLength = decryptedPrivateKeyBuffer.readUInt32BE(0);
    const lambdaBuffer = decryptedPrivateKeyBuffer.subarray(4, 4 + lambdaLength);
    ...
    const privateKey = new PrivateKey(lambda, mu, votingPublicKey);
    return privateKey;
  }
