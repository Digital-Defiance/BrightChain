...
describe('Property 16: Environment missing variables validation', () => {
  ...
  describe('Azure — incomplete env var subsets should throw', () => {
    ...
    it('should throw listing missing vars for any incomplete Azure subset', () => {
      ...
      const azureVarArb = fc.record(
        {
          AZURE_STORAGE_CONNECTION_STRING: fc.option(fc.constant('DefaultEndpointsProtocol=https;AccountName=t;AccountKey=dA==;EndpointSuffix=core.windows.net'), { nil: undefined }),
          AZURE_STORAGE_ACCOUNT_NAME: fc.option(fc.constant('teststorage'), { nil: undefined }),
          AZURE_STORAGE_ACCOUNT_KEY: fc.option(fc.constant('dGVzdGtleQ=='), { nil: undefined }),
          AZURE_STORAGE_CONTAINER_NAME: fc.option(fc.constant('testcontainer'), { nil: undefined }),
        },
        { requiredKeys: [] },
      );
      ...
      fc.assert(
        fc.property(azureVarArb, (vars) => {
          ...
          const hasAuth = vars.AZURE_STORAGE_CONNECTION_STRING !== undefined ||
    ...
    it('should succeed with complete Azure connection string auth', () => {
      process.env['BRIGHTCHAIN_BLOCKSTORE_TYPE'] = BlockStoreType.AzureBlob;
      for (const [k, v] of Object.entries(azureRequiredVarSets.connectionStringAuth)) {
        process.env[k] = v;
      }
    ...
    it('should succeed with complete Azure account name auth', () => {
      process.env['BRIGHTCHAIN_BLOCKSTORE_TYPE'] = BlockStoreType.AzureBlob;
      for (const [k, v] of Object.entries(azureRequiredVarSets.accountNameAuth)) {
        process.env[k] = v;
      }
      expect(() => new Environment(undefined, true)).not.toThrow();
    });
  ...
  describe('S3 — incomplete env var subsets should throw', () => {
    ...
    it('should throw listing missing vars when AWS_S3_BUCKET_NAME is absent', () => {
      ...
      const s3OptionalArb = fc.record(
        {
          AWS_S3_KEY_PREFIX: fc.option(fc.string({ minLength: 1, maxLength: 20 }), { nil: undefined }),
          AWS_REGION: fc.option(fc.constantFrom('us-east-1', 'eu-west-1', 'ap-southeast-1'), { nil: undefined }),
          AWS_ACCESS_KEY_ID: fc.option(fc.constant('AKIAIOSFODNN7EXAMPLE'), { nil: undefined }),
          AWS_SECRET_ACCESS_KEY: fc.option(fc.constant('wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY'), { nil: undefined }),
        },
        { requiredKeys: [] },
      );
  ...
  describe('Invalid BlockStoreType should throw', () => {
    it('should throw for any invalid store type string', () => {
      const invalidTypeArb = fc.string({ minLength: 1, maxLength: 20 }).filter(
