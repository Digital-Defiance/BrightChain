...
> {
  ...
  private getServerService(): ServerService | null {
  ...
  private async handleCreateChannel(req: unknown): Promise<{
    ...
    try {
      const { name, topic, visibility, serverId, categoryId } = (req as CreateChannelBody).body;
      if (!name || !visibility) {
      ...
      if (serverId && serverService) {
        // Create channel within a server (handles category assignment)
        channel = await serverService.createChannelInServer(
          serverId,
          memberId,
        ...
        channel = await this.getChannelService().createChannel(
          name,
          memberId,
          visibility,
          topic,
  ...
  private mapServiceError(error: unknown): {
    ...
    if (error instanceof ServerNotFoundError) {
      return notFoundError('Server', error.message);
    }
