    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (creatorId, otherIds, name) => {
          ...
          const currentEpochMap =
            updatedGroup.encryptedSharedKey.get(currentEpoch)!;
          ...
          expect(currentEpochMap.size).toBe(remainingIds.length);
          for (const id of remainingIds) {
            expect(currentEpochMap.has(id)).toBe(true);
            const decrypted = extractKeyFromDefault(currentEpochMap.get(id)!);
