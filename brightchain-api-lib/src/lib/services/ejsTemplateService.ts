...
export class EjsTemplateService {
  ...
  buildContext(
    ...
    const { headAssets, bodyScripts } = this.extractBundleTags(
      locals.cspNonce ?? '',
    );
   ...
   */
  private extractBundleTags(cspNonce: string): {
    headAssets: string;
    bodyScripts: string;
  } {
    ...
    try {
      ...
      const scriptTags =
        html.match(/<script\s+[^>]*type=["']module["'][^>]*><\/script>/gi) ||
        [];

      // Also extract <link rel="modulepreload" ...> tags (Vite preload hints)
      const preloadTags =
        html.match(/<link\s+[^>]*rel=["']modulepreload["'][^>]*>/gi) || [];
