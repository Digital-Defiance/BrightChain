...
export class BlockService {
  ...
  public static async processFileInChunks(
    ...
    const createBlock = async (dataSlice: Buffer): Promise<void> => {
      ...
      if (encrypt && recipient) {
        const encryptedData = await eciesService.encrypt(
          recipient.publicKey,
          dataSlice,
        );
  ...
  public static async createCBL(
    ...
    const totalSize = checksumService.checksumBufferLength * blocks.length;
    ...
    const metadata = new CblBlockMetadata(
      BlockSize.Message, // Use smallest block size instead of Huge
