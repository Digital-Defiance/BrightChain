...
const AdminBlockExplorerPanel: FC = () => {
  ...
  const [networkResult, setNetworkResult] = useState<NetworkSearchResult | null>(null);

  // UI state
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [detailBlock, setDetailBlock] = useState<BlockEntry | null>(null);
  const [discoverResult, setDiscoverResult] = useState<Array<{ nodeId: string; latencyMs?: number }> | null>(null);
  ...
  const fetchBlocks = useCallback(async (cursor?: string) => {
    ...
    try {
      ...
      const res = await authenticatedApi.get(`/admin/blocks?${params.toString()}`);
      const data = res.data.data ?? res.data;
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const res = await authenticatedApi.get(`/admin/blocks/search?${params.toString()}`);
      const data = res.data.data ?? res.data;
  ...
  const handleNetworkSearch = async () => {
    ...
    try {
      ...
      const res = await authenticatedApi.post('/admin/blocks/network-search', body);
  ...
  const handleDiscover = async (blockId: string) => {
    ...
    try {
      const res = await authenticatedApi.post(`/admin/blocks/${blockId}/discover`);
      const data = res.data.data ?? res.data;
    ...
    <Box>
      ...
      <Box display="flex" alignItems="center" gap={1} mb={1}>
        <TextField
          ...
          value={searchId}
          onChange={(e) => { setSearchId(e.target.value); if (!e.target.value) setSearchResults(null); }}
      ...
      <Box display="flex" alignItems="center" gap={1} mb={2}>
        <TextField
          ...
          sx={{ minWidth: 300 }}
        />
        <IconButton onClick={handleNetworkSearch} title="Search across network via gossip">
      ...
      {searchResults && (
        <Box mb={1}>
          <Chip
            ...
            onDelete={() => { setSearchResults(null); setSearchId(''); }}
          />
        </Box>
      )}
        ...
        <Box mb={2} p={2} border={1} borderColor="divider" borderRadius={1}>
          <Typography variant="subtitle2" fontWeight="bold" mb={1}>
            ...
            {networkResult.warning && (
              <Chip size="small" label={networkResult.warning} color="warning" sx={{ ml: 1 }} />
            )}
          </Typography>
          <Typography variant="body2" color="text.secondary" mb={1}>
            Queried {networkResult.queriedPeers ?? 0} peers in {networkResult.duration ?? 0}ms
          </Typography>
          {networkResult.blockId && (
            <Box>
              <Typography variant="body2">
                Block {networkResult.blockId}: {networkResult.found ? 'Found' : 'Not found'}
              </Typography>
              {networkResult.locations?.map((loc, i) => (
                <Typography key={i} variant="body2" sx={{ pl: 2 }}>
                  Node: {loc.nodeId} {loc.latencyMs != null && `(${loc.latencyMs}ms)`}
                </Typography>
              ))}
            </Box>
          )}
          ...
          {!networkResult.blockId && (!networkResult.hits || networkResult.hits.length === 0) && (
            <Typography variant="body2">No results found on the network.</Typography>
          )}
          <Button size="small" onClick={() => setNetworkResult(null)} sx={{ mt: 1 }}>
          ...
          {detailBlock.metadata && (
            <>
              {detailBlock.metadata.size != null && (
                <Typography variant="body2">Size: {detailBlock.metadata.size} bytes</Typography>
              )}
              {detailBlock.metadata.createdAt && (
                <Typography variant="body2">
                  Created: {new Date(detailBlock.metadata.createdAt).toLocaleString()}
                </Typography>
              )}
              {detailBlock.metadata.durabilityLevel && (
                <Typography variant="body2">Durability: {detailBlock.metadata.durabilityLevel}</Typography>
              )}
              {detailBlock.metadata.checksum && (
                <Typography variant="body2">Checksum: {detailBlock.metadata.checksum}</Typography>
              )}
            </>
          )}
          <Box display="flex" gap={1} mt={1}>
            <Button size="small" startIcon={<FontAwesomeIcon icon={faRadar} />}
              onClick={() => handleDiscover(detailBlock.hash)}>
              {t(BrightChainStrings.Admin_Blocks_DiscoverNodes)}
            </Button>
            <Button size="small" color="error" startIcon={<FontAwesomeIcon icon={faTrash} />}
              onClick={() => { setDeleteBlockId(detailBlock.hash); setDeleteOpen(true); }}>
      ...
      <Table size="small">
        <TableHead>
          <TableRow>
            ...
            <TableCell>{t(BrightChainStrings.Admin_Blocks_ColActions)}</TableCell>
          </TableRow>
        </TableHead>
        <TableBody>
          {loading && displayBlocks.length === 0 ? (
            ...
            displayBlocks.map((block) => (
              ...
              >
                ...
                <TableCell>
                  <Tooltip title={t(BrightChainStrings.Admin_Blocks_DiscoverNodes)}>
                    <IconButton size="small" onClick={(e) => { e.stopPropagation(); handleDiscover(block.hash); }}>
      ...
      {!searchResults && (
        <Box display="flex" justifyContent="center" alignItems="center" gap={2} mt={2}>
          <Button size="small" disabled={cursorStack.length <= 1} onClick={handlePrevPage}>
      ...
      <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)}>
        <DialogTitle>{t(BrightChainStrings.Admin_Blocks_DeleteTitle)}</DialogTitle>
        <DialogContent>
          <DialogContentText>
