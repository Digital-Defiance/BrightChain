...
export interface AuthResult {
  ...
  email: string;
  password: string;
  /** Server-generated mnemonic. Present when the user didn't provide their own. */
  mnemonic?: string;
}
...
/**
 * Load the shared auth token created by global-setup.
 * Falls back to per-test registration if the file is missing.
 */
function loadSharedAuth(): SharedAuth | null {
  try {
    if (fs.existsSync(SHARED_AUTH_PATH)) {
      return JSON.parse(fs.readFileSync(SHARED_AUTH_PATH, 'utf-8'));
    }
  } catch {
    // ignore
  }
 ...
 *
 * `authenticatedPage` uses the shared auth token from global-setup to avoid
 * registering a new user per test. This prevents the API server from being
 * overwhelmed by concurrent crypto-heavy registration operations.
 *
 * Tests that need a unique user can call `registerViaApi()` directly.
 */
export const test = base.extend<{
  ...
  authResult: async ({ baseURL }, use) => {
    // Prefer the shared auth from global-setup
    const shared = loadSharedAuth();
    if (shared) {
      await use(shared);
      return;
    }
    // Fallback: register a fresh user (global-setup may not have run)
