...
describe('RecipientLookupService TCP server', () => {
  ...
  /** Encode a payload as a netstring: `<length>:<payload>,` */
  function toNetstring(payload: string): string {
    const bytes = Buffer.byteLength(payload, 'utf-8');
    return `${bytes}:${payload},`;
  }

  /** Parse a single netstring response and return the payload. */
  function parseNetstring(data: string): string {
    const colonIdx = data.indexOf(':');
    if (colonIdx === -1) throw new Error(`No colon in netstring: ${data}`);
    const length = parseInt(data.slice(0, colonIdx), 10);
    const payload = data.slice(colonIdx + 1, colonIdx + 1 + length);
    return payload;
  }

  /** Helper: send a socketmap request (as a netstring) and read the netstring response. */
  function sendRequest(port: number, payload: string): Promise<string> {
    return new Promise((resolve, reject) => {
      const client = net.createConnection({ port, host: '127.0.0.1' }, () => {
        client.write(toNetstring(payload));
      });
      let data = '';
      client.setEncoding('utf-8');
      client.on('data', (chunk) => {
        data += chunk;
        // A complete netstring ends with ','
        if (data.endsWith(',')) {
          client.end();
        }
      });
  ...
  it('handles multiple requests on the same connection', async () => {
    ...
    const responses = await new Promise<string[]>((resolve, reject) => {
      const client = net.createConnection({ port, host: '127.0.0.1' }, () => {
        client.write(toNetstring('virtual alice@brightchain.org'));
        client.write(toNetstring('virtual nobody@brightchain.org'));
      });
      let data = '';
      ...
      client.on('data', (chunk) => {
        data += chunk;
        // Parse all complete netstrings from the accumulated data.
        while (true) {
          const colonIdx = data.indexOf(':');
          if (colonIdx === -1) break;
          const length = parseInt(data.slice(0, colonIdx), 10);
