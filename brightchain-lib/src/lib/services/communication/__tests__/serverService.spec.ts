...
describe('ServerService — unit tests', () => {
  ...
  describe('getServer', () => {
    it('returns the server when it exists', async () => {
      const { serverService } = createServices();
      const created = await serverService.createServer('owner-1', {
        name: 'Test',
      });
    ...
    it('throws ServerNotFoundError for a non-existent ID', async () => {
      const { serverService } = createServices();
      await expect(serverService.getServer('non-existent-id')).rejects.toThrow(
        ServerNotFoundError,
      );
    });
  });
  ...
  describe('listServersForMember', () => {
    it('returns only servers the member belongs to', async () => {
      ...
      const ss2 = new ServerService({ channelService: ch2, storageProvider: createMockStorageProvider() });
  ...
  describe('addMembers — key distribution (Requirements 6.1, 6.2, 6.3)', () => {
    it('adds new members to all server channels with wrapped epoch keys', async () => {
      ...
      const added = await serverService.addMembers(server.id, ownerId, [
        newMember,
      ]);
      ...
      for (const channelId of refreshed.channelIds) {
        ...
        expect(channel).toBeDefined();
        expect(channel!.members.some((m) => m.memberId === newMember)).toBe(
          true,
        );
