...
export async function brightchainDatabaseInit(
  ...
  try {
    ...
    // Create BrightDb — all stores now implement createHeadRegistry():
    ...
    const headRegistry = blockStore.createHeadRegistry?.();

    let db = new BrightDb(
      blockStore,
      headRegistry
        ? { name: environment.memberPoolName, headRegistry }
        : undefined,
    );
