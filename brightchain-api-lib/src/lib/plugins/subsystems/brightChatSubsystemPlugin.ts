...
export class BrightChatSubsystemPlugin implements IAppSubsystemPlugin {
  ...
  public async initialize(context: ISubsystemContext): Promise<void> {
    ...
    const ensureMemberPublicKey = async (memberId: string): Promise<void> => {
      ...
      const hexKey = await (
        context.memberStore as MemberStore
      ).getMemberPublicKeyHex(memberIdBytes);
      if (!hexKey) {
        console.warn(
    ...
    // Load persisted servers from storage into the in-memory Map
