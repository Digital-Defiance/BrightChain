...
describe('SymmetricService', () => {
  ...
  const testKey = randomBytes(ECIES.SYMMETRIC.KEY_SIZE);
  ...
  describe('buffer encryption', () => {
    ...
    it('should throw error with incorrect key length', () => {
      const shortKey = Buffer.alloc(ECIES.SYMMETRIC.KEY_SIZE - 1, 'a');
      expect(() =>
  ...
  describe('json encryption', () => {
    ...
    it('should handle error cases', () => {
      ...
      const invalidKey = Buffer.alloc(ECIES.SYMMETRIC.KEY_SIZE, 'x');
      expect(() =>
        SymmetricService.decryptJson<string>(
          encrypted.encryptedData,
          invalidKey,
        ),
      ).toThrow(); // AES-GCM authentication will fail before JSON parsing
    });
  });
});
