...
import type { IBrightTrustDocument } from '../interfaces/document/brightTrustDocument';
import { ServiceProvider } from '../services/service.provider';
import { SchemaDefinition, SerializedValue } from '../sharedTypes';
import { Checksum } from '../types';

export class BrightTrustDocumentSchema<TID extends PlatformID = Uint8Array> {
  ...
  public schema: SchemaDefinition<IBrightTrustDocument<TID>> = {
    checksum: {
      type: Object,
      required: true,
    ...
    encryptedData: {
      ...
      required: false,
      serialize: (
        value: BrightTrustDataRecord<TID> | undefined,
      ): SerializedValue | null => {
        if (!value) return null;
        const json = value.toJson();
        return JSON.parse(json);
      },
      hydrate: (value: string): BrightTrustDataRecord<TID> | undefined => {
        ...
        return BrightTrustDataRecord.fromJson<TID>(
