...
describe('CBLService', () => {
  ...
  describe('Signature Validation', () => {
    ...
    it('should fail validation with wrong creator', () => {
      // Get service instances (needed again if not scoped outside)
      const localEciesService = ServiceProvider.getInstance().eciesService;
      const localVotingService = ServiceProvider.getInstance().votingService;
      const { member: wrongCreator } = BrightChainMember.newMember(
        localEciesService, // Added
        localVotingService, // Added
        MemberType.User,
        faker.person.fullName(),
        new EmailString(faker.internet.email()),
      );
  ...
  describe('CBL Address Capacity Calculation', () => {
    ...
    it('should calculate correct capacity for basic CBL', () => {
      // Added BlockEncryptionType.None as the second argument
    ...
    it('should calculate correct capacity with encryption overhead', () => {
      // Changed boolean to EncryptedBlockType enum
      const withEncryption = cblService.calculateCBLAddressCapacity(
        BlockSize.Large,
        BlockEncryptionType.SingleRecipient, // Use SingleRecipient for basic encryption overhead
      );
      // Changed boolean to EncryptedBlockType enum
      const withoutEncryption = cblService.calculateCBLAddressCapacity(
        BlockSize.Large,
        BlockEncryptionType.None, // Use enum for 'false'
      );
    ...
    it('should calculate correct capacity with extended header', () => {
      ...
      const mimeType = 'text/plain';
      // Changed boolean to EncryptedBlockType enum and combined fileName/mimeType into an object
      const extendedCapacity = cblService.calculateCBLAddressCapacity(
        BlockSize.Large,
        BlockEncryptionType.None, // Use enum for 'false'
        { fileName, mimeType }, // Pass as object
      );
    ...
    it('should handle combined overhead', () => {
      ...
      const withExtended = cblService.calculateCBLAddressCapacity(
        BlockSize.Large,
        BlockEncryptionType.None, // Use enum for 'false'
        { fileName: filename, mimeType: mimeType }, // Pass as object
      );
      // Changed boolean to EncryptedBlockType enum and combined fileName/mimeType into an object
