...
All IDs in request paths and JSON bodies are hex-encoded strings. The server converts them to the internal `GuidV4Buffer` representation. Responses serialize all ID fields back to hex strings.
...
- [Quorum](#quorum-burnbagquorum) — `/burnbag/quorum`
...
|--------|------|------|-------------|
| GET | `/` | Yes | List all containers |
| POST | `/` | Yes | Create a new container |
| GET | `/:id` | Yes | Get container details |
| POST | `/:id/lock` | Yes | Lock a container (read-only) |
| POST | `/:id/destroy` | Yes | Destroy a container and all contents |

---

## GET /

List all vault containers owned by the authenticated user. Returns summary information including file/folder counts and seal status.
...
Create a new vault container. A root folder is automatically created inside the container.
...
Get detailed information about a specific vault container.
...
# Quorum (`/burnbag/quorum`)

Multi-party approval for sensitive operations on quorum-governed files and folders. When a file or folder has `quorumGoverned: true`, operations like destruction, external sharing, bulk deletion, and ACL changes require approval from a configurable quorum of authorized members.
