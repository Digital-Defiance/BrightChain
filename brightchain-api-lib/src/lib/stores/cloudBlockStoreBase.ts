...
import { createCloudHeadRegistry } from './cloudHeadRegistryFactory';
  ...
  implements IBlockStore, IPooledBlockStore
{
  ...
  /**
   * Create a DriverBackedHeadRegistry using this store's cloud I/O primitives.
   * Each head pointer is stored as a separate object (O(1) per write), replacing
   * the legacy single-blob CloudHeadRegistry approach.
   */
  public createHeadRegistry(): IHeadRegistry {
    return createCloudHeadRegistry({
  ...
  public async listPools(): Promise<PoolId[]> {
    ...
    for (const key of keys) {
      ...
      if (slashIndex > 0) {
        ...
        if (poolId === 'parity') continue;
        // Skip reserved internal namespaces (e.g. '__brightchain-heads',
        // any future '__brightchain-*' prefix). These are not pools and
        // their contents are not blocks.
        if (poolId.startsWith('__')) continue;
        pools.add(poolId);
      }
    }
  ...
  public async *listBlocksInPool(
    ...
    validatePoolId(pool);
    // Reserved internal namespaces (e.g. '__brightchain-heads') are not
    // pools and contain no blocks; surface an empty iterable rather than
    // leaking internal object names as block hashes.
    if (pool.startsWith('__')) {
      return;
    }
