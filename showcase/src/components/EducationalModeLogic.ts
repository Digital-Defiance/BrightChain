...
export const createDefaultEducationalContent = (): EducationalContent => ({
  stepExplanations: new Map([
    ['file-read', {
      ...
      whatsHappening: 'The system is reading your file from disk and loading it into memory as binary data.',
      whyItMatters: 'This is the first step in the BrightChain process. We need the file data in memory to break it into blocks.',
      technicalDetails: 'The file is read as an ArrayBuffer and converted to a Uint8Array for binary manipulation.',
      relatedConcepts: ['binary-data', 'memory-management', 'file-system'],
      visualCues: ['File icon animation', 'Progress bar showing read progress']
    }],
    ['chunking', {
      title: 'Breaking into Chunks',
      description: 'Dividing the file into fixed-size segments',
      whatsHappening: 'The file data is being divided into equal-sized chunks that will become the foundation for blocks.',
      whyItMatters: 'Fixed-size chunks ensure uniform block sizes, which is essential for the distributed storage system.',
      technicalDetails: 'Data is split using array slicing with a predetermined block size (e.g., 1024 bytes).',
      relatedConcepts: ['block-size', 'data-segmentation', 'distributed-storage'],
      visualCues: ['File splitting animation', 'Chunk size indicators', 'Visual separators']
    }],
    ['padding', {
      title: 'Adding Padding',
      description: 'Adding random data to reach uniform block size',
      whatsHappening: 'Random data is being added to the last chunk to ensure all blocks are exactly the same size.',
      whyItMatters: 'Uniform block sizes prevent information leakage about file structure and improve security.',
      technicalDetails: 'Cryptographically secure random bytes are appended using crypto.getRandomValues().',
      relatedConcepts: ['cryptographic-padding', 'security', 'randomization'],
      visualCues: ['Random data visualization', 'Block size equalization', 'Padding indicators']
    }],
    ['checksum', {
      title: 'Calculating Checksums',
      description: 'Computing SHA-512 hashes for each block',
      whatsHappening: 'Each block is being processed through a cryptographic hash function to create a unique fingerprint.',
      whyItMatters: 'Checksums ensure data integrity and allow verification that blocks haven\'t been corrupted.',
      technicalDetails: 'SHA-512 hashing algorithm produces a 512-bit hash for each block, providing strong integrity guarantees.',
      relatedConcepts: ['cryptographic-hashing', 'data-integrity', 'sha-512'],
      visualCues: ['Hash calculation progress', 'Checksum generation animation', 'Mathematical visualization']
    }],
    ['storage', {
      title: 'Storing Blocks',
      description: 'Adding blocks to the soup storage system',
      whatsHappening: 'The processed blocks are being stored in the distributed "soup" where they mix with other file blocks.',
      whyItMatters: 'The soup storage system provides redundancy and makes it impossible to identify which blocks belong together.',
      technicalDetails: 'Blocks are stored with their checksums as identifiers in a key-value store.',
      relatedConcepts: ['distributed-storage', 'block-soup', 'redundancy'],
      visualCues: ['Soup can creation', 'Block placement animation', 'Storage confirmation']
    }],
    ['cbl-creation', {
      title: 'Creating CBL',
      description: 'Generating Constituent Block List metadata',
      whatsHappening: 'A metadata file is being created that contains the list of block IDs needed to reconstruct your file.',
      whyItMatters: 'The CBL is like a recipe that tells the system which blocks to collect and how to reassemble them.',
      technicalDetails: 'CBL contains block checksums, order information, and reconstruction parameters in a compact format.',
      relatedConcepts: ['metadata', 'block-list', 'file-reconstruction'],
      visualCues: ['Metadata compilation', 'Block reference collection', 'CBL structure visualization']
    }],
    ['magnet-url', {
      title: 'Generating Magnet URL',
      description: 'Creating a shareable link for file access',
      whatsHappening: 'A magnet URL is being generated that contains the CBL data and allows others to retrieve your file.',
      whyItMatters: 'Magnet URLs provide a decentralized way to share files without relying on central servers.',
      technicalDetails: 'The magnet URL encodes the CBL data using base64 encoding in a standardized URI format.',
      relatedConcepts: ['magnet-links', 'decentralization', 'file-sharing'],
      visualCues: ['URL generation', 'Link creation animation', 'Sharing indicator']
    }],
    ['cbl-processing', {
      title: 'Processing CBL',
      description: 'Reading Constituent Block List metadata',
      whatsHappening: 'The system is reading the CBL to understand which blocks are needed to reconstruct the file.',
      whyItMatters: 'The CBL contains all the information needed to find and reassemble the original file from the soup.',
      technicalDetails: 'CBL data is parsed to extract block checksums, order information, and reconstruction parameters.',
      relatedConcepts: ['metadata-parsing', 'block-list', 'file-reconstruction'],
      visualCues: ['CBL reading animation', 'Metadata extraction', 'Block list display']
    }],
    ['block-selection', {
      title: 'Selecting Blocks',
      description: 'Identifying required blocks from the soup',
      whatsHappening: 'The system is searching through the soup to find all the blocks listed in the CBL.',
      whyItMatters: 'We need to locate every single block to successfully reconstruct the original file.',
      technicalDetails: 'Block selection uses checksum matching to identify the correct blocks from the storage system.',
      relatedConcepts: ['block-identification', 'checksum-matching', 'soup-navigation'],
      visualCues: ['Block highlighting', 'Search animation', 'Selection indicators']
    }],
    ['block-retrieval', {
      title: 'Retrieving Blocks',
      description: 'Collecting blocks from storage',
      whatsHappening: 'The identified blocks are being retrieved from the soup and prepared for reassembly.',
      whyItMatters: 'All blocks must be successfully retrieved before the file can be reconstructed.',
      technicalDetails: 'Blocks are fetched from storage using their checksum identifiers and loaded into memory.',
      relatedConcepts: ['data-retrieval', 'block-collection', 'memory-loading'],
      visualCues: ['Block collection animation', 'Retrieval progress', 'Assembly preparation']
    }],
    ['validation', {
      title: 'Validating Checksums',
      description: 'Verifying block integrity',
      whatsHappening: 'Each retrieved block is being verified against its expected checksum to ensure data integrity.',
      whyItMatters: 'Checksum validation ensures that the blocks haven\'t been corrupted and are safe to use.',
      technicalDetails: 'SHA-512 hashes are recalculated for each block and compared against the stored checksums.',
      relatedConcepts: ['data-integrity', 'checksum-verification', 'corruption-detection'],
      visualCues: ['Validation progress', 'Checksum comparison', 'Integrity indicators']
    }],
    ['reassembly', {
      title: 'Reassembling File',
      description: 'Combining blocks and removing padding',
      whatsHappening: 'The validated blocks are being combined in the correct order and padding is being removed.',
      whyItMatters: 'This final step reconstructs your original file exactly as it was before encoding.',
      technicalDetails: 'Blocks are concatenated in order, then padding bytes are stripped to restore the original file.',
      relatedConcepts: ['file-reconstruction', 'padding-removal', 'data-assembly'],
      visualCues: ['Block combination', 'Padding removal', 'File reconstruction progress']
    }]
  ]),
  ...
  conceptGlossary: new Map([
    ['binary-data', {
      ...
      technicalDefinition: 'Data represented in base-2 format, where each bit can be either 0 or 1',
    ...
    ['block-size', {
      ...
      technicalDefinition: 'A predetermined number of bytes that standardizes block dimensions for storage efficiency',
    ...
    ['cryptographic-hashing', {
      ...
      definition: 'A mathematical function that converts data into a fixed-size string',
      technicalDefinition: 'A one-way function that produces a deterministic, fixed-length output from variable-length input',
    ...
    ['distributed-storage', {
      ...
      technicalDefinition: 'A storage architecture that spreads data across multiple nodes to improve reliability and availability',
      examples: ['Cloud storage', 'Peer-to-peer networks', 'Blockchain storage'],
    ...
    ['block-soup', {
      ...
      technicalDefinition: 'A storage system where blocks from multiple files are intermixed to prevent pattern recognition',
      examples: ['Mixed file blocks', 'Randomized storage', 'Anonymous data pool'],
  ...
  processCompletionSummaries: new Map([
    ['encoding', {
      ...
      description: 'Your file has been successfully broken down into secure, distributed blocks.',
      keyAchievements: [
    ...
    ['reconstruction', {
      ...
      description: 'Your original file has been successfully reassembled from the block soup.',
      keyAchievements: [
...
export class EducationalModeLogic {
  ...
  onStepStart(step: ProcessStep): void {
    ...
    if (this.state.config.stepByStepMode && this.state.config.pauseBetweenSteps) {
