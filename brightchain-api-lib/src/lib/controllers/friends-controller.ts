 ...
 * Uses both instanceof and duck-typing to handle module identity issues
 * that can arise when the same module is loaded multiple times in parallel
 * Jest workers.
 */
function handleFriendsError(err: unknown, res: Response): void {
  const potentialCode = (err as FriendsServiceError)?.code as
    | FriendsErrorCode
    | undefined;
  const isFriendsServiceError =
    err instanceof FriendsServiceError ||
    (err instanceof Error &&
      err.constructor.name === 'FriendsServiceError' &&
      potentialCode !== undefined &&
      potentialCode in errorCodeToHttpStatus);
  if (isFriendsServiceError) {
    const status = errorCodeToHttpStatus[potentialCode!] ?? 500;
  ...
  router.get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        res.status(200).json({
          success: true,
          message: translate(BrightChainStrings.Friends_SentRequestsRetrieved),
          items: result.items,
          hasMore: result.hasMore,
        });
      } catch (err) {
