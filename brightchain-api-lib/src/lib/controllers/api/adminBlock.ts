...
> {
  ...
  private async handleListBlocks(
    ...
    try {
      ...
      if (isPooledBlockStore(blockStore)) {
        ...
        const result = await this.streamBlockPage(
          blockStore,
          pool,
          limit,
          cursor,
      ...
      const poolTotal = blockStore.getBlockCount
        ? await blockStore.getBlockCount()
        : 0;
  ...
  private async handleListPools(
    ...
    try {
      ...
      const blockCount = blockStore.getBlockCount
        ? await blockStore.getBlockCount()
        : 0;
      const pools =
        blockCount > 0 || !blockStore.getBlockCount
        ...
        for (const pool of pools) {
          ...
          for await (const hash of blockStore.listBlocksInPool(pool)) {
            if (hash.toLowerCase().startsWith(query)) {
              const metadata = await blockStore
                .getMetadata(hash)
                .catch(() => null);
