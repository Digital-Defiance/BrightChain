...
import { FileHeadRegistryDriver } from '@brightchain/db';
...
export class DiskBlockStore implements IBlockStore {
  ...
  public createHeadRegistry(): IHeadRegistry {
    return new DriverBackedHeadRegistry(
      new FileHeadRegistryDriver({ dir: this.storePath }),
    );
  }
  ...
  public async *listBlockIds(options?: { limit?: number; cursor?: string }): AsyncIterable<string> {
    ...
    let files: string[];
    try {
      files = await readdir(this.blocksDir);
      files.sort(); // Ensure consistent ordering for cursor pagination
    } catch {
      return; // blocksDir doesn't exist yet
    }
