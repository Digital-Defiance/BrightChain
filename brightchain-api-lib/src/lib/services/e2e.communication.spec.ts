...
async function buildMultiNodeEnv(count: number): Promise<{
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Catch-all error handler so middleware errors surface as 500 with a
    // message instead of being swallowed or producing misleading status codes.
  ...
  describe('Pagination across nodes', () => {
    it('paginate group messages — send on node-0, paginate on node-1', async () => {
      ...
      for (let i = 0; i < 12; i++) {
        ...
        if (res.status !== 201) {
          throw new Error(
            `POST /groups/${gid}/messages msg-${i} returned ${res.status}: ${JSON.stringify(res.body)}`,
          );
        }
