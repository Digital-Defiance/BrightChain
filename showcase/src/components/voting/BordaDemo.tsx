      ...
      <div className="voting-demo">
        <div className="election-intro">
          ...
          <div className="intro-story">
            <p>🌍 <strong>IOC Committee Room:</strong> Five nations must choose the next Olympic host city. But everyone has preferences!</p>
            <p>🎯 <strong>Borda Count</strong> gives points based on ranking: 1st place = 3 points, 2nd = 2 points, 3rd = 1 point.</p>
            <p className="intro-challenge">💡 This rewards consensus picks over polarizing choices. The city with the most total points wins!</p>
          </div>
  ...
  return (
    <>
      ...
      <div className="demo-header">
        ...
        <p className="election-tagline">📊 Rank all cities. Points = consensus!</p>
      </div>
      ...
      {!results ? (
        <>
          ...
          <div className="voters-section">
            <h4>IOC Members ({votes.size}/{voters.length} voted)</h4>
            {voters.map(voter => {
                ...
                <div key={voter} className="voter-card">
                  ...
                  {!hasVoted && (
                    <div className="borda-quick-vote">
                      <button onClick={() => submitRanking(voter, [0, 1, 2])} className="vote-btn">
                        {candidates[0].emoji} → {candidates[1].emoji} → {candidates[2].emoji}
                      </button>
                      <button onClick={() => submitRanking(voter, [1, 0, 2])} className="vote-btn">
                        {candidates[1].emoji} → {candidates[0].emoji} → {candidates[2].emoji}
                      </button>
                      <button onClick={() => submitRanking(voter, [2, 1, 0])} className="vote-btn">
                        {candidates[2].emoji} → {candidates[1].emoji} → {candidates[0].emoji}
                      </button>
                    </div>
                  )}
            ...
            return (
              <div key={idx} className={`result-bar ${isWinner ? 'winner' : ''}`}>
                ...
                <div className="progress-bar">
                  <div className="progress-fill" style={{ width: `${(points / 15) * 100}%` }} />
                </div>
              </div>
            );
          })}
          <button onClick={reset} className="reset-btn">New Vote</button>
        </div>
      )}
      </div>
    </>
