/**
 * @fileoverview Unit tests for BrightTrustGossipHandler
 *
 * Tests:
...
function makeProposalMetadata(
  overrides: Partial<BrightTrustProposalMetadata> = {},
): BrightTrustProposalMetadata {
  return {
...
function makeVoteMetadata(
  overrides: Partial<BrightTrustVoteMetadata> = {},
): BrightTrustVoteMetadata {
  return {
    proposalId: PROPOSAL_ID,
...
function createMockGossipService(): IGossipService & {
  ...
  return {
    ...
    announceBrightTrustProposal: jest.fn(async () => {}),
    announceBrightTrustVote: jest.fn(async () => {}),
    ...
    onBrightTrustProposal: jest.fn((handler: ProposalHandler) => {
      proposalHandlers.add(handler);
    }),
    offBrightTrustProposal: jest.fn((handler: ProposalHandler) => {
      proposalHandlers.delete(handler);
    }),
    onBrightTrustVote: jest.fn((handler: VoteHandler) => {
      voteHandlers.add(handler);
    }),
    offBrightTrustVote: jest.fn((handler: VoteHandler) => {
...
function createMockDatabase(options?: {
  ...
  } as IBrightTrustDatabase;
}

interface MockStateMachine extends IBrightTrustStateMachine {
...
function createMockStateMachine(): MockStateMachine {
  ...
  return {
    submittedProposals,
    submittedVotes,
    getMode: jest.fn(async () => 'brightTrust' as const),
    initialize: jest.fn(),
...
describe('BrightTrustGossipHandler', () => {
  ...
  beforeEach(() => {
    ...
    handler = new BrightTrustGossipHandler(gossipService, stateMachine, db);
  });
  ...
  describe('start/stop lifecycle', () => {
    it('should register handlers on start', () => {
      handler.start();
      expect(gossipService.onBrightTrustProposal).toHaveBeenCalledTimes(1);
      expect(gossipService.onBrightTrustVote).toHaveBeenCalledTimes(1);
    ...
    it('should unregister handlers on stop', () => {
      handler.start();
      handler.stop();
      expect(gossipService.offBrightTrustProposal).toHaveBeenCalledTimes(1);
      expect(gossipService.offBrightTrustVote).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('invalid proposer/voter message rejection', () => {
    ...
    it('should reject proposal from inactive member', async () => {
      ...
      const inactiveHandler = new BrightTrustGossipHandler(
        gossipService,
        stateMachine,
        inactiveDb,
      );
    ...
    it('should reject vote from unknown member', async () => {
      ...
      const handlerWithProposal = new BrightTrustGossipHandler(
        gossipService,
        stateMachine,
        dbWithProposal,
      );
    ...
    it('should reject vote from inactive member', async () => {
      ...
      const handlerWithInactive = new BrightTrustGossipHandler(
        gossipService,
        stateMachine,
        dbWithInactive,
      );
    ...
    it('should reject vote for non-pending proposal', async () => {
      ...
      const handlerWithApproved = new BrightTrustGossipHandler(
        gossipService,
        stateMachine,
        dbWithApproved,
      );
    ...
    it('should silently discard announcement without brightTrustProposal metadata', async () => {
      const announcement: BlockAnnouncement = {
        ...
        timestamp: new Date(),
        ttl: 5,
        // brightTrustProposal is missing
      };
    ...
    it('should silently discard announcement without brightTrustVote metadata', async () => {
      const announcement: BlockAnnouncement = {
        type: 'brightTrust_vote',
        blockId: 'some-id',
        nodeId: 'remote-node',
  ...
  describe('duplicate message handling', () => {
    ...
    it('should skip proposal that already exists in database', async () => {
      ...
      const handlerWithExisting = new BrightTrustGossipHandler(
        gossipService,
        stateMachine,
        dbWithExisting,
      );
  ...
  describe('CBL attachment retrieval and caching', () => {
    it('should cache CBL attachment when proposal has attachmentCblId', async () => {
      ...
      const handlerWithCBL = new BrightTrustGossipHandler(
        gossipService,
        stateMachine,
        db,
        cblProvider,
    ...
    it('should handle CBL retrieval failure gracefully', async () => {
      ...
      const handlerWithFailing = new BrightTrustGossipHandler(
        gossipService,
        stateMachine,
        db,
        failingProvider,
    ...
    it('should handle null CBL content gracefully', async () => {
      ...
      const handlerWithNull = new BrightTrustGossipHandler(
        gossipService,
        stateMachine,
        db,
        nullProvider,
