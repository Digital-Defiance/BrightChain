  ...
  async buildRequestUserDTO(userId: string): Promise<IRequestUserDTO | null> {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          `[Auth] RBAC role lookup failed for member ${memberId}:`,
          rbacErr instanceof Error ? rbacErr.message : rbacErr,
        );
  ...
  private async buildRequestUserDTOFromDb(
    ...
    try {
      ...
      try {
        ...
        if (!userRoleDoc) {
          userRoleDoc = await userRolesCol.findOne({ userId } as never);
        }
        if (!userRoleDoc) {
          const dashed = idHex.replace(
            /^([0-9a-f]{8})([0-9a-f]{4})([0-9a-f]{4})([0-9a-f]{4})([0-9a-f]{12})$/i,
        ...
        console.warn(
          `[Auth] Role lookup failed for user ${userId}:`,
