...
describe('MemberDocument', () => {
  ...
  describe('Constructor', () => {
    it('should create a document with valid members', () => {
    ...
    it('should throw error for invalid member data', () => {
      ...
      expect(() => {
        new MemberDocument(invalidMember, privateMember);
      }).toThrow(MemberError);
    });
  });
  ...
  describe('CBL Generation', () => {
    ...
    beforeEach(() => {
      doc = new MemberDocument(publicMember, privateMember);
    });

    it('should generate CBLs successfully', async () => {
      await doc.generateCBLs();
      
      expect(doc.getPublicCBL()).toBeInstanceOf(Uint8Array);
      expect(doc.getPrivateCBL()).toBeInstanceOf(Uint8Array);
    });
  ...
  describe('CBL Serialization/Deserialization', () => {
    ...
    it('should create document from CBLs', async () => {
      const newDoc = new MemberDocument(publicMember, privateMember);
    ...
    it('should preserve member data through CBL round-trip', async () => {
      ...
      expect(idProvider.idToString(restoredPublic.id)).toBe(idProvider.idToString(originalPublic.id));
    ...
    it('should handle invalid CBL data', async () => {
      ...
      await expect(newDoc.createFromCBLs(invalidCBL, privateCBL))
  ...
  describe('Member Conversion', () => {
    ...
    it('should convert to public member', async () => {
      ...
      expect(idProvider.idToString(member.id)).toBe(idProvider.idToString(publicMember.id));
    ...
    it('should convert to private member', async () => {
      ...
      expect(member).toBeInstanceOf(Member);
      expect(idProvider.idToString(member.id)).toBe(idProvider.idToString(privateMember.id));
    });
  });
  ...
  describe('Generic Type Support', () => {
    it('should work with string IDs', async () => {
      ...
      const doc = new MemberDocument<string>(stringMember, stringMember);
      await doc.generateCBLs();
    ...
    it('should maintain type consistency through CBL operations', async () => {
      const doc = new MemberDocument<Uint8Array>(publicMember, privateMember);
      await doc.generateCBLs();
      
      const publicCBL = await doc.toPublicCBL();
      const privateCBL = await doc.toPrivateCBL();
      
      const newDoc = new MemberDocument<Uint8Array>(publicMember, privateMember);
      await newDoc.createFromCBLs(publicCBL, privateCBL);
  ...
  describe('Block Store Integration', () => {
    ...
    it('should store constituent blocks in the block store', async () => {
      ...
      expect(publicCBLId).toBeInstanceOf(Uint8Array);
      expect(privateCBLId).toBeInstanceOf(Uint8Array);
    });
  });

  describe('Configuration Options', () => {
    it('should respect custom block size configuration', () => {
      const smallDoc = new MemberDocument(
        publicMember,
        privateMember,
        undefined,
        undefined,
      ...
      const largeDoc = new MemberDocument(
        publicMember,
        privateMember,
        undefined,
        undefined,
