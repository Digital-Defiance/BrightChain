    ...
    fc.assert(
      fc.property(arbBrandConfig, fc.boolean(), (brandConfig, hasChildren) => {
        ...
        if (hasChildren) {
          ...
          expect(
            screen.queryByTestId('mock-outlet'),
          ).not.toBeInTheDocument();
        } else {
          expect(screen.getByTestId('mock-outlet')).toBeInTheDocument();
          expect(
            screen.queryByTestId('test-children'),
          ).not.toBeInTheDocument();
        }
...
describe('Property 13: Theme merging from BrandConfig', () => {
  ...
  function ThemeReader() {
    ...
    return (
      <div data-testid="theme-primary-color">
    ...
    fc.assert(
      fc.property(arbBrandConfig, (brandConfig) => {
        ...
        if (brandConfig.logo) {
          ...
          expect(
            screen.queryByTestId('layout-logo'),
          ).not.toBeInTheDocument();
        }
...
describe('Property 15: BrandConfig context availability', () => {
  ...
  function BrandConfigReader() {
    ...
    return (
      <div data-testid="brand-config-appname">{config.appName}</div>
    );
  }
  ...
  it('descendant using useBrandConfig() receives the same BrandConfig', () => {
    fc.assert(
      fc.property(arbBrandConfig, (brandConfig) => {
        ...
        expect(
          screen.getByTestId('brand-config-appname'),
        ).toHaveTextContent(brandConfig.appName);
