...
/**
 * Compute a single overall delivery status string from a raw deliveryStatus
 * value. The stored value may be a serialized Map (plain object keyed by
 * recipient address) or already a string.
 */
function summarizeDeliveryStatus(raw: unknown): string {
  if (typeof raw === 'string') return raw;
  if (raw && typeof raw === 'object') {
    const values = Object.values(raw as Record<string, unknown>).map((v) =>
      String(v).toLowerCase(),
    );
    if (values.length === 0) return 'pending';
    if (values.every((s) => s === 'delivered' || s === 'read'))
      return 'delivered';
    if (values.some((s) => s === 'failed' || s === 'bounced')) return 'failed';
    if (values.some((s) => s === 'delivered' || s === 'read')) return 'partial';
    return values[0];
  }
  return 'unknown';
}
    ...
    try {
      ...
      if (mailService?.listEmails) {
        ...
        const emails = result.emails.map((email: Record<string, unknown>) => ({
          ...
          subject: String(email['subject'] ?? '').slice(0, 100),
          createdAt: email['createdAt'] ?? null,
          deliveryStatus: summarizeDeliveryStatus(email['deliveryStatus']),
        }));
      ...
      if (brightDb) {
        ...
        const emails = docs.map((doc: Record<string, unknown>) => ({
          ...
          subject: String(doc['subject'] ?? '').slice(0, 100),
          createdAt: doc['createdAt'] ?? null,
          deliveryStatus: summarizeDeliveryStatus(doc['deliveryStatus']),
        }));
