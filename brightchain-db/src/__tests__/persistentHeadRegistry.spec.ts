...
describe('PersistentHeadRegistry', () => {
  ...
  describe('persistence (write-through)', () => {
    ...
    it('should write valid JSON to the per-key file', async () => {
      const reg = createRegistry();
      await reg.setHead('db1', 'col1', 'block-abc');

      // Two-level layout: <__heads>/<encodedDb>/<encodedColl>.json
      // Key 'db1:col1' → __heads/db1/col1.json
      const filePath = join(dataDir, '__heads', 'db1', 'col1.json');
      const content = await fs.readFile(filePath, 'utf-8');
      const parsed = JSON.parse(content);
      expect(parsed.blockId).toEqual('block-abc');
      expect(parsed.timestamp).toBeDefined();
    });

    it('should silently ignore the deprecated fileName option and use per-key storage', async () => {
      // fileName is deprecated and no longer used; verify option is accepted without error
      // and data is still stored in __heads/ subdirectory
      const reg = createRegistry({ fileName: 'custom-heads.json' });
      await reg.setHead('db1', 'col1', 'block-abc');

      // Data must be in the two-level layout file, not in any custom-named file
      const filePath = join(dataDir, '__heads', 'db1', 'col1.json');
      const content = await fs.readFile(filePath, 'utf-8');
      const parsed = JSON.parse(content);
      expect(parsed.blockId).toEqual('block-abc');
  ...
  describe('load – corrupt/missing file handling', () => {
    ...
    it('should log warning and start empty when file contains invalid JSON', async () => {
      // Write an invalid legacy file — migration detects it cannot parse the JSON
      ...
      expect(warnSpy).toHaveBeenCalledWith(
        expect.stringContaining('Legacy file is not valid JSON'),
      );
      warnSpy.mockRestore();
    });
    ...
    it('should reflect on-disk changes when load is called again', async () => {
      const reg = createRegistry();
      await reg.setHead('db1', 'col1', 'block-a');

      // Externally delete the per-key file and add a different one
      const headsDir = join(dataDir, '__heads');
      await fs.unlink(join(headsDir, 'db1', 'col1.json'));
      await fs.mkdir(join(headsDir, 'db2'), { recursive: true });
      await fs.writeFile(
      ...
      await reg.load();
      // Key that was deleted from disk should be gone from cache
      expect(reg.getHead('db1', 'col1')).toBeUndefined();
      // New key that appeared on disk should be loaded
