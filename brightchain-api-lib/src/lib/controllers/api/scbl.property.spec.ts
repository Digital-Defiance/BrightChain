...
/**
 * Ensure the i18n subsystem is fully bootstrapped.
 *
 * Under parallel Jest workers the GlobalActiveContext singleton can lose its
 * default context (e.g. when another test file clears it). The upstream
 * authenticateToken middleware accesses `context.adminTimezone.constructor`
 * which throws if the context map is empty, causing spurious 403/500 errors.
 *
 * Calling this before every property-test iteration guarantees the context
 * is always present.
 */
function ensureI18nContext(): void {
  ...
  // Ensure the default context exists in the GlobalActiveContext map
  const ctx = GlobalActiveContext.getInstance();
  try {
    // Access .context to verify the default key is present
    ctx.context;
  } catch {
    // Context map was cleared — recreate it
...
const createTestApp = (storePath: string): Express => {
  ...
  // Re-bootstrap i18n before every request — parallel Jest workers can
  // clear the GlobalActiveContext singleton between requests.
...
const cleanupTestDir = (dir: string) => {
  if (existsSync(dir)) {
    try {
      ...
      // Best-effort cleanup — open file handles under parallel load can cause
      // ENOTEMPTY on macOS even with recursive:true. The tmp dir is ephemeral;
      // leaving it behind does not affect test correctness.
    }
  }
};
...
describe('Super CBL Operations Property Tests', () => {
  ...
  describe('Property 4: Super CBL Round-Trip with Metadata', () => {
    ...
    it('Property 4a: Small data round-trip preserves content', async () => {
      const testDir = join(TEST_DIR, `small-${Date.now()}`);
    ...
    it('Property 4b: Store response includes valid metadata', async () => {
      const testDir = join(TEST_DIR, `meta-${Date.now()}`);
      ensureTestDir(testDir);
      const app = createTestApp(testDir);
      try {
        await fc.assert(
    ...
    it('Property 4c: Encryption flag is preserved in round-trip', async () => {
      const testDir = join(TEST_DIR, `enc-${Date.now()}`);
      ensureTestDir(testDir);
      const app = createTestApp(testDir);
      try {
        await fc.assert(
  ...
  describe('Property 5: Super CBL Durability Propagation', () => {
    ...
    it('Property 5a: All durability levels are accepted', async () => {
      ...
      const testDir = join(TEST_DIR, `dur-${Date.now()}`);
      ensureTestDir(testDir);
      const app = createTestApp(testDir);
      try {
        await fc.assert(
    ...
    it('Property 5b: Durability level is applied to stored blocks', async () => {
      const testDir = join(TEST_DIR, `durprop-${Date.now()}`);
      ensureTestDir(testDir);
      const app = createTestApp(testDir);
      // Shared verification store reads from the same directory the app writes to
      const verifyStore = new DiskBlockAsyncStore({
        storePath: testDir,
          ...
          fc.asyncProperty(
            ...
            async (data, durabilityLevel) => {
              ...
              for (const blockId of rootBlockIds) {
                const metadata = await verifyStore.getMetadata(blockId);
    ...
    it('Property 5c: Default durability level is Standard', async () => {
      const testDir = join(TEST_DIR, `default-${Date.now()}`);
      ensureTestDir(testDir);
      const app = createTestApp(testDir);
      // Shared verification store reads from the same directory the app writes to
      const verifyStore = new DiskBlockAsyncStore({
