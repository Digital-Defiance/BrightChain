/**
 * @fileoverview IBrightTrustDatabase interface.
 *
 * Abstraction over BrightDb with a dedicated "brightTrust-system" pool.
...
import { BrightTrustAuditLogEntry } from '../auditLogEntry';
...
import { IBrightTrustMember } from './brightTrustService';

/**
 * Abstraction over BrightDb with a dedicated "brightTrust-system" pool.
 *
 * Implementations live in brightchain-api-lib (BrightTrustDatabaseAdapter).
 * All BrightTrust data is routed through the PooledStoreAdapter for isolation.
 *
 * @template TID - Platform ID type for frontend/backend DTO compatibility
 */
export interface IBrightTrustDatabase<TID extends PlatformID = Uint8Array> {
  // === Epoch Management ===
   ...
   */
  saveEpoch(epoch: BrightTrustEpoch<TID>): Promise<void>;
  ...
  /**
   * Persist a BrightTrust member record.
   * @param member - The member to save
   */
  saveMember(member: IBrightTrustMember<TID>): Promise<void>;
  ...
  /**
   * List all active members in the BrightTrust.
   * @returns Array of active BrightTrust members
   */
  listActiveMembers(): Promise<IBrightTrustMember<TID>[]>;
   ...
   */
  saveDocument(doc: BrightTrustDataRecord<TID>): Promise<void>;
   ...
   */
  getDocument(docId: TID): Promise<BrightTrustDataRecord<TID> | null>;
   ...
   */
  appendAuditEntry(entry: BrightTrustAuditLogEntry<TID>): Promise<void>;
  ...
  /**
   * Check if the BrightTrust database pool is available.
