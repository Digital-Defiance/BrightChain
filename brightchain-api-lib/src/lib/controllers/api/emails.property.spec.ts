...
function buildTestEnv(): TestEnv {
  ...
  // Re-bootstrap i18n before every request
  app.use(
    (
    ...
    try {
      await fc.assert(
        fc.asyncProperty(arbEmailInput, async (emailInput) => {
          ...
          // Verify from matches the server-derived sender (the authenticated
          // user `${username}@${emailDomain}`) — any client-supplied `from`
          // is ignored to prevent sender spoofing.
