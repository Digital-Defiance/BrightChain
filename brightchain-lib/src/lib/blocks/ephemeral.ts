...
export class EphemeralBlock extends BaseBlock implements IDataBlock {
  ...
  protected static override initialize() {
    ...
    if (!EphemeralBlock.eciesService) {
      EphemeralBlock.eciesService = ServiceProvider.getECIESService();
    }
  }

  /**
   * Gets the ECIES service
   * @throws Error if the ECIES service is not initialized
   */
  protected static get ECIESService(): ECIESService {
    if (!EphemeralBlock.eciesService) {
      throw new Error('ECIESService not initialized');
    }
    return EphemeralBlock.eciesService;
  }
   ...
   */
  protected readonly _encrypted: boolean;
  /**
   ...
   */
  protected readonly _creatorId: GuidV4;
  ...
  public get canEncrypt(): boolean {
    ...
    return (
      !this._encrypted &&
      this._lengthBeforeEncryption + ECIES.OVERHEAD_SIZE <= this.blockSize
    );
  }
