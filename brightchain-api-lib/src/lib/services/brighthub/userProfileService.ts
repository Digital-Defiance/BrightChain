...
export class UserProfileService implements IUserProfileService {
  ...
  constructor(application: IApplicationWithCollections) {
    ...
    this.postsCollection = application.getModel<{
      _id: string;
      authorId: string;
      isDeleted?: boolean;
    }>('brighthub_posts');
  ...
  private recordToProfile(record: UserProfileRecord): IBaseUserProfile<string> {
    return {
      ...
      bio: record.bio,
      formattedBio: record.formattedBio,
      pinnedPostId: record.pinnedPostId,
  ...
  async getProfile(
    ...
    // Compute postCount dynamically from the posts collection
    // so it stays accurate after creates/deletes without manual sync.
    try {
      if (this.postsCollection.countDocuments) {
        const postCount = await this.postsCollection
          .countDocuments({
      ...
      // If posts collection is unavailable, fall back to stored value
    }
   ...
   * Returns the same shape as getProfile but resolves via username instead of ID.
   */
  async getProfileByUsername(
    username: string,
    requesterId?: string,
  ): Promise<IBaseUserProfile<string>> {
    // Search the profiles collection by username (case-insensitive)
    const profile = await this.userProfilesCollection
      .findOne({
        username: {
          $regex: new RegExp(
    ...
    if (!profile) {
      // Try the main users collection as fallback
      let mainUser: { _id: string; username?: string } | null = null;
      try {
        mainUser = (await this.usersCollection
          .findOne({
            username: {
              $regex: new RegExp(
                `^${username.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`,
          ...
          .exec()) as { _id: string; username?: string } | null;
      } catch {
        // users collection lookup failed
      }

      if (mainUser) {
        // Auto-create profile and delegate to getProfile
        return this.getProfile(mainUser._id, requesterId);
      }
    ...
    // Delegate to getProfile with the resolved ID for consistent privacy/follow logic
    return this.getProfile(profile._id, requesterId);
  }
  ...
  async updateProfile(
    ...
    if (updates.bio !== undefined) {
      ...
      // Generate formatted HTML from the bio markdown/icon content
      const formatter = getTextFormatter();
      const formatted = formatter.format(updates.bio, { isBlogPost: true });
      updateFields.formattedBio = formatted.formatted;
    }
    ...
    if (updates.pinnedPostId !== undefined) {
      updateFields.pinnedPostId = updates.pinnedPostId;
    }
