...
import { IAvailabilityService, NodeIdSource, NodeStatus } from '@brightchain/brightchain-lib';
        ...
        async ({ active, threshold, members }) => {
          ...
          if (active) {
            ...
            for (const member of members) {
              const found = response.brightTrust.members.find(
                (m) => m.name === member.name,
              );
              expect(found).toBeDefined();
              if (member.role !== undefined) {
                expect(found!.role).toBe(member.role);
              }
            }
          } else {
