---
title: "BrightTrust System in BrightChain"
parent: "Governance & Voting"
nav_order: 3
---
# BrightTrust System in BrightChain

## Status: ✅ CORE IMPLEMENTATION COMPLETE (January 2025)

The BrightTrust system is **fully operational** with Shamir's Secret Sharing, encrypted document management, and configurable threshold governance. This document provides technical details for developers.

## Overview

The BrightChain BrightTrust system provides a decentralized governance structure for managing sensitive data, identity verification, and brokered anonymity. It implements a digital statute of limitations through time-based data expiration and requires consensus for sensitive operations.
...
```typescript
// Main BrightTrust class
class BrightChainBrightTrust<TID extends PlatformID = Uint8Array> {
  ...
  private readonly _documentsById: SimpleStore<ShortHexGuid, BrightTrustDataRecord<TID>>;
  private readonly _sealingService: SealingService<TID>;
}

// Document record with encrypted data and shares
class BrightTrustDataRecord<TID extends PlatformID = Uint8Array> {
  public readonly id: TID;
...
enum NodeTrustLevel {
  ...
  BRIGHT_TRUST_MEMBER,   // Trusted for BrightTrust operations
  BRIGHT_TRUST_ADMIN,    // Can manage BrightTrust membership
}

interface BrightTrustMemberMetadata {
...
// 5. Create BrightTrustDataRecord
return new BrightTrustDataRecord(
  agent,
...
```typescript
// Currently implemented in BrightTrustDataRecord
...
```typescript
// ✅ Implemented
interface BrightTrustDataRecord {
  id: TID;
...
```typescript
// Add member to BrightTrust (internal)
...
public async addDocument<T>(
  ...
  const newDoc = await this._sealingService.BrightTrustSeal(
    agent,
    document,
    amongstMembers,
    sharesRequired
...
public async getDocument<T>(
  ...
  return await this._sealingService.BrightTrustUnseal(doc, members);
}
...
```typescript
// Future implementation for BrightTrust voting
...
```typescript
// Shares are managed within BrightTrustDataRecord
...
interface BrightTrustDataRecordDto {
  id: ShortHexGuid;
...
public toDto(): BrightTrustDataRecordDto;
public toJson(): string;
public static fromDto<TID>(dto: BrightTrustDataRecordDto): BrightTrustDataRecord<TID>;
public static fromJson<TID>(json: string): BrightTrustDataRecord<TID>;
```
...
### Basic BrightTrust Setup

```typescript
import { BrightChainBrightTrust } from 'brightchain-lib';
...
if (BrightTrust.canUnlock(document.id, memberSubset)) {
  // Unseal document
