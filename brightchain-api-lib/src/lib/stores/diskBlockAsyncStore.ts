 ...
 */
export class DiskBlockAsyncStore extends DiskBlockStore implements IBlockStore, IPooledBlockStore {
  /**
  ...
  // =========================================================================
  // IPooledBlockStore — Pool-Scoped CRUD
  // =========================================================================
  ...
  public async hasInPool(pool: PoolId, hash: string): Promise<boolean> {
    ...
    for (const size of this._supportedBlockSizes) {
      const filePath = this.poolBlockPath(pool, checksum, size);
  ...
  public async putInPool(
    ...
    // Pick the smallest supported size that fits
    const fitting = this._supportedBlockSizes.find(
      (s) => data.length <= (s as number),
    );
    const blockSize = fitting ?? this._supportedBlockSizes[0];

    const filePath = this.poolBlockPath(pool, checksum, blockSize);
    if (existsSync(filePath)) return hash; // idempotent

    this.ensurePoolBlockPath(pool, checksum, blockSize);
    await writeFile(filePath, Buffer.from(data));

    // Create metadata with pool association
    const keyHex = this.keyToHex(checksum);
    const metadata = createDefaultBlockMetadata(keyHex, data.length, keyHex, options, pool);
    try {
      await this.metadataStore.create(metadata);
    } catch {
      // Metadata may already exist
      if (this.metadataStore.has(keyHex)) {
        await this.metadataStore.update(keyHex, { poolId: pool });
      }
    }
  ...
  public async listPools(): Promise<PoolId[]> {
    ...
    for (const size of this._supportedBlockSizes) {
      ...
      let entries: string[];
      try { entries = await readdir(basePath); } catch { continue; }

      for (const entry of entries) {
        // Pool dirs have names longer than 1 char (hex char dirs are 1 char)
  ...
  /** List block hashes in a pool with cursor-based pagination (streaming) */
  public async *listBlocksInPool(pool: PoolId, options?: ListOptions): AsyncIterable<string> {
    validatePoolId(pool);
    ...
    for (const size of this._supportedBlockSizes) {
      ...
      let char1Dirs: string[];
      try { char1Dirs = await readdir(poolPath); } catch { continue; }

      for (const c1 of char1Dirs.sort()) {
        const c1Path = join(poolPath, c1);
        let c1Stat;
        try { c1Stat = await stat(c1Path); } catch { continue; }
        if (!c1Stat.isDirectory()) continue;

        let char2Dirs: string[];
        try { char2Dirs = await readdir(c1Path); } catch { continue; }

        for (const c2 of char2Dirs.sort()) {
          const c2Path = join(c1Path, c2);
          let c2Stat;
          try { c2Stat = await stat(c2Path); } catch { continue; }
          if (!c2Stat.isDirectory()) continue;

          let files: string[];
          try { files = await readdir(c2Path); } catch { continue; }

          for (const file of files.sort()) {
            if (file.endsWith('.m.json') || file.includes(PARITY_FILE_EXTENSION_PREFIX)) continue;

            if (!pastCursor) {
              if (file === cursor) pastCursor = true;
              continue;
            }
            yield file;
            yielded++;
  ...
  public async getPoolStats(pool: PoolId): Promise<PoolStats> {
    ...
    return { poolId: pool, blockCount, totalBytes, createdAt: earliest, lastAccessedAt: latest };
  }
  ...
  public async getRandomBlocksFromPool(pool: PoolId, count: number): Promise<Checksum[]> {
    ...
    for (const size of this._supportedBlockSizes) {
      ...
      const poolPath = join(this._storePath, blockSizeString, pool);
      const hashes = await this.collectPoolBlockHashes(poolPath);
      for (const h of hashes) allHashes.push(h);
    }

    const actualCount = Math.min(count, allHashes.length);
    const shuffled = [...allHashes].sort(() => Math.random() - 0.5);
  ...
  public async storeCBLWithWhiteningInPool(
    ...
    if (paddedCbl.length > this.blockSize) {
      throw new StoreError(StoreErrorType.BlockValidationFailed, undefined, {
        ERROR: `CBL data too large: padded size (${paddedCbl.length}) exceeds block size (${this.blockSize})`,
      });
    }

    // Select randomizer from pool or generate fresh
    const poolRandomBlocks = await this.getRandomBlocksFromPool(pool, 1);
    let randomBlock: Uint8Array;
    if (poolRandomBlocks.length > 0) {
      const existingData = await this.getFromPool(pool, poolRandomBlocks[0].toHex());
      randomBlock = existingData.length >= paddedCbl.length
        ? existingData.slice(0, paddedCbl.length)
        : XorService.generateKey(paddedCbl.length);
    } else {
    ...
    try {
      ...
      block1Stored = true;
      const block2Id = toStorageKey(await this.putInPool(pool, xorResult, options));

      const magnetUrl = this.generateCBLMagnetUrl(block1Id, block2Id, this.blockSize);
      ...
      if (block1Stored) {
        try { await this.deleteFromPool(pool, block1Id); } catch { /* */ }
      }
      throw error;
    }
  }
  ...
  public override async put(
    ...
    const keyChecksum = typeof key === 'string' ? Checksum.fromHex(key) : key;
    // Determine block size: prefer existing block size on disk; otherwise pick
    // the smallest supported size that fits the data, falling back to the
    // closest BlockSize enum value if no supported size accommodates it.
    const existingSize = await this.findBlockSize(keyChecksum);
    let putBlockSize: BlockSize;
    if (existingSize !== null) {
      putBlockSize = existingSize;
    } else {
      const fitting = this.supportedBlockSizes.find(
        (s) => data.length <= (s as number),
      );
  ...
  public override async storeCBLWithWhitening(
    ...
    try {
      ...
      if (!(await this.has(block1Checksum))) {
        ...
        block1Stored = true;
      } else if (options?.durabilityLevel !== undefined) {
        // Block already exists (reused randomizer); update its durability metadata
        // so the returned block IDs reflect the requested durability level.
        await this.updateMetadata(block1Checksum, {
  ...
  // === Block Enumeration (complex directory layout) ===

  /**
   * List all block IDs by walking the complex directory structure.
   * Layout: storePath/blockSize/char1/char2/hash
   */
  public override async *listBlockIds(options?: { limit?: number; cursor?: string }): AsyncIterable<string> {
    const limit = options?.limit;
    const cursor = options?.cursor;
    let pastCursor = cursor === undefined;
    let yielded = 0;
    ...
    for (const size of this._supportedBlockSizes) {
      ...
      let char1Dirs: string[];
      try { char1Dirs = await readdir(basePath); } catch { continue; }

      for (const c1 of char1Dirs.sort()) {
        if (c1.length !== 1) continue; // skip non-hex-char dirs
        const c1Path = join(basePath, c1);
        let c1Stat;
        try { c1Stat = await stat(c1Path); } catch { continue; }
        if (!c1Stat.isDirectory()) continue;
  ...
  /**
   * Get the total number of blocks by counting files in the complex directory structure.
   */
  public override async getBlockCount(): Promise<number> {
    let count = 0;
    for (const size of this._supportedBlockSizes) {
      const blockSizeString = blockSizeToSizeString(size);
      const basePath = join(this._storePath, blockSizeString);
