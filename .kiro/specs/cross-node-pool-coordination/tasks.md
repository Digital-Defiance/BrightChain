...
- [x] 1. Extend BlockAnnouncement and validation with pool support (brightchain-lib)
  - [x] 1.1 Add optional `poolId` field to `BlockAnnouncement` interface and extend `type` union to include `'pool_deleted'` in `brightchain-lib/src/lib/interfaces/availability/gossipService.ts`
  ...
  - [x] 1.2 Write property test for announcement validation with poolId
    - **Property 3: Announcement validation rejects invalid poolIds**
    - **Validates: Requirements 1.6, 2.1**

- [x] 2. Add Pool Deletion Tombstone interface and extend IGossipService (brightchain-lib)
  - [x] 2.1 Create `IPoolDeletionTombstone` interface and `PoolDeletionTombstoneConfig` in `brightchain-lib/src/lib/interfaces/availability/poolDeletionTombstone.ts`
  ...
  - [x] 2.2 Extend `IGossipService` interface in `brightchain-lib/src/lib/interfaces/availability/gossipService.ts`
...
- [x] 3. Extend ILocationRecord with pool context (brightchain-lib)
  - [x] 3.1 Add optional `poolId` field to `ILocationRecord` and update serialization in `brightchain-lib/src/lib/interfaces/availability/locationRecord.ts`
  ...
  - [x] 3.2 Write property test for location record poolId round-trip serialization
    - **Property 12: Location record poolId round-trip serialization**
    - **Validates: Requirements 5.4**

- [x] 4. Extend IBlockRegistry with pool-scoped exports (brightchain-lib)
  - [x] 4.1 Add `PoolScopedBloomFilter`, `PoolScopedManifest` interfaces and extend `IBlockRegistry` in `brightchain-lib/src/lib/interfaces/availability/blockRegistry.ts`
  ...
  - [x] 4.2 Write property test for pool-scoped manifest export
    - **Property 7: Pool-scoped manifest groups blocks by pool**
    - **Validates: Requirements 3.1, 3.5**

- [x] 5. Extend IDiscoveryProtocol with pool-scoped discovery (brightchain-lib)
  - [x] 5.1 Update `IDiscoveryProtocol` and `DiscoveryResult` in `brightchain-lib/src/lib/interfaces/availability/discoveryProtocol.ts`
...
- [x] 6. Extend ReconciliationResult with pool statistics (brightchain-lib)
  - [x] 6.1 Add pool-scoped fields to `ReconciliationResult` in `brightchain-lib/src/lib/interfaces/availability/reconciliationService.ts`
...
- [x] 7. Checkpoint - Ensure all brightchain-lib interface changes compile cleanly
  - Ensure all tests pass, ask the user if questions arise.

- [x] 8. Update GossipService implementation with pool support (brightchain-api-lib)
  - [x] 8.1 Update `GossipService` in `brightchain-api-lib/src/lib/availability/gossipService.ts`
  ...
  - [x] 8.2 Write property test for gossip pool announcement creation
    - **Property 1: Block announcements include pool context**
    - **Validates: Requirements 1.1, 6.3**

  - [x] 8.3 Write property test for gossip poolId preservation during forwarding
    - **Property 2: Gossip preserves poolId during forwarding**
    - **Validates: Requirements 1.2, 2.7**

  - [x] 8.4 Write property test for pool deletion announcement creation
    - **Property 4: Pool deletion creates correct announcement**
    - **Validates: Requirements 2.2**

- [x] 9. Update AvailabilityAwareBlockStore with pool-aware announcements and tombstones (brightchain-api-lib)
  - [x] 9.1 Add tombstone tracking and pool-aware storage to `AvailabilityAwareBlockStore` in `brightchain-api-lib/src/lib/stores/availabilityAwareBlockStore.ts`
  ...
  - [x] 9.2 Write property test for pool deletion cleanup
    - **Property 5: Pool deletion cleanup removes all pool blocks and registry entries**
    - **Validates: Requirements 2.3, 2.4**

  - [x] 9.3 Write property test for tombstone blocking storage
    - **Property 6: Tombstone blocks storage in deleted pool**
    - **Validates: Requirements 2.5, 2.6**

  - [x] 9.4 Write property test for location records including pool context
    - **Property 11: Location records include pool context**
    - **Validates: Requirements 5.2**

  - [x] 9.5 Write property test for received blocks stored in announced pool
    - **Property 15: Received blocks stored in announced pool**
    - **Validates: Requirements 1.3**

- [x] 10. Checkpoint - Ensure gossip and store changes pass tests
  - Ensure all tests pass, ask the user if questions arise.

- [x] 11. Implement pool-scoped discovery and Bloom filter support (brightchain-api-lib)
  - [x] 11.1 Update `IBlockRegistry` implementations to support pool-scoped addLocal/removeLocal and pool-scoped exports
  ...
  - [x] 11.2 Write property test for pool-scoped Bloom filter accuracy
    - **Property 10: Pool-scoped Bloom filter eliminates cross-pool false positives**
    - **Validates: Requirements 4.2, 4.5, 4.6**

- [x] 12. Implement pool-scoped reconciliation (brightchain-api-lib)
  - [x] 12.1 Update `IReconciliationService` implementation to use pool-scoped manifests
  ...
  - [x] 12.2 Write property test for reconciliation storing blocks in correct pool
    - **Property 8: Reconciliation stores blocks in correct pool**
    - **Validates: Requirements 3.2, 3.3**

  - [x] 12.3 Write property test for reconciliation skipping tombstoned pools
    - **Property 9: Reconciliation skips tombstoned pools**
    - **Validates: Requirements 3.4**

- [x] 13. Implement pool-aware replication tracking (brightchain-api-lib)
  - [x] 13.1 Update replication tracking in `AvailabilityAwareBlockStore` to be pool-scoped
  ...
  - [x] 13.2 Write property test for pool-scoped replication counting
    - **Property 14: Pool-scoped replication counting**
    - **Validates: Requirements 6.1, 6.2**

- [x] 14. Implement distinct location records for different pool placements (brightchain-api-lib)
  - [x] 14.1 Update `IAvailabilityService` implementation to treat location records with different poolIds as distinct
  ...
  - [x] 14.2 Write property test for distinct location records
    - **Property 13: Distinct location records for different pool placements**
    - **Validates: Requirements 5.5**

- [x] 15. Update exports and ensure cross-project compatibility
  - [x] 15.1 Ensure all new types are exported from `brightchain-lib` public API
...
- [x] 16. Full scale testing
  - Write multi-node end to end testing that exercises all of these scenarios with real, un-mocked components
  
- [x] 17. Final checkpoint - Ensure all tests pass across both projects
  - Ensure all tests pass, ask the user if questions arise.
