...
- [ ] 1. Create stub types and bridge interfaces for upstream compatibility
  - [ ] 1.1 Create `brightchain-api-lib/src/lib/interfaces/brightchain-init-result.ts` with `IBrightChainInitResult<TID>` stub
  ...
  - [ ] 1.2 Create no-op factory functions in `brightchain-api-lib/src/lib/upstream-stubs.ts`
...
- [ ] 2. Update BrightChain's AppRouter to be compatible with upstream Application
  - [ ] 2.1 Make `AppRouter<TID>` in `brightchain-api-lib/src/lib/routers/app.ts` extend the upstream `AppRouter` or implement a compatible interface
...
- [ ] 3. Refactor App to extend upstream Application
  - [ ] 3.1 Update `App<TID>` in `brightchain-api-lib/src/lib/application.ts` to extend upstream `Application` instead of local `BaseApplication`
  ...
  - [ ] 3.2 Update `App` constructor to pass required arguments to `super()`
  ...
  - [ ] 3.3 Refactor `App.start()` to delegate to `super.start()`
  ...
  - [ ] 3.4 Resolve server access for WebSocket attachment
  ...
  - [ ] 3.5 Refactor `App.stop()` to delegate to `super.stop()`
...
- [ ] 4. Preserve DocumentStore functionality
  - [ ] 4.1 Move DocumentStore management from local `BaseApplication` into `App`
  ...
  - [ ] 4.2 Simplify or remove local `BaseApplication` in `brightchain-api-lib/src/lib/application-base.ts`
...
- [ ] 6. Update environment configuration
  - [ ] 6.1 Update `brightchain-api/src/.env.example` with Let's Encrypt env vars and descriptions
  ...
  - [ ] 6.2 Verify that `Environment_Local` inherits `letsEncrypt` getter from upstream
...
- [ ] 7. Write tests for the refactored App
  - [ ] 7.1 Write unit tests for constructor and inheritance
  ...
  - [ ] 7.2 Write unit tests for start/stop lifecycle
  ...
  - [ ] 7.3 Write unit tests for API surface compatibility
  ...
  - [ ] 7.4 Write property test for hostname parsing round-trip
  ...
  - [ ] 7.5 Write property test for ILetsEncryptConfig JSON round-trip
...
- [ ] 9. Verify existing tests still pass
  - [ ] 9.1 Run the full brightchain-api-lib test suite
    - Run: `NX_TUI=false npx nx test brightchain-api-lib --outputStyle=stream`
    - Fix any regressions caused by the refactor (changed imports, removed BaseApplication, etc.)
    - _Requirements: 7.1, 7.2, 7.3, 7.4_

  - [ ] 9.2 Run the full workspace build to check for compilation errors
...
- [ ] 10. Final checkpoint - All tests and builds pass
  - Ensure all tests pass, ask the user if questions arise.
