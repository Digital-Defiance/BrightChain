      ...
      fc.property(
        ...
        (orgIns, orgSkip, roleIns, roleSkip, invIns, invSkip) => {
          ...
          expect(output).toContain(
            `organizations ${orgIns} inserted / ${orgSkip} skipped`,
          );
          expect(output).toContain(
            `healthcareRoles ${roleIns} inserted / ${roleSkip} skipped`,
          );
          expect(output).toContain(
            `invitations ${invIns} inserted / ${invSkip} skipped`,
          );
        },
      ),
      { numRuns: 100 },
...
describe('Feature: org-role-dev-seeding, Property 2: Idempotent upsert preserves existing data', () => {
  ...
  it('only missing records get insertOne called; pre-existing are untouched; inserted + skipped = 7', () => {
    fc.assert(
      fc.asyncProperty(
        fc.subarray(ALL_SEED_IDS, {
          minLength: 0,
          maxLength: ALL_SEED_IDS.length,
        }),
