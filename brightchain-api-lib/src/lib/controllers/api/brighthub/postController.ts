...
interface IPostHandlers extends TypedHandlers {
  ...
  pinPost: ApiRequestHandler<IApiMessageResponse | ApiErrorResponse>;
  unpinPost: ApiRequestHandler<IApiMessageResponse | ApiErrorResponse>;
}
...
> {
  ...
  protected initRouteDefinitions(): void {
    this.routeDefinitions = [
      ...
      routeConfig('post', '/:id/pin', {
        handlerKey: 'pinPost',
        useAuthentication: true,
        useCryptoAuthentication: false,
        openapi: {
          summary: 'Pin a post to profile',
          description: 'Pin a post to the author\'s profile. Only the post author can pin their own posts.',
          tags: ['BrightHub Posts'],
          responses: {
            200: { schema: 'MessageResponse', description: 'Post pinned' },
            403: { schema: 'ErrorResponse', description: 'Not authorized' },
          },
        },
      }),
      routeConfig('delete', '/:id/pin', {
        handlerKey: 'unpinPost',
        useAuthentication: true,
        useCryptoAuthentication: false,
        openapi: {
          summary: 'Unpin a post from profile',
          description: 'Unpin a post from the author\'s profile.',
          tags: ['BrightHub Posts'],
          responses: {
            200: { schema: 'MessageResponse', description: 'Post unpinned' },
            403: { schema: 'ErrorResponse', description: 'Not authorized' },
          },
        },
      }),
    ...
    this.handlers = {
      ...
      pinPost: this.handlePinPost.bind(this),
      unpinPost: this.handleUnpinPost.bind(this),
    };
  }
  ...
  private async handleGetThread(
    ...
    try {
      ...
      // Resolve author profiles for all posts in the thread
      const authors = await this.resolveThreadAuthors(thread);

      return {
        statusCode: 200,
        response: { message: 'OK', data: { ...thread, authors } },
      };
    } catch (error) {
  ...
  /**
   * Resolve author profiles for all posts in a thread.
   */
  private async resolveThreadAuthors(
    thread: { rootPost: IBasePostData<string>; replies: IBasePostData<string>[] },
  ): Promise<Record<string, IBaseUserProfile<string>>> {
    if (!this.userProfileService) return {};

    const authorIds = new Set<string>();
    authorIds.add(thread.rootPost.authorId);
    ...
    await Promise.all(
      [...authorIds].map(async (id) => {
        try {
          authors[id] = await this.userProfileService!.getProfile(id);
        } catch {
  ...
  /**
   * POST /api/brighthub/posts/:id/pin
   * Pin a post to the author's profile.
   */
  private async handlePinPost(
    req: unknown,
  ): Promise<IStatusCodeResponse<IApiMessageResponse | ApiErrorResponse>> {
    try {
      const { id } = (req as { params: { id: string } }).params;
      ...
      const service = this.getPostService();
      await (service as unknown as { pinPost(postId: string, userId: string): Promise<void> }).pinPost(id, userId);

      // Update user profile's pinnedPostId
      if (this.userProfileService) {
        try {
          ...
          // Non-fatal — post is still pinned even if profile update fails
        }
      }

      return { statusCode: 200, response: { message: 'Post pinned' } };
    } catch (error) {
      if (error instanceof PostServiceError)
        return this.mapPostServiceError(error);
      return handleError(error);
  ...
  /**
   * DELETE /api/brighthub/posts/:id/pin
   * Unpin a post from the author's profile.
   */
  private async handleUnpinPost(
    req: unknown,
  ): Promise<IStatusCodeResponse<IApiMessageResponse | ApiErrorResponse>> {
    try {
      const { id } = (req as { params: { id: string } }).params;
      ...
      const service = this.getPostService();
      await (service as unknown as { unpinPost(postId: string, userId: string): Promise<void> }).unpinPost(id, userId);

      // Clear user profile's pinnedPostId
      if (this.userProfileService) {
        try {
          await this.userProfileService.updateProfile(userId, {
      ...
      return { statusCode: 200, response: { message: 'Post unpinned' } };
    } catch (error) {
      if (error instanceof PostServiceError)
        return this.mapPostServiceError(error);
      return handleError(error);
