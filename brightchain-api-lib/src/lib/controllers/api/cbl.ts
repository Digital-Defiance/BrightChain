...
> extends BaseController<TID, any, CBLHandlers, CoreLanguageCode> {
  ...
  constructor(application: IBrightChainApplication<TID>) {
    ...
    if (!registeredStore) {
      throw new Error(
        'No block store registered in the service container for CblController.',
      );
    }
    this.blockStore = registeredStore;
  }
  ...
  public async retrieveCBL(req: Request): Promise<{
    ...
    if (magnetUrl) {
      ...
      return {
        ...
        response: {
          success: false,
          message: 'Either magnetUrl or both b1 and b2 parameters are required',
        },
      };
    }
