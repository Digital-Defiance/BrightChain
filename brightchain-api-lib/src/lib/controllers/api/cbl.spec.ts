  ...
  beforeEach(() => {
    ...
    const mockApp = {
      ...
      services: (() => {
        ...
        const map = new Map<string, unknown>([['blockStore', blockStore]]);
        return {
          get: <T>(key: string) => map.get(key) as T,
          ...
          async (seed: number) => {
            ...
            try {
              ...
              if (response.status !== 200) {
                // Transient supertest/Express race under parallel test load
                // — socket recycling can yield empty-body 4xx/5xx. Skip.
                if (
              ...
              if (retrieveResponse.status !== 200) {
                if (
                  Object.keys(retrieveResponse.body ?? {}).length === 0 &&
                  retrieveResponse.status >= 400
                ) {
                  return;
                }
                throw new Error(
