...
export class EciesCryptoCore {
  ...
  public getHeaderSize(
    ...
    const dataLengthSize =
      options?.dataLengthSize ?? ECIES.MULTIPLE.DATA_LENGTH_SIZE;
    return (
  ...
  public encrypt(
    ...
    const maxDataSize = options?.maxDataSize ?? ECIES.MULTIPLE.MAX_DATA_SIZE;
    if (
      (preamble?.length ?? 0) +
        message.length +
  ...
  public parseSimpleOrSingleEncryptedMessage(
    ...
    if (
      ...
      throw new EciesError(
        ...
        undefined,
        {
          required: String(ECIES.MULTIPLE.FIXED_OVERHEAD_SIZE),
    ...
    const iv = data.subarray(offset, offset + ECIES.IV_LENGTH);
    ...
    const dataLengthBuffer = includeLengthAndCrc
      ? data.subarray(offset, offset + ECIES.MULTIPLE.DATA_LENGTH_SIZE)
      : Buffer.alloc(0);
    ...
    return {
      header: {
        ...
        iv,
        authTag,
        // dataLength, // Not part of ISingleEncryptedParsedHeader
        // crc16, // Not part of ISingleEncryptedParsedHeader
