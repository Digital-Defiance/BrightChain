...
// Mock the service wrapper so the controller constructor doesn't require a block store.
// The endpoints under test use brightTrustStateMachine / auditLogService, not the wrapper.
jest.mock('../../services/brightTrust', () => ({
  BrightTrustServiceWrapper: jest.fn().mockImplementation(() => ({
    getService: jest.fn(),
  })),
}));
