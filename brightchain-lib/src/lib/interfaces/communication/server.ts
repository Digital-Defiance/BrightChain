...
export interface IServer<TId = string, TData = string> {
  ...
  name: string;
  iconUrl?: string;
  /** FontAwesome icon class (e.g. "fa-solid fa-gamepad"). Takes precedence over iconUrl when set. */
  iconFaClass?: string;
  iconAssetId?: TId;
  iconVaultContainerId?: TId;
  ownerId: TId;
  memberIds: TId[];
  channelIds: TId[];
...
export interface IServerUpdate {
  name?: string;
  iconUrl?: string;
  /** FontAwesome icon class (e.g. "fa-solid fa-gamepad") */
  iconFaClass?: string;
  iconAssetId?: string;
