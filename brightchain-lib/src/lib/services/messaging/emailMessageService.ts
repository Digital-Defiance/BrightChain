...
export interface IEmailServiceConfig {
  ...
  /**
   * When `true`, all recipients (including those whose domain matches
   * `canonicalDomain`) are treated as external and routed through the
   * registered `ExternalEmailDispatcher` (e.g. SES / Postfix).
   *
   * Use this when the canonical domain is served by a real SMTP provider
   * (such as AWS SES) rather than by BrightChain gossip delivery, so that
   * same-domain users actually receive their email.
   *
   * @default false
   */
  relayAllRecipientsExternally: boolean;
}
...
export const DEFAULT_EMAIL_SERVICE_CONFIG: IEmailServiceConfig = {
  ...
  nodeId: 'localhost',
  relayAllRecipientsExternally: false,
};
...
export interface IEmailInput {
  ...
  /**
   * Digital Burnbag vault container ID for email attachments.
   * Pre-populated by InboundProcessor after creating a private vault.
   * Stored in IEmailMetadata so the UI can construct per-attachment download links.
   */
  vaultContainerId?: string;

  // ─── Folder / Spam Routing ────────────────────────────────────────────

  /** Mail folder for this email (e.g. 'inbox', 'spam'). Defaults to 'inbox'. */
  folder?: string;

  /** Spam score from anti-spam engine (e.g. SpamAssassin). */
  spamScore?: number;

  /** Spam classification derived from score vs. configured thresholds. */
...
export interface IAttachmentInput {
  ...
  /**
   * Digital Burnbag vault file ID for this attachment.
   * Pre-populated by InboundProcessor after uploading to a private vault.
   * Stored in the resulting IAttachmentMetadata for UI download links.
   */
  vaultFileId?: string;
}
...
export interface IInboxQuery {
  ...
  /** Filter by folder (e.g. 'inbox', 'spam', 'sent'). */
  folder?: string;

  /** Filter by label/keyword tags. */
  labels?: string[];
 ...
 */
/**
 * Callback invoked once per `sendEmail` when external recipients are present.
 * Implementations are responsible for relaying the message to a real
 * outbound email transport (e.g. SES, Postfix). Local recipients (matching
 * `canonicalDomain`) are filtered out by `EmailMessageService` before this
 * callback fires.
 *
 * Recipients are exposed split by header field so the dispatcher can preserve
 * RFC 5322 envelope semantics (To/CC are visible to all recipients; BCC is
 * not). When no external recipients exist for a given field, that array is
 * empty.
 */
export type ExternalEmailDispatcher = (input: {
  messageId: string;
  from: IMailbox;
  externalTo: IMailbox[];
  externalCc: IMailbox[];
  externalBcc: IMailbox[];
  subject: string;
  textBody?: string;
  htmlBody?: string;
}) => Promise<void> | void;
...
export class EmailMessageService {
  ...
  private externalDispatcher?: ExternalEmailDispatcher;
  ...
  /**
   * Register a dispatcher that delivers messages destined for recipients
   * outside the canonical domain to a real outbound email transport
   * (e.g. SES, Postfix). The dispatcher is invoked once per `sendEmail`
   * when at least one external recipient exists, with local recipients
   * already filtered out.
   */
  public setExternalDispatcher(
    dispatcher: ExternalEmailDispatcher | undefined,
  ): void {
  ...
  async sendEmail(email: IEmailInput): Promise<ISendEmailResult> {
    ...
    try {
      ...
      await this.dispatchExternalRecipients(email, messageId);
      return {
        messageId,
        brightchainMessageId,
    ...
    // 5c. Hand off external recipients to the configured outbound transport
    //     (e.g. SES). Local recipients are filtered out by canonicalDomain.
    await this.dispatchExternalRecipients(email, messageId);
  ...
  /**
   * Store an inbound email received from an external source.
   *
   * Identical to `sendEmail` up through metadata construction and storage,
   * but deliberately skips the external-dispatch step so that inbound
   * messages are never relayed back out through SES / Postfix.
   *
   * Use this from the InboundProcessor instead of `sendEmail()` to avoid
   * creating a relay loop.
   *
   * @param email - Parsed inbound email input
   * @returns Send result with messageId and brightchainMessageId
   */
  async receiveEmail(email: IEmailInput): Promise<ISendEmailResult> {
    ...
    const metadata: IEmailMetadata = {
      ...
      textBody: email.textBody,
      htmlBody: email.htmlBody,
      vaultContainerId: email.vaultContainerId,
      folder: email.folder ?? 'inbox',
      spamScore: email.spamScore,
      spamClassification: email.spamClassification,
      isStarred: false,
      attachments: undefined,
      deliveryReceipts: new Map(),
      readReceipts: new Map(),
    ...
    try {
      ...
      return {
        ...
        deliveryStatus: new Map<string, IDeliveryReceipt>(),
        success: false,
        error: `Failed to store inbound email metadata: ${err instanceof Error ? err.message : String(err)}`,
      };
    }
  ...
  private isExternalMailbox(mailbox: IMailbox): boolean {
    if (this.config.relayAllRecipientsExternally) {
      return true;
    }
  ...
  /**
   * Collect all external recipients (To + CC + BCC) and hand them to the
   * configured outbound dispatcher. Local recipients (matching
   * `canonicalDomain`) are filtered out. Errors from the dispatcher are
   * caught and logged so they cannot fail the BrightMail send pipeline.
   */
  private async dispatchExternalRecipients(
    email: IEmailInput,
    messageId: string,
  ): Promise<void> {
    if (!this.externalDispatcher) {
      return;
    }
    const externalTo = this.partitionRecipients(email.to ?? []).external;
    const externalCc = this.partitionRecipients(email.cc ?? []).external;
    const externalBcc = this.partitionRecipients(email.bcc ?? []).external;
    if (
      externalTo.length === 0 &&
      externalCc.length === 0 &&
    ...
    try {
      ...
      // Outbound transport failures are non-fatal for the BrightMail send.
      // Log so operators can diagnose; the message is already stored locally.

      console.error(
        `[email] external dispatcher failed for messageId=${messageId}:`,
        err,
      );
    }
  }
