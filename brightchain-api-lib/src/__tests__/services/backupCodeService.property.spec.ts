...
async function setup(username: string, email: string) {
  ...
  const { member: testMember } = Member.newMember(
    ecies,
    MemberType.User,
    username,
...
describe('BrightChainBackupCodeService Property-Based Tests', () => {
  ...
  describe('Property 2: Encrypt-then-validate round-trip', () => {
    it('each generated plaintext validates against stored codes; random strings do not', async () => {
      ...
      await fc.assert(
        fc.asyncProperty(
          usernameArb, emailArb, randomNonCodeArb,
          async (username, email, randomStr) => {
            const { service, memberStore, memberId } = await setup(username, email);
            ...
            for (const code of plaintextCodes) {
              expect(BackupCode.validateBackupCode(storedCodes, code)).toBe(true);
            }
            if (!plaintextCodes.includes(randomStr)) {
              expect(BackupCode.validateBackupCode(storedCodes, randomStr)).toBe(false);
            }
          },
        ),
        { numRuns: 20 },
  ...
  describe('Property 3: Consumption removes exactly one code', () => {
    it('useBackupCode removes exactly the matched code, leaving N-1', async () => {
      await fc.assert(
        fc.asyncProperty(
          usernameArb, emailArb, fc.nat({ max: TEST_CONSTANTS.Count - 1 }),
          async (username, email, codeIndex) => {
            ...
            const { newCodesArray, code: consumedCode } = service.useBackupCode(storedCodes, chosenCode);
            ...
            const expectedRemaining = originalChecksums.filter((cs) => cs !== consumedCode.checksum);
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (username, email, codeIndex) => {
            ...
            const originalPubKeyHex = Buffer.from(originalMember.publicKey).toString('hex');
            const plaintextCodes = await service.generateCodes(memberId);
            const chosenCode = plaintextCodes[codeIndex];
            const { user: recoveredUser, codeCount } =
              await service.recoverKeyWithBackupCode(memberId, chosenCode);
            const recoveredPubKeyHex = Buffer.from(recoveredUser.publicKey).toString('hex');
      ...
      await fc.assert(
        fc.asyncProperty(usernameArb, emailArb, async (username, email) => {
          ...
          for (const oldCode of oldCodes) {
            expect(BackupCode.validateBackupCode(storedCodes, oldCode)).toBe(false);
          }
          for (const newCode of newCodes) {
            expect(BackupCode.validateBackupCode(storedCodes, newCode)).toBe(true);
          }
        }),
        { numRuns: 20 },
      );
        ...
        fc.asyncProperty(
          ...
          async (username, email, codeIndex) => {
            ...
            const { member: newSysUser } = Member.newMember(
              ecies, MemberType.System, 'new-system-user', new EmailString('newsys@bc.org'),
            );
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const oldSysUser = (service as any).systemUser;
            const rewrapped = await service.rewrapAllUsersBackupCodes(oldSysUser, newSysUser);
