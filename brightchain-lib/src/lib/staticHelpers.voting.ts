import { KeyPair, PublicKey, PrivateKey } from 'paillier-bigint';
...
export abstract class StaticHelpersVoting {
  ...
  public static keyPairToEncryptedPrivateKey(keyPair: KeyPair, walletPublicKey: Buffer): Buffer {
    ...
    const encryptedPrivateKey = EthereumECIES.encrypt(walletPublicKey, Buffer.concat([lambdaLengthBuffer, lambda, mu]));
  ...
  public static encryptedPrivateKeyToKeyPair(encryptedPrivateKey: Buffer, walletPrivateKey: Buffer, votingPublicKey: PublicKey): PrivateKey {
    const decryptedPrivateKeyBuffer = EthereumECIES.decrypt(walletPrivateKey, encryptedPrivateKey);
