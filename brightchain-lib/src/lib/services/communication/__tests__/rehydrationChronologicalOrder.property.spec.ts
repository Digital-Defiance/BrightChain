...
describe('Feature: brightchat-persistence-rehydration, Property 3: Message chronological ordering', () => {
  ...
  it('consecutive messages within a fixed contextId satisfy m[i].createdAt <= m[i+1].createdAt', () => {
    ...
    fc.assert(
      fc.property(
        fc.array(arbValidMessage(fixedContextId), {
          minLength: 2,
          maxLength: 50,
        }),
        (messages) => {
  ...
  it('ordering holds independently per contextId when multiple groups exist', () => {
    ...
    fc.assert(
      fc.property(
        fc.tuple(
          fc.array(arbValidMessage(contextIds[0]), {
            minLength: 1,
            maxLength: 15,
          }),
          fc.array(arbValidMessage(contextIds[1]), {
            minLength: 1,
            maxLength: 15,
          }),
          fc.array(arbValidMessage(contextIds[2]), {
            minLength: 1,
            maxLength: 15,
          }),
        ),
