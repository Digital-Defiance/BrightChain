...
export function createIntegrationControllers(db?: InMemoryDb) {
  ...
  return {
    db: sharedDb,
    orgHandlers: (orgController as unknown as { handlers: OrgHandlers })
      .handlers,
    roleHandlers: (roleController as unknown as { handlers: RoleHandlers })
      .handlers,
    invHandlers: (invController as unknown as { handlers: InvHandlers })
      .handlers,
  };
}
