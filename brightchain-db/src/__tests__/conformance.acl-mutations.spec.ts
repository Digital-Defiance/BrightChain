...
describe('WriteAclManager mutations e2e (real ECDSA)', () => {
  ...
  it('setAcl with invalid signature is rejected', async () => {
    ...
    await expect(
      mgr.setAcl(acl, badSig, adminKeys.publicKey),
    ).rejects.toThrow();
  });
  ...
  it('addWriter with valid admin signature succeeds', async () => {
    ...
    expect(mgr.isAuthorizedWriter(newWriterKeys.publicKey, 'adddb')).toBe(
      false,
    );
  ...
  it('non-admin cannot add a writer', async () => {
    ...
    const fakeSig = await auth.signChallenge(
      fakePayload,
      nonAdminKeys.privateKey,
    );
