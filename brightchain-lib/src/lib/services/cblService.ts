...
// Correctly import the enum itself
...
export class CBLService {
  ...
  public calculateCBLAddressCapacity(
    blockSize: BlockSize,
    encryptedBlockType: BlockEncryptionType, // Corrected type name
    cbl?: {
      fileName: string;
      mimeType: string;
    },
  ): number {
    // Correctly determine blockType based on both encryption and extended info
    let blockType: BlockType;
    const isEncrypted = encryptedBlockType !== BlockEncryptionType.None;
    if (isEncrypted) {
      blockType = cbl
