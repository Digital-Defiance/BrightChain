  ...
  public async createMemberCbl(
    ...
    try {
      ...
      for (const block of blocks) {
        ...
        const checksumService = ServiceProvider.getInstance().checksumService; // Get ChecksumService
        await tuple.xor(checksumService, this.blockStore, metadata); // Corrected arguments
        tuples.push(tuple);
      }
  ...
  public async hydrateMember(
    ...
    try {
      ...
      for (const tuple of tuples) {
        ...
        const checksumService = ServiceProvider.getInstance().checksumService; // Get ChecksumService
        const xoredBlock = await tuple.xor(
          checksumService,
          this.blockStore,
          metadata,
        ); // Corrected arguments
        // Await the data promise and assert as Buffer, as we know xor stores a Buffer
        const blockData = (await xoredBlock.data) as Buffer;
      ...
      try {
        ...
        const member = BrightChainMember.fromJson(
          memberJson,
          eciesService,
          votingService,
        );
