...
export class MessagePassingService {
  ...
  /**
   * Register the outbound dispatcher used to relay messages addressed to
   * recipients outside the canonical email domain (e.g. SES, Postfix).
   * Local recipients are filtered out by `EmailMessageService` before the
   * dispatcher is invoked. Must be called after `configureEmail`.
   */
  setExternalEmailDispatcher(
    dispatcher: ExternalEmailDispatcher | undefined,
  ): void {
    this.getEmailService().setExternalDispatcher(dispatcher);
  }
   ...
   */
  public getEmailService(): EmailMessageService {
