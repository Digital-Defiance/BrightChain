...
describe('EthereumECIES', () => {
  ...
  test('walletAndSeedFromMnemonic should generate valid wallet and seed', () => {
    const { seed, wallet } = EthereumECIES.walletAndSeedFromMnemonic(testMnemonic);
  ...
  test('mnemonicToSimpleKeyPairBuffer should generate valid key pair', () => {
    const testKeyPair = EthereumECIES.mnemonicToSimpleKeyPairBuffer(testMnemonic);
    expect(testKeyPair).toHaveProperty('privateKey');
    expect(testKeyPair).toHaveProperty('publicKey');
    expect(testKeyPair.privateKey).toHaveLength(32);
    expect(testKeyPair.publicKey).toHaveLength(65);
  ...
  it('should derive a known key', () => {
    ...
    const expectedPublicKey = '0405b7d0996e99c4a49e6c3b83288f4740d53662839eab1d97d14660696944b8bbe24fabdd03888410ace3fa4c5a809e398f036f7b99d04f82a012dca95701d103';
  ...
  it('should sign a message', () => {
    ...
    const signature: Buffer = EthereumECIES.signMessage(keyPair.privateKey, message);
  ...
  it('should verify a signature using a 04 prefixed public key', () => {
    const message: Buffer = Buffer.from(faker.lorem.sentence());
    const signature: Buffer = EthereumECIES.signMessage(keyPair.privateKey, message);
    const verified: boolean = EthereumECIES.verifyMessage(keyPair.publicKey, message, signature);
    expect(verified).toBe(true);
  });
  it('should verify a signature using a public key straight from the wallet', () => {
    const message: Buffer = Buffer.from(faker.lorem.sentence());
    const wallet: Wallet = EthereumECIES.walletFromSeed(mnemonicToSeedSync(testMnemonic));
    expect(wallet).toBeInstanceOf(Wallet);
    const signature: Buffer = EthereumECIES.signMessage(wallet.getPrivateKey(), message);
    const verified: boolean = EthereumECIES.verifyMessage(wallet.getPublicKey(), message, signature);
    expect(verified).toBe(true);
  });
  ...
  it('should return false when a different public key is given', () => {
    ...
    expect(EthereumECIES.verifyMessage(newPublicKey, message, signature)).toBe(false);
  });
  it('should return false when a the message is altered', () => {
    const message: Buffer = Buffer.from(faker.lorem.sentence());
    const signature: Buffer = EthereumECIES.signMessage(keyPair.privateKey, message);
    message[0] = ' '.charCodeAt(0);
    expect(EthereumECIES.verifyMessage(keyPair.publicKey, message, signature)).toBe(false);
  });
  ...
  it('should return false when a the signature is altered', () => {
    ...
    expect(EthereumECIES.verifyMessage(keyPair.publicKey, message, signature)).toBe(true);
    expect(EthereumECIES.verifyMessage(keyPair.publicKey, message, modifiedSignature)).toBe(false);
  });
