...
const mockCalendarSidebar = jest.fn((props: Record<string, unknown>) => (
  <div
    data-testid="CalendarSidebar"
    data-calendars-count={String((props.calendars as unknown[]).length)}
  >
    <button
      data-testid="toggle-visibility"
      onClick={() => {
        const onVisibilityChange = props.onVisibilityChange as (
          s: Set<string>,
        ) => void;
...
jest.mock('@brightchain/brightcal-react-components', () => ({
  CalendarSidebar: (props: Record<string, unknown>) =>
    mockCalendarSidebar(props),
  CalendarWidget: (props: Record<string, unknown>) => (
    <div
      data-testid="CalendarWidget"
      data-events-count={String((props.events as unknown[]).length)}
    />
  ),
...
describe('CalendarPage', () => {
  ...
  it('renders CalendarSidebar in the sidebar slot of ResponsiveCalendarLayout', () => {
    ...
    expect(
      sidebarSlot.querySelector('[data-testid="CalendarSidebar"]'),
    ).toBeTruthy();
  });

  // Validates: Requirement 13.2
  it('passes updated visibilitySet to useEvents calendarIds when sidebar toggles', async () => {
    (loadVisibilitySet as jest.Mock).mockReturnValue(
      new Set(['cal-1', 'cal-2']),
    );
