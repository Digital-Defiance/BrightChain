/**
 * @fileoverview End-to-End tests for Block Storage, CBL, and BrightTrust operations
 *
 ...
 * 4. Sealing documents with BrightTrust members
 ...
 * - BrightTrustController / BrightTrustService / SealingService
 *
...
describe('E2E: Block Storage and BrightTrust Integration', () => {
  ...
  /**
   * Helper to create a BrightTrustService
   */
  ...
  // ============================================================================
  // SECTION 3: BrightTrust Document Sealing/Unsealing E2E Tests
  // ============================================================================

  describe('BrightTrust Document Sealing/Unsealing E2E', () => {
    it('should seal a document and unseal with minimum required shareholders', async () => {
      ...
      for (let i = 0; i < memberCount; i++) {
        const memberWithMnemonic = createTestMember(
          `BrightTrustMember${i}`,
          `brightTrust${timestamp}${i}@example.com`,
        );
        memberData.push(memberWithMnemonic);

        const metadata: BrightTrustMemberMetadata = {
          name: `BrightTrustMember${i}`,
          email: `brightTrust${timestamp}${i}@example.com`,
      ...
      const sealResult = await brightTrustService.sealDocument(
        memberData[0].member,
        secretDocument,
        memberIds,
        sharesRequired,
    ...
    it('should fail to unseal with insufficient shareholders', async () => {
      ...
      await expect(
        brightTrustService.unsealDocument(sealResult.documentId, [
          memberData[0].member,
          memberData[1].member,
        ]),
      ).rejects.toThrow();
    ...
    it('should correctly report can-unlock status', async () => {
      ...
      const canUnlockWith1 = await brightTrustService.canUnlock(
        sealResult.documentId,
        [memberIds[0]],
      );
      ...
      const canUnlockWith2 = await brightTrustService.canUnlock(
        sealResult.documentId,
      ...
      const canUnlockWithAll = await brightTrustService.canUnlock(
        sealResult.documentId,
        memberIds,
      );
    ...
    it('should handle member removal while preserving document access', async () => {
      ...
      // Remove member 2 from the BrightTrust
      await brightTrustService.removeMember(memberIds[2]);

      // Verify member 2 is no longer active
      const member2 = await brightTrustService.getMember(memberIds[2]);
  ...
  // ============================================================================
  // SECTION 4: Integrated Block + CBL + BrightTrust E2E Tests
  // ============================================================================

  describe('Integrated Block + CBL + BrightTrust E2E', () => {
    it('should store blocks, create CBL, seal CBL reference, and recover', async () => {
      ...
      // Step 3: Seal the file reference with brightTrust
      const sealResult = await brightTrustService.sealDocument(
        memberData[0].member,
        fileReference,
        memberIds,
      ...
      const unsealedReference = await brightTrustService.unsealDocument<
    ...
    it('should handle multiple sealed documents with different shareholder groups', async () => {
      ...
      const seal1 = await brightTrustService.sealDocument(
        memberData[0].member,
        doc1,
      ...
      const seal2 = await brightTrustService.sealDocument(
        memberData[2].member,
        doc2,
      ...
      const seal3 = await brightTrustService.sealDocument(
        memberData[0].member,
        doc3,
        memberIds,
        3,
      ...
      const unsealed1 = await brightTrustService.unsealDocument<typeof doc1>(
        seal1.documentId,
      ...
      const unsealed2 = await brightTrustService.unsealDocument<typeof doc2>(
        seal2.documentId,
      ...
      const unsealed3 = await brightTrustService.unsealDocument<typeof doc3>(
        seal3.documentId,
      ...
      const canUnlockDoc2WithMember1 = await brightTrustService.canUnlock(
        seal2.documentId,
    ...
    it('should demonstrate full workflow: store file, create CBL, seal access, distribute shares', async () => {
      ...
      const twoExecs = await brightTrustService.canUnlock(
        sealResult.documentId,
        [shareholders[0].id, shareholders[1].id],
      );
      expect(twoExecs.canUnlock).toBe(false);
      expect(twoExecs.sharesProvided).toBe(2);

      // CEO + CFO + CTO = 3 shares (enough!)
      const threeExecs = await brightTrustService.canUnlock(
        sealResult.documentId,
        [shareholders[0].id, shareholders[1].id, shareholders[2].id],
      );
      expect(threeExecs.canUnlock).toBe(true);
      expect(threeExecs.sharesProvided).toBe(3);

      // Unseal with CEO, CFO, and Legal (3 shareholders)
      const unsealedManifest = await brightTrustService.unsealDocument<
        typeof fileManifest
      >(sealResult.documentId, [
