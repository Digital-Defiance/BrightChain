...
// Mock brightmail-react-components to avoid the deep transitive module chain:
// brightmail-react-components → BrightMailLayout → brightchain-lib →
// encryptedBlockFactory → browserConfig → ECIESService (GUID validation).
// We provide the real handleApiCall logic inline since it's a simple pure function.
jest.mock('@brightchain/brightmail-react-components', () => {
  const { isAxiosError } = jest.requireActual('axios');
  return {
    handleApiCall: async (call: () => Promise<unknown>) => {
      try {
...
// IApiEnvelope is a type-only import; define a compatible shape for tests.
