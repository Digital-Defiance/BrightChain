...
/**
 * Ensure the i18n subsystem is fully bootstrapped so the upstream
 * setGlobalContextLanguageFromRequest middleware does not throw.
 */
...
describe('Email API – Multi-Node E2E Integration', () => {
  ...
  describe('Cross-node send and retrieve', () => {
    ...
    it('email sent via node-0 is retrievable via node-1 and node-2', async () => {
      ...
      // Note: the controller now derives `from` from the authenticated user's
      // mailbox (username @ configured emailDomain) and ignores any `from`
      // supplied in the request body. The test's mock auth resolves to
      // `userId = 'test-user'` and the controller's default emailDomain is
      // `brightchain.org`, so we assert against those server-derived values.
      expect(metadata.from.localPart).toBe('test-user');
      expect(metadata.from.domain).toBe('brightchain.org');
      ...
      expect(content.metadata.from.localPart).toBe('test-user');
      expect(content.metadata.from.domain).toBe('brightchain.org');
