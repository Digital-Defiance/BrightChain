...
describe('Property 1: Bug Condition — Document Store Divergence', () => {
      ...
      fc.asyncProperty(
        ...
        async ({ title, postId }) => {
          ...
          try {
            ...
            const collection =
              app.getModel<Record<string, unknown>>('brighthub_posts');
      ...
      fc.asyncProperty(
        ...
        async ({ memberA, memberB }) => {
          ...
          try {
            ...
            const chatStorageProvider = createChatStorageProvider((name) =>
              app.getModel(name),
            );
      ...
      fc.asyncProperty(
        ...
        async ({ memberId, vaultName, masterPassword }) => {
          ...
          try {
            ...
            const vaultMetadataCollection = app.services.has(
            ...
            // Query BrightDb (admin controller path)
            const brightDb = plugin.brightDb;
            const adminCollection = brightDb.collection('brightpass_vaults');
            const count = await adminCollection.countDocuments();
      ...
      fc.asyncProperty(
        ...
        async ({ messageId, subject, senderEmail }) => {
          ...
          try {
            ...
            const emailStore =
              app.services.get<IEmailMetadataStore>('emailMetadataStore');
            ...
            // Query BrightDb (admin controller path)
            const brightDb = plugin.brightDb;
            const adminCollection = brightDb.collection('brightmail_emails');
            const count = await adminCollection.countDocuments();
  ...
  it('Area 6: admin controllers use prefixed collection names matching service writes', () => {
    ...
    const adminChatSource = fs.readFileSync(
      path.resolve(__dirname, '../controllers/api/adminChat.ts'),
      'utf-8',
    );
    const adminPassSource = fs.readFileSync(
      path.resolve(__dirname, '../controllers/api/adminPass.ts'),
      'utf-8',
    );
    const adminMailSource = fs.readFileSync(
      path.resolve(__dirname, '../controllers/api/adminMail.ts'),
      'utf-8',
    );
